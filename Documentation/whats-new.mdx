...
id: whats-new
title: What's New
description: Latest updates and improvements across all Memoir packages
sidebar_position: 1
---

# What's New

Track the latest features, improvements, and fixes across the entire Memoir ecosystem.

<Callout type="info" title="Single Source of Truth">
This is the complete changelog for all Memoir packages. Individual package CHANGELOG.md files have been consolidated here for easier tracking.
</Callout>
...
The new `chat-sdk` template provides a production-ready chat application built on top of the Vercel AI SDK Chat UI components with full Memoir integration:
...
memoir init my-chat-app --template chat-sdk
cd my-chat-app
npm run dev
```
...
const share = await memoir.conversations.share({
...
```typescript
const snapshot = await memoir.conversations.snapshot({
  conversationId: 'conv-123',
...
const collab = await memoir.conversations.create({
  memorySpaceId: 'team-space',
  type: 'user-agent',
  participants: {
...
const images = await memoir.attachments.list({
  memorySpaceId: 'user-123-space',
  type: 'image',
});
```
...
# Streaming
await memoir.artifacts.start_streaming(
...
- `lib/layer-tracking.ts` re-exports from `@getmemoir/vercel-ai-provider/react`

Run `memoir update --sync-template` to get the updated quickstart files.
...
🔍 `memoir update` now auto-discovers apps in deployment directories  
...
The `update` command now scans deployment directories for Memoir apps that aren't registered in `~/.memoirrc`. This helps users who created apps before the `.memoir` registration system was introduced.

Detected patterns:
- `quickstart/` folders with Next.js + `@getmemoir/vercel-ai-provider`
- Any subdirectory with `@getmemoir/sdk` dependencies

```bash
$ memoir update --sync-template
...
**Version bumps:**
- `@getmemoir/sdk` 0.31.0 → 0.31.1
- `@getmemoir/cli` 0.31.0 → 0.31.1
- `@getmemoir/vercel-ai-provider` 0.29.0 → 0.29.1
- `memoir` (Python) 0.31.0 → 0.31.1
...
**Adaptive Batch Sizing for `memoir db clear`**
...
**`memoir db clear` fails on tables with embeddings**

The `memoir db clear` command would fail with "Too many bytes read in a single function execution (limit: 16777216 bytes)" when clearing tables containing records with vector embeddings (memories and facts tables).
...
<DataTable 
  ...
  rows={[
    ["MEMOIR_RECALL_LIMIT_MEMORIES", "20", "Max vector memories per recall"],
    ["MEMOIR_RECALL_LIMIT_FACTS", "15", "Max facts per recall"],
    ["MEMOIR_RECALL_GRAPH_HOPS", "2", "Graph traversal depth"],
    ["MEMOIR_RECALL_GRAPH_ENTITIES_PER_HOP", "5", "Entities expanded per hop"],
    ["MEMOIR_RECALL_GRAPH_RESULTS_PER_ENTITY", "3", "Results fetched per entity"],
    ["MEMOIR_RECALL_LIMIT_TOTAL", "30", "Final result cap"],
  ]} 
/>
...
```typescript
// Legacy (still works)
await memoir.memory.recall({ ..., limit: 25 });

// New (recommended)
await memoir.memory.recall({ ..., limits: { total: 25 } });
```
...
const facts = await memoir.facts.semanticSearch(memorySpaceId, embedding, {
  minConfidence: 80,
  limit: 20,
});
```
...
🔗 Graph sync now automatic when `MEMOIR_GRAPH_SYNC=true`  
...
Graph database synchronization for facts and entities is now automatic when `MEMOIR_GRAPH_SYNC=true`:
...
```typescript
// Before (v0.28.x)
await memoir.facts.store(params, { syncToGraph: true });

// After (v0.29.0+) - automatic when MEMOIR_GRAPH_SYNC=true
await memoir.facts.store(params);
```

APIs affected:
- `memoir.facts.store()`, `update()`, `delete()`
- `memoir.memory.remember()`, `forget()`, `delete()`
- `memoir.vector.store()`, `update()`, `delete()`
- `memoir.conversations.create()`, `addMessage()`, `delete()`
- `memoir.contexts.create()`, `update()`, `delete()`
- `memoir.memorySpaces.register()`

**Migration:** Remove `{ syncToGraph: true/false }` from all API calls. Set `MEMOIR_GRAPH_SYNC=true` in your environment to enable graph sync.
...
- **Removed `syncToGraph` option** - Graph sync is now automatic:
  ```typescript
  // Before (v0.28.x)
  await memoirMemory.remember("Hello", "Hi", { syncToGraph: true });
  
  // After (v0.29.0+) - automatic when enableGraphMemory is configured
  await memoirMemory.remember("Hello", "Hi");
  ```

#### Changed

- `MemoirMemoryProvider.doGenerate()` no longer passes `syncToGraph` to `remember()`
- `MemoirMemoryProvider.doStream()` no longer passes `syncToGraph` to `rememberStream()`
...
- **Update command version display** - `memoir update` now shows version transitions clearly:
...
- **Basic template graph initialization** - Now uses `Memoir.create()` for automatic graph configuration:
  - Async initialization enables auto-configuration from environment variables
  - `MEMOIR_GRAPH_SYNC=true` with `NEO4J_URI` or `MEMGRAPH_URI` auto-connects graph adapter
...
- **Dev-linked apps now refresh on update** - In dev mode, apps with `file:...` references are no longer skipped:
  ...
  - Fixes scenario: bump local SDK version → `memoir update --dev` → changes reflected
...
- **Automatic shell tab completion** - Tab completion auto-installs during `npm install -g @getmemoir/cli`:
  - Completes all commands, subcommands, and options with descriptions
  - Dynamic completion of deployment and app names from `~/.memoirrc`
  - Completion scripts installed to `~/.memoir/completions/`
  - Source line added automatically to shell RC files (idempotent)
  - Clean removal via preuninstall script on `npm uninstall -g`
  - Manual fallback: `memoir completion <zsh|bash|fish>` outputs completion script

#### Changed

- Package version display now shows "Vercel AI" instead of "AI" for clarity in `memoir update` output
...
Complete headless demo of Memoir SDK with both CLI and HTTP server modes:
...
- **Basic template tracking in CLI config** - Basic template projects are now registered in `memoir.config.json`:
  - `memoir init` automatically registers basic projects in the `apps` section
  - `memoir update --sync-template` now works with basic template projects
  - `memoir config list` shows basic projects alongside quickstart apps

- **Sessions and factHistory table support** - `memoir db clear` and `memoir db stats` now include all Convex tables:
...
- `memoir db clear` now clears 13 tables (was 11)
...
**`/api/chat-v6` route** now has full feature parity with the v5 route using `createMemoirMemoryAsync`:
...
```typescript
import {
  createMemoirCallOptionsSchema,  // Type-safe call options
  MemoirCallOptions,
...
const activeSessions = await memoir.sessions.getActive("user-123");
await memoir.sessions.expireIdle({ maxIdleMs: 30 * 60 * 1000 });
```
...
🔄 `memoir update` now checks all enabled deployments by default  
...
- **Multi-deployment update command** - `memoir update` now checks all enabled deployments:
...
- **Enhanced `memoir status` dashboard**:
...
🚀 Optional demo app installation during `memoir init`  
...
- **Template apps management**:
  - New `apps` section in config (`~/.memoirrc`)
  - Apps shown in `memoir config list`
  - `memoir start` automatically starts enabled apps

- **Default enabled** - Deployments and apps from `memoir init` enabled by default
...
- **Extended peerDependencies** - `@getmemoir/vercel-ai-provider` now accepts `ai` versions `^3.0.0 || ^4.0.0 || ^5.0.0 || ^6.0.0`
...
**Memoir CLI Integration**

🔧 Optional `@getmemoir/cli` installation during setup  
...
- **Memoir CLI Integration**:
  - New optional step to install `@getmemoir/cli` during project setup
  - Automatic CLI installation as dev dependency when selected
  - Adds scripts: `npm run memoir`, `memoir:setup`, `memoir:stats`, `memoir:spaces`
...
- **Non-interactive Convex setup in `memoir init`**:
...
🔧 Fixed `memoir dev` overwriting local deployment configs  
...
- **`memoir dev` overwriting `.env.local`** - Inherited `CONVEX_*` variables were polluting child processes
...
- **Secure password generation** - `memoir init` generates 20-character passwords for graph databases
...
🎉 First release of Memoir Provider for Vercel AI SDK  
...
- `createMemoirMemory()` - Factory function for memory-augmented models
...
- `memoirMemory.search()` - Manual memory search
- `memoirMemory.remember()` - Manual memory storage
- `memoirMemory.getMemories()` - Retrieve all memories
- `memoirMemory.clearMemories()` - Delete memories
...
**Deprecated**: The `create-memoir-memories` package has been superseded by `@getmemoir/cli`. Use `npx @getmemoir/cli init` instead.
</Callout>

**Initial Release**

🎉 Interactive CLI wizard for Memoir project setup  
...
- ~~`npm create memoir-memories`~~ → Use `npx @getmemoir/cli init`
...
🎉 First release of @getmemoir/cli  
...
🔗 Graph sync automatic with `MEMOIR_GRAPH_SYNC=true`  
...
🔍 `memoir update` auto-discovers unregistered apps in deployment directories  
📝 Prompts to register discovered apps for template sync  
🚀 Apps created before the registration system are now detected
...
🔄 `memoir update` checks all deployments  
...
🎉 Complete CLI for Memoir memory management
...
📦 New `/react` subpath export: `@getmemoir/vercel-ai-provider/react`  
📉 Reduces memory visualization boilerplate from ~170 lines to ~6 lines  
🚀 Quickstart template updated to use new helpers (`memoir update --sync-template`)
...
🔧 Optional `@getmemoir/cli` installation
...
Memoir packages follow semantic versioning:
