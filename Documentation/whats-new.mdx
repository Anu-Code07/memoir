...
- `lib/layer-tracking.ts` re-exports from `@memoir/vercel-ai-provider/react`
...
Detected patterns:
- `quickstart/` folders with Next.js + `@memoir/vercel-ai-provider`
- Any subdirectory with `@memoir/sdk` dependencies
...
**Version bumps:**
- `@memoir/sdk` 0.31.0 → 0.31.1
- `@memoir/cli` 0.31.0 → 0.31.1
- `@memoir/vercel-ai-provider` 0.29.0 → 0.29.1
...
- **Automatic shell tab completion** - Tab completion auto-installs during `npm install -g @memoir/cli`:
...
- **Extended peerDependencies** - `@memoir/vercel-ai-provider` now accepts `ai` versions `^3.0.0 || ^4.0.0 || ^5.0.0 || ^6.0.0`
...
🔧 Optional `@memoir/cli` installation during setup  
...
- **Memoir CLI Integration**:
  - New optional step to install `@memoir/cli` during project setup
...
**Deprecated**: The `create-memoir-memories` package has been superseded by `@memoir/cli`. Use `npx @memoir/cli init` instead.
</Callout>
...
- ~~`npm create memoir-memories`~~ → Use `npx @memoir/cli init`
...
📦 New `/react` subpath export: `@memoir/vercel-ai-provider/react`  
...
🔧 Optional `@memoir/cli` installation
