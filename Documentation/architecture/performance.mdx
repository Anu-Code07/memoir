...
Optimization techniques, scaling strategies, resilience layer, and performance characteristics of Memoir on Convex.

## Overview

Memoir is designed for **high performance** at scale with **built-in resilience**. With proper indexing, query patterns, and the resilience layer, Memoir handles millions of memories, thousands of memory spaces, and hundreds of concurrent users efficiently.
...
Memoir includes a **resilience layer** that protects all operations from overload and failures:
...
do {
  const page = await memoir.memory.list("agent-1", {
    cursor,
    limit: 100,
  });
...
await memoir.memory.search("agent-1", "user preferences", {
...
for (const item of items) {
  await memoir.memory.store("agent-1", item);
}
...
const contexts = await memoir.contexts.list({ memorySpaceId: "agent-1" });
const user = await memoir.users.get("user-123");
// 3× latency

// Parallel: Fast
const [memories, contexts, user] = await Promise.all([
  memoir.memory.search("agent-1", query),
  memoir.contexts.list({ memorySpaceId: "agent-1" }),
...
const allMemories = await Promise.all(
  allAgents.map((a) => memoir.memory.search(a.id, query)),
);
```
...
const results = await memoir.memory.search("agent-1", query, {
  limit: 20, // Don't return 1000s of results
});

// Don't load everything
const all = await memoir.memory.list("agent-1"); // Could be huge!
```
