...
High-level architecture of Memoir and how it leverages Convex.

## Architecture at a Glance

Memoir is built **natively on Convex** - a reactive TypeScript database with built-in vector search. We organize storage into **four layers** (ACID + Vector + Facts + Convenience) plus **coordination entities** and a **resilience layer**:

<ArchitectureDiagram title="Memoir System Architecture" caption="Four-layer architecture with resilience layer, coordination entities, and optional graph integration">
...
>
  <ArchitectureBox title="TypeScript SDK">
    memoir.memory.remember() → Orchestrates L1 + L2 + L3 • memoir.memory.recall() → Unified retrieval + ranking • TypeScript helpers over Convex functions • Automatic orchestration across all layers
  </ArchitectureBox>
</ArchitectureLayer>
...
await memoir.memory.remember({ memorySpaceId: "user-123-personal", ... });
await memoir.memory.search("user-123-personal", query, filters);
...
await memoir.memory.update(memorySpaceId, memoryId, { content: "New value" });
...
- **Automatic sync when `MEMOIR_GRAPH_SYNC=true`** - no manual `syncToGraph` needed
...
Memoir offers two levels of graph capabilities:
...
- Syncs Memoir data to Neo4j or Memgraph
- Enables deep traversals (6+ hops), complex pattern matching, and graph algorithms
- **Automatic sync when `MEMOIR_GRAPH_SYNC=true`** - no manual `syncToGraph` option needed
...
// Accessed via memoir.users.* wrapper API
```
...
<FlowDiagram title="Storing a Conversation Memory">
  ...
  <FlowNode title="1. memoir.memory.remember()" icon="code" variant="primary">
...
<FlowDiagram title="Searching Memories">
  ...
  <FlowNode title="1. memoir.memory.recall()" icon="code" variant="primary">
    SDK Layer 4 • Or: memoir.memory.search()
  </FlowNode>
  <FlowNode title="2. Resilience Layer" icon="shield" variant="secondary">
...
<ArchitectureLayer title="API Call" variant="primary" showArrow={true}>
  <ArchitectureBox title="memoir.users.delete()" subtitle="cascade: true">
...
Memoir includes a **resilience layer** that protects all operations from overload:
...
```
Your App → Memoir SDK → Your Convex Instance
```
...
```
Your App → Memoir Cloud API → Your Convex Instance
             ↓
      Memoir Services:
...
**Data stays in YOUR Convex instance** - Memoir Cloud is a management layer, not a data host.
...
- **Packaging:** npm package (`@memoir-platform/sdk`)
...
- **Embeddings:** Memoir-managed (OpenAI)
- **Pub/Sub:** Memoir-managed (Redis)
- **Agent Triggers:** Memoir-managed (webhooks)
- **Analytics:** Memoir-managed (aggregation service)
