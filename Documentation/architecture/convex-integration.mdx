...
How Memoir leverages Convex features for persistent memory, vector search, and real-time updates.

## Overview

Memoir is built **natively on Convex** - not as a wrapper, but as a first-class Convex application. We use Convex's features directly:
...
```typescript
// SDK call
const memory = await memoir.memory.get("agent-1", "mem_abc");
...
await memoir.memory.store('agent-1', { content, embedding });

// Cloud Mode (planned): Memoir action calls embedding API
...
```typescript
// Memoir SDK wraps Convex types
import { MemoryEntry } from "@memoir-platform/sdk";

// Convex Doc<"memories"> → MemoirEntry
class MemoirSDK {
  async get(
...
```typescript
// Memoir SDK catches Convex errors
try {
  await memoir.memory.store("agent-1", data);
} catch (error) {
  // Transform Convex error to Memoir error
  if (error.data?.code) {
    throw new MemoirError(
      error.data.code,
      error.data.message,
      error.data.details,
    );
...
// Memoir SDK uses your Convex client
...
### Cloud Mode (Memoir-Managed Functions) - Planned

```typescript
// Your Convex instance (just storage)
// No custom functions needed - Memoir Cloud provides them
...
// Memoir Cloud API (planned):
// - Deploys functions to your Convex
// - OR uses Memoir-hosted functions
