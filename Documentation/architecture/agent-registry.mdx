...
1. **Memory Spaces (`memoir.memorySpaces.*`)**: Define isolation boundaries and manage participants
2. **Agent Registry (`memoir.agents.*`)**: Track agent metadata, analytics, and capabilities
...
await memoir.memory.store('chatbot-v1', {
  content: 'User prefers dark mode',
  ...
});

await memoir.memory.store('support-agent', {
...
- **On-Demand**: Stats are computed on-demand when calling `memoir.agents.get()` or `memoir.agents.computeStats()`
...
await memoir.memory.store('test-agent-123', { ... });
await memoir.memory.store('dev-agent-temp', { ... });
...
const agentsWithStats = await Promise.all(
  allAgents.map(agent => memoir.agents.get(agent.id))
);
...
const billingAgents = await memoir.agents.search({
  capabilities: ["billing"], // Searches in metadata.capabilities
});

// Find by metadata
const productionAgents = await memoir.agents.search({
...
await memoir.memory.store('agent-2', { ... });
...
for (const agent of ['agent-1', 'agent-2']) {
  await memoir.agents.computeStats(agent);
}
...
await memoir.memory.store('test-agent-' + Date.now(), { ... });
// Just use string ID
```
...
// Find agents
const spanishSupport = await memoir.agents.search({
...
await memoir.agents.configure("demo-agent", {
  memoryVersionRetention: 1,
});
```
