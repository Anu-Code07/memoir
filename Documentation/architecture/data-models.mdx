...
Detailed Convex schema definitions, indexes, and data structures for all Memoir tables.

## Overview

Memoir uses **12+ Convex tables** to implement the 4-layer architecture plus coordination and governance:
...
The **ONLY special type** is `'user'` - it's accessed via `memoir.users.*` wrapper which provides GDPR cascade deletion:

```typescript
// User profile (SPECIAL - has memoir.users.* wrapper)
{
...
// Accessed via:
await memoir.users.get('user-123');  // ← Wrapper API
// Equivalent to:
await memoir.immutable.get('user', 'user-123');  // ← Direct API
```
...
**Key Point:** Only `type='user'` gets special API treatment (`memoir.users.*` wrapper). All other types are accessed via `memoir.immutable.*` directly.
