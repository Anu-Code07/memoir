...
sidebar_position: 9
description: Centralized control over data retention, purging, and compliance rules across all Memoir storage layers
---
...
Centralized control over data retention, purging, and compliance rules across all Memoir storage layers.

## Overview

Memoir provides built-in governance through the `memoir.governance.*` API. This enables enterprise-grade data lifecycle management with compliance templates for GDPR, HIPAA, SOC2, and FINRA.
...
await memoir.governance.setAgentOverride("audit-agent-space", {
  vector: {
    retention: {
...
await memoir.governance.setAgentOverride("temp-agent-space", {
  vector: {
    retention: {
...
await memoir.governance.setPolicy({
  organizationId: "org-123",
  ...(await memoir.governance.getTemplate("GDPR")),
});

// Tier 2: Department overrides
await memoir.governance.setAgentOverride("legal-dept-space", {
  conversations: {
    retention: {
...
await memoir.governance.setAgentOverride("poc-project-space", {
  vector: {
...
1. **Global defaults** (Memoir built-in)
...
```typescript
import { GovernanceValidationError } from "@memoir-platform/sdk";
