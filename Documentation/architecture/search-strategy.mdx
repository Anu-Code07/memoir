...
Memoir provides **four primary search strategies** that can be used independently or combined:
...
// SDK vector search
const results = await memoir.vector.search(memorySpaceId, query, {
...
The following shows how semantic search works internally. Use `memoir.vector.search()` or `memoir.memory.recall()` in your application code.
</Callout>
...
```typescript
// Using the Memoir SDK
const facts = await memoir.facts.search(
  memorySpaceId,
...
<Callout type="warning" title="Conceptual Example">
The following shows how facts search works internally. Use `memoir.facts.search()` or `memoir.memory.recall()` in your application code.
</Callout>
...
<Callout type="warning" title="Conceptual Example">
The following shows entity-centric fact retrieval. Use `memoir.facts.search()` with subject filters or `memoir.memory.recall()` for orchestrated search.
</Callout>

```typescript
// Conceptual: Query facts about a specific entity
// In practice, use memoir.facts.search() with subject filters
const userFacts = await memoir.facts.search(memorySpaceId, "*", {
  subject: "user-123",
  limit: 100,
});
...
```typescript
// Using the Memoir SDK
const results = await memoir.vector.search(memorySpaceId, "password Blue", {
  userId: "user-123",
  limit: 20,
});
...
<Callout type="warning" title="Conceptual Example">
The following shows how keyword search works internally. Use `memoir.vector.search()` or `memoir.memory.recall()` in your application code.
</Callout>
...
```typescript
// Using memoir.memory.recall() for recent memories
const result = await memoir.memory.recall({
...
<Callout type="warning" title="Conceptual Example">
The following shows how temporal search works internally. Use `memoir.memory.recall()` in your application code.
</Callout>
...
The `memoir.memory.recall()` method automatically combines multiple search strategies:
...
<Callout type="warning" title="Conceptual Example">
The following shows how `recall()` combines strategies internally. Use `memoir.memory.recall()` directly in your application code.
</Callout>

```typescript
// Conceptual: How recall() combines strategies internally
// In practice, use memoir.memory.recall() which handles this automatically
...
const vectors = await memoir.vector.search(memorySpaceId, query, {
  embedding,
  limit: 20,
});
...
<Callout type="warning" title="Conceptual Example">
The following shows how scoring works internally. `memoir.memory.recall()` applies this automatically.
</Callout>
...
The `memoir.memory.recall()` method automatically selects and combines strategies:
...
<Callout type="warning" title="Conceptual Example">
The following shows how strategy selection works internally. Use `memoir.memory.recall()` directly.
</Callout>
...
async function recall(memorySpaceId: string, query: string, options: any) {
  ...
  const facts = await memoir.facts.search(memorySpaceId, query, options);
  results.push(...facts.map(f => factToRecallItem(f, "facts")));

  // Search vectors if embedding provided
  if (options.embedding) {
    const vectors = await memoir.vector.search(memorySpaceId, query, options);
...
async function searchWithFallback(
  ...
  try {
    ...
    const recentResult = await memoir.memory.recall({
      memorySpaceId,
      query: "*",
      sources: {
        vector: { limit: 0 }, // Disable vector search
