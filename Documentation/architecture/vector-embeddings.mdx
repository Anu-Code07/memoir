...
Memoir is **embedding-agnostic** - bring your own embeddings or use Cloud Mode for automatic generation. We support any embedding model and any dimension size that fits your needs.

**Key Principle:** Memoir provides the storage and search infrastructure; you choose the embedding strategy.
...
<FlowDiagram title="Embedding Modes" caption="Direct Mode and Cloud Mode provide different levels of embedding management">
  ...
  <FlowNode title="Store with Memoir" icon="database" variant="success">
    Store with memoir.memory.store()
  </FlowNode>
  <FlowNode title="Memoir Indexes" icon="search" variant="primary">
    Memoir indexes in Convex vector search
  </FlowNode>
  <FlowNode title="Cloud Mode (Memoir Manages)" icon="cloud" variant="default">
    Set autoEmbed: true (planned)
  </FlowNode>
  <FlowNode title="Automatic Generation" icon="code" variant="warning">
    Memoir generates embeddings automatically (planned)
  </FlowNode>
  <FlowNode title="Automatic Storage" icon="database" variant="success">
    Memoir stores and indexes (planned)
  </FlowNode>
...
Memoir supports **any embedding model** that produces float64 arrays:
...
import { Memoir } from "@memoir-platform/sdk";
...
await memoir.memory.store("user-123-personal", {
...
// Use like OpenAI
await memoir.memory.store('my-memory-space', {
...
// await memoir.memory.store("agent-1", {
//   content: "User prefers dark mode",
//   contentType: "raw",
//   autoEmbed: true, // ← Memoir Cloud generates embedding (planned)
//   source: { type: "system", timestamp: new Date() },
...
// const results = await memoir.memory.search("agent-1", "user preferences", {
//   autoEmbed: true, // ← Memoir Cloud generates query embedding (planned)
//   limit: 10,
// });
```
...
For improved search accuracy, Memoir supports enrichment fields that enhance semantic search:
...
const results = await memoir.memory.search("agent-1", "user password", {
...
Memoir provides several options to refine search results:
...
async function hybridSearch(memorySpaceId: string, query: string) {
  ...
  const semanticResults = await memoir.memory.search(memorySpaceId, query, {
    embedding: await embed(query),
    limit: 20,
  });

  // 2. Keyword search (without embedding)
  const keywordResults = await memoir.memory.search(memorySpaceId, query, {
...
Memoir works **with or without** embeddings:
...
const results = await memoir.memory.search("agent-1", "email", {
...
await memoir.memory.update("agent-1", "mem-123", {
...
async function storeWithSelectiveEmbedding(
  ...
  await memoir.memory.store(memorySpaceId, {
    ...data,
...
async function expandedSearch(memorySpaceId: string, query: string) {
  ...
  const allResults = await Promise.all(
    embeddings.map((emb) =>
      memoir.memory.search(memorySpaceId, query, {
        embedding: emb,
        limit: 10,
      }),
    ),
...
async function semanticQuery(memorySpaceId: string, userQuery: string) {
  ...
  return await memoir.memory.search(memorySpaceId, improved, {
...
const context = await memoir.memory.search("chatbot", req.body.message, {
...
await memoir.vector.store("kb-agent", {
...
const results = await memoir.memory.search("any-agent", "refund policy", {
...
const logs = await memoir.memory.search("agent-1", "started", {
...
<FlowDiagram title="Cloud Mode Auto-Embed Pipeline" caption="Planned Cloud Mode embedding generation flow">
  <FlowNode title="SDK: autoEmbed=true" icon="code" variant="default">
    Developer calls memoir.memory.store() with autoEmbed: true
  </FlowNode>
  <FlowNode title="Memoir Cloud API" icon="cloud" variant="primary">
    Request received by Memoir Cloud service
  </FlowNode>
...
1. Developer calls `memoir.memory.store({ autoEmbed: true })`
2. Memoir Cloud receives request
...
const memories = await memoir.memory.list('agent-1');
...
if (memory.accessCount > 10) {
  await memoir.vector.update(memorySpaceId, memory.memoryId, {
...
async function upgradeDimensions(memorySpaceId: string) {
  const memories = await memoir.vector.search(memorySpaceId, "*", {
