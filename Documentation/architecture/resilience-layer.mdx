...
<Callout type="info" title="Built-in Resilience">
  All Memoir operations include automatic:
...
```typescript
import { getPresetForPlan } from '@getmemoir/sdk';
...
```typescript
import { ResiliencePresets } from '@getmemoir/sdk';
...
```typescript
import { CircuitOpenError, QueueFullError, AcquireTimeoutError } from '@getmemoir/sdk';
...
await memoir.shutdown(30000); // Default: 30s timeout
