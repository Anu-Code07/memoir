...
const chain = await memoir.contexts.getChain(childContextId);
...
await memoir.contexts.addParticipant(contextId, "legal-space");
...
const memory = await memoir.memories.store({
  memorySpaceId: "finance-space",
...
for (const context of [chain.root, ...chain.descendants]) {
  const memories = await memoir.memories.list({
...
async function createSequentialWorkflow(purpose: string, steps: any[]) {
  ...
  for (const step of steps) {
    const stepContext = await memoir.contexts.create({
...
async function createApprovalChain(request: any) {
  // Level 1: Request
  const requestCtx = await memoir.contexts.create({
  ...
  const managerCtx = await memoir.contexts.create({
  ...
  const financeCtx = await memoir.contexts.create({
...
const activeWorkflows = await memoir.contexts.list({
  memorySpaceId: "finance-space",
  status: "active",
});

// Get blocked workflows (needs attention)
const blockedWorkflows = await memoir.contexts.list({
  status: "blocked",
});
```
...
const parent = await memoir.contexts.get(parentId);
...
const chain = await memoir.contexts.getChain(contextId);

// Get just children (with optional recursion)
const children = await memoir.contexts.getChildren(contextId, {
