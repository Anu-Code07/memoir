...
Memoir's **Infinite Context** capability solves this by separating _conversation accumulation_ from _context retrieval_. Instead of sending all past messages to the LLM, Memoir retrieves only the most relevant facts and memories from unlimited history.
...
1. **Store each conversation exchange** in Memoir (L1a + L3 facts)
...
async function respondToUser(userMessage: string, memorySpaceId: string) {
  ...
  const retrieved = await memoir.memory.recall({
    memorySpaceId,
    query: userMessage,
...
```typescript
const facts = await memoir.memory.search(memorySpaceId, query, {
...
```typescript
const summaries = await memoir.memory.search(memorySpaceId, query, {
  embedding: await embed(query),
...
const relevantTickets = await memoir.memory.search(
  memorySpaceId,
...
const papers = await memoir.memory.search(memorySpaceId, query, {
  contentType: "fact",
...
// Governance policy
// Note: Verify governance API signature matches your Memoir version
await memoir.governance.addPolicy({
...
const context = await memoir.memory.search(memorySpaceId, query, {
  embedding: await embed(query),
...
if (context.length < 15) {
  const additional = await memoir.memory.search(memorySpaceId, query, {
    embedding: await embed(query),
    minImportance: 40,
...
async function adaptiveRetrieval(
  ...
  let facts = await memoir.memory.search(memorySpaceId, query, {
    contentType: "fact",
...
- With Memoir: Remember 100,000 messages perfectly
...
<ComparisonTable
  headers={["Capability", "Memoir", "Traditional LLM Apps"]}
  items={[
...
With Memoir Infinite Context (Retrieval-Based):
...
for (const topic of nextTopics) {
  await memoir.memory.search(memorySpaceId, topic); // Cache results
}
```
...
const categories = await memoir.memory.search(memorySpaceId, query, {
...
const contexts = await Promise.all([
  memoir.memory.search("user-123-personal", query),
  memoir.memory.search("user-123-work", query, {
...
**Infinite Context is Memoir's killer feature:**
...
This capability alone justifies Memoir's existence. It's not just an incremental improvement - it's a **fundamental architectural shift** from accumulation to retrieval.
