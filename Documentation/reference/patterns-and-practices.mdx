...
sidebar_position: 3
description: Common implementation patterns and best practices for building with Memoir
---

# Patterns & Best Practices

This guide covers proven patterns for common use cases and best practices to follow when building with Memoir.
...
const context = await memoir.memory.search(memorySpaceId, req.body.message, {
...
await memoir.mutable.update("inventory", "widget-qty", (qty) => {
...
const facts = await memoir.memory.search(
  "user-123-personal",
  "user employment",
  {
    userId: "user-123",
...
```typescript
// Run MCP server
// $ memoir-mcp-server --convex-url=$CONVEX_URL
...
// Note: memoir.graph.traverse() is planned for a future release.
// Currently, use the low-level graph adapter directly:
// import { CypherGraphAdapter } from "@getmemoir/sdk/graph";
...
Use `memoir.memory.*` for most operations - it handles the complexity for you:
...
// Preview
const count = await memoir.memory.count("user-123-personal", oldDebugLogs);

// Export
await memoir.memory.export("user-123-personal", {
  ...oldDebugLogs,
  format: "json",
});

// Delete
await memoir.memory.deleteMany("user-123-personal", oldDebugLogs);
```
...
try {
  await memoir.memory.store("user-123-personal", data);
} catch (error) {
...
<QuickNav>
  ...
  <QuickNavItem
    title="Core Concepts"
    description="Understanding Memoir fundamentals"
