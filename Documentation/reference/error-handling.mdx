...
Memoir uses structured errors with specific error codes for predictable error handling. Errors are categorized into:
...
```typescript
import { MemoryValidationError } from '@getmemoir/sdk';
...
```typescript
import { CircuitOpenError } from '@getmemoir/sdk';
...
```typescript
import { RateLimitExceededError } from '@getmemoir/sdk';
...
```typescript
import { QueueFullError } from '@getmemoir/sdk';
...
```typescript
import { GraphConnectionError } from "@getmemoir/sdk";
...
try {
  await memoir.memory.remember({
    // ... params
    // Graph sync happens automatically if MEMOIR_GRAPH_SYNC=true
  });
} catch (error) {
...
```typescript
import { GraphAuthenticationError } from "@getmemoir/sdk";
...
try {
  ...
  if (error instanceof GraphAuthenticationError) {
    ...
    console.error(`  Username: ${error.username}`);
    console.error("Check NEO4J_PASSWORD in .env");
    // Disable graph sync by unsetting MEMOIR_GRAPH_SYNC or setting to false
  }
}
```
...
async function rememberWithRetry(params, maxRetries = 3) {
  for (let i = 0; i < maxRetries; i++) {
    try {
      return await memoir.memory.remember(params);
    } catch (error) {
      if (error instanceof CircuitOpenError) {
...
MEMOIR_GRAPH_SYNC=true  # Enable graph sync
```
...
// Usage
await withRetry(() => memoir.memory.remember({...}));
```
...
await breaker.execute(() => memoir.memory.remember({...}));
```
...
```typescript
import { GraphConnectionError, CircuitOpenError } from "@getmemoir/sdk";

// v0.29.0+: Graph sync is automatic when MEMOIR_GRAPH_SYNC=true
// Graph failures are non-fatal - data is still stored in Convex
async function rememberWithFallback(params: RememberParams) {
  try {
    // Graph sync happens automatically if MEMOIR_GRAPH_SYNC=true
    return await memoir.memory.remember({
      ...params,
      extractFacts: true,
    });
  } catch (error) {
