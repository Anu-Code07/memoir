...
sidebar_position: 1
description: Understand how Memoir orchestrates memory across four layers with two-phase recall/remember architecture
---

# Memory Orchestration

The heart of Memoir: automatic orchestration across all memory layers with batteries-included defaults.
...
Memoir is an **AI memory orchestration platform** that manages the complete lifecycle of agent memory. Instead of manually coordinating conversations, vector stores, facts, and graphs, you call a single method and Memoir handles everything.
...
```typescript
// Without Memoir: Manual coordination
...
// Graph sync handled automatically when MEMOIR_GRAPH_SYNC=true (v0.29.0+)
...
```typescript
// With Memoir: One call, full orchestration
...
Memoir organizes memory into four layers, each serving a specific purpose:
...
>
  <FlowNode 
    ...
    variant="primary"
  >
    memoir.memory.remember() • memoir.memory.recall() • Start here
  </FlowNode>
  <FlowNode 
...
- `memoir.memory.*` handles everything automatically
...
await memoir.vector.search(memorySpaceId, query);
```
...
```typescript
// Layer 1a
await memoir.conversations.addMessage(conversationId, {...});

// Layer 1b
await memoir.immutable.store({...});

// Layer 1c
await memoir.mutable.set(namespace, key, value);
```
...
<Callout type="tip" title="Which Layer Should I Use?">
  **Start with Layer 4** (`memoir.memory.*`). It handles all orchestration for you.
...
Configure defaults via environment variables (`MEMOIR_RECALL_LIMIT_*`) or override per-call. See [Configuration](/getting-started/configuration#recall-limits-configuration).
</Callout>
...
await memoir.memory.remember({...});  // Stored in tenant-acme
await memoir.memory.recall({...});    // Only searches tenant-acme
```
...
**Memory Orchestration is the core value of Memoir:**
