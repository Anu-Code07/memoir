...
**v0.30.0**: Facts now support **semantic search** via embeddings. Configure `embedding` once at SDK init (or set `MEMOIR_EMBEDDING=true`) and embeddings are auto-generated for all `remember()` and `recall()` calls - no manual embedding code needed!
</Callout>
...
// Configure Memoir with LLM client
...
export MEMOIR_FACT_EXTRACTION=true
```
...
<Callout type="info" title="Automatic Conflict Resolution (v0.24.0+)">
  When you extract a fact that conflicts with an existing fact, Memoir handles it automatically:
...
const facts = await memoir.facts.search(spaceId, 'color preference');

// Or semantic search via memory API (embedding auto-generated!)
const memories = await memoir.memory.search(spaceId, 'what colors?', {
  // No manual embedding needed - auto-generated from query!
  contentType: 'fact',
  limit: 5,
});
...
const identity = await memoir.facts.list({
  memorySpaceId: spaceId,
  factType: 'identity',
});
```
...
const changes = await memoir.facts.history('fact-123');
...
const chain = await memoir.facts.getSupersessionChain('fact-123');
// Returns: [oldest] -> [older] -> [current]

// Get all versions of a fact
const versions = await memoir.facts.getHistory(spaceId, 'fact-123');
...
<Callout type="info" title="Benchmark Methodology">
  All models tested under identical conditions using Memoir's standard extraction prompt with structured JSON output. Metrics include extracted facts, enriched entities, relation triples, and end-to-end latency. Results may vary based on conversation complexity and model updates.
</Callout>

Memoir supports multiple OpenAI models for fact extraction. Here are the performance characteristics:
