...
const reviewBotMemories = await memoir.memory.list({
  memorySpaceId: 'team-engineering',
  participantId: 'code-review-bot',
});

// Audit who contributed what
const allMemories = await memoir.memory.list({ memorySpaceId: 'team-engineering' });
...
const memories = await memoir.memory.list({ memorySpaceId: 'team-workspace' });
...
const badMemories = await memoir.memory.list({
  memorySpaceId: 'team-workspace',
  participantId: 'buggy-bot',
});

for (const memory of badMemories) {
  await memoir.memory.delete('team-workspace', memory.id);
}
```
