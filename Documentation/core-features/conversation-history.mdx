...
// Retrieve history
const history = await memoir.conversations.getHistory(conv.id, {
  limit: 50,
  order: 'desc',
});
...
Conversations live in **Layer 1a** of Memoir's [4-layer architecture](/core-features/memory-orchestration). They work together with Layer 2 (Vector) for efficient retrieval:
...
// Note: memoir.a2a.send() handles this automatically (recommended)
const a2aConv = await memoir.conversations.create({
  type: 'agent-agent',
  participants: {
...
const full = await memoir.conversations.get(conv.id);
...
const convs = await memoir.conversations.list({
  type: 'user-agent',
...
Communication between agents. **Recommended:** Use `memoir.a2a.send()` which handles conversation creation automatically:
...
async function buildRecentContext(conversationId: string) {
  const history = await memoir.conversations.getHistory(conversationId, {
    limit: 10,
  });
