...
```typescript
const results = await memoir.memory.search(spaceId, query, {
...
```typescript
const results = await memoir.memory.search(spaceId, "dark mode", {
  strategy: "keyword",
});
```
...
Memoir chooses the best strategy automatically:
...
<Callout type="info">
  Memoir is **embedding-agnostic**. Use any provider that returns vectors.
</Callout>
...
const memories = await memoir.memory.search(spaceId, query, {
  embedding: await embed(query),
  userId: "user-123",
...
const critical = await memoir.memory.search(spaceId, query, {
  embedding: await embed(query),
  minImportance: 80,
...
// Temporal search
const thisMonth = await memoir.memory.search(spaceId, query, {
  embedding: await embed(query),
...
await memoir.memory.search(spaceId, query, {
  embedding: await embed(query),
  userId: userId,
});
...
```typescript
// Vague - poor results
await memoir.memory.search(spaceId, "user");

// Specific - better results
await memoir.memory.search(spaceId, 
...
```typescript
let memories = await memoir.memory.search(spaceId, query, {
  embedding: await embed(query),
  userId,
});
...
if (memories.length === 0) {
  ...
  memories = await memoir.memory.search(spaceId, query, {
    embedding: await embed(query),
    limit: 5,
  });
}
...
if (memories.length === 0) {
  ...
  memories = await memoir.memory.search(spaceId, "*", {
    userId,
    sortBy: "createdAt",
    sortOrder: "desc",
    limit: 10,
...
for (const query of queries) {
  ...
  await memoir.memory.search(spaceId, query, { embedding });
}

// Fast: batch embeddings first
const embeddings = await embedBatch(queries);
for (let i = 0; i < queries.length; i++) {
  await memoir.memory.search(spaceId, queries[i], { 
...
const all = await memoir.memory.search(spaceId, query, { limit: 100 });
const highPriority = all.filter(m => m.metadata.importance >= 80);

// Fast: filter during search
const highPriority = await memoir.memory.search(spaceId, query, {
  minImportance: 80,
  limit: 20,
});
```
...
async function buildContext(spaceId: string, userId: string, message: string) {
  const memories = await memoir.memory.search(spaceId, message, {
...
async function getFact(spaceId: string, userId: string, question: string) {
  const memories = await memoir.memory.search(spaceId, question, {
...
const total = await memoir.memory.count(spaceId);
if (total === 0) {
  console.log("Memory space is empty");
}

// Try broader query
const broader = await memoir.memory.search(spaceId, "preferences", {
