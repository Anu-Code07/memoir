...
<Callout type="info" title="What is a Memory Space?">
  A memory space is the **fundamental isolation boundary** in Memoir. Think of it like a personal hard drive, team workspace, or project folder—isolated from others but shared among authorized participants.
</Callout>
...
const memories = await memoir.memory.search("user-bob-personal", "secret");
...
const cursorMemories = await memoir.memory.list("user-123-personal", {
...
await memoir.memory.remember({ memorySpaceId: "project-apollo", ... });
...
await memoir.memorySpaces.delete("project-apollo", {
  cascade: true,
...
await memoir.memorySpaces.register({ memorySpaceId: "user-123-personal", ... });
