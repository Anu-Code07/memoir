...
const results = await memoir.memory.search(spaceId, "task deadlines");
...
const related = await memoir.memory.search(spaceId, "project milestones");
...
const results = await memoir.memory.search(spaceId, "Python web frameworks", {
...
const results = await memoir.memory.searchMultiSpace({
  spaces: [
...
const context = await memoir.contexts.create(personalSpace, {
...
await memoir.memory.remember(spaceId, {
  role: "user",
...
const semanticResults = await memoir.memory.search(spaceId, query, {
  embedding: await embed(query),
  limit: 20,
});

const keywordResults = await memoir.memory.search(spaceId, query, {
...
const results = await memoir.memory.search(spaceId, "deployment process", {
...
const fullMemory = await memoir.memory.get(spaceId, results[0].id, {
...
async function hybridSearch(spaceId: string, query: string) {
  const [semantic, keyword] = await Promise.all([
    memoir.memory.search(spaceId, query, {
      embedding: await embed(query),
      limit: 20,
    }),
    memoir.memory.search(spaceId, query, {
...
async function searchMultiSpace(spaces: string[], query: string) {
  const results = await Promise.all(
    spaces.map(spaceId => memoir.memory.search(spaceId, query, { limit: 10 }))
  );
