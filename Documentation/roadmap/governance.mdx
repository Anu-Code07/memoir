...
- Manual enforcement via `memoir.governance.enforce()`
...
await memoir.memory.store(spaceId, { content: "...", metadata: { importance: 10 } });
await memoir.memory.store(spaceId, { content: "...", metadata: { importance: 15 } });
// ... 20 more stores ...
...
// Store versions
await memoir.immutable.store(spaceId, { key: "config", value: { v: 1 } });
await memoir.immutable.store(spaceId, { key: "config", value: { v: 2 } });
await memoir.immutable.store(spaceId, { key: "config", value: { v: 3 } });
await memoir.immutable.store(spaceId, { key: "config", value: { v: 4 } });
await memoir.immutable.store(spaceId, { key: "config", value: { v: 5 } });
await memoir.immutable.store(spaceId, { key: "config", value: { v: 6 } });
...
await memoir.sessions.expireIdle(spaceId, {
  idleTimeoutMs: 30 * 60 * 1000,
});

await memoir.sessions.expireByDuration(spaceId, {
...
const validation = await memoir.governance.validatePolicy(spaceId, {
  vector: {
...
// Apply valid policy
await memoir.governance.setPolicy(spaceId, validPolicy);
```
...
**Planned Behavior:**
```typescript
const metrics = await memoir.governance.getEnforcementMetrics(spaceId, {
  period: "last-30-days",
});
...
for (const item of items) {
  await memoir.memory.store(spaceId, item);
}
