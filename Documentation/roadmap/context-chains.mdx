...
await memoir.contexts.update(spaceId, contextId, {
  status: "completed",
});
...
const parent = await memoir.contexts.get(spaceId, parentId);
const children = await memoir.contexts.getChain(spaceId, parentId);

const allComplete = children.every(c => c.status === "completed");
if (allComplete) {
  await memoir.contexts.update(spaceId, parentId, { status: "completed" });
}
```
...
await memoir.contexts.update(spaceId, childId, {
...
**Configuration:**
```typescript
await memoir.contexts.create(spaceId, {
...
const chain = await memoir.contexts.getChain(spaceId, contextId);
...
const context = await memoir.contexts.get(spaceId, contextId, {
...
const child = await memoir.contexts.addChild(spaceId, parentId, {
  purpose: "Fetch User Data",
  data: { timeout: 10000, endpoint: "/users" },
});

// Automatic merge
const childData = await memoir.contexts.get(spaceId, child.id, { inheritData: true });
...
for (const ctx of chain) {
  await memoir.contexts.addParticipant(spaceId, ctx.id, "user-123");
}
```
...
await memoir.contexts.addParticipant(spaceId, contextId, "user-123", {
...
for (let i = 0; i < 100; i++) {
  const child = await memoir.contexts.addChild(spaceId, currentId, {
    purpose: `Level ${i}`,
  });
  currentId = child.id;
}
...
await memoir.contexts.update(spaceId, contextId, { status: "completed" });
```
...
async function checkAndCompleteParent(spaceId: string, parentId: string) {
  const chain = await memoir.contexts.getChain(spaceId, parentId);
