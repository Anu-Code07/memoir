...
```typescript
// Preview deletion
const result = await memoir.memory.deleteMany(spaceId, {
...
if (result.wouldDelete < 100) {
  await memoir.memory.deleteMany(spaceId, {
    filters: { "metadata.importance": { $lt: 20 } },
  });
}
```
...
```typescript
// Preview updates
const result = await memoir.contexts.updateMany(spaceId, {
...
```typescript
const result = await memoir.contexts.deleteMany(spaceId, {
...
```typescript
// Search by purpose
const contexts = await memoir.contexts.list(spaceId, {
  filters: {
...
const data = await memoir.memory.export(spaceId);
await fs.writeFile("memories.json", data);

// Planned: Direct file writing
await memoir.memory.export(spaceId, {
...
```typescript
await memoir.contexts.export(spaceId, {
...
const data = await memoir.conversations.export(spaceId, conversationId);

// Planned: Filter messages
await memoir.conversations.export(spaceId, conversationId, {
...
```typescript
const sessions = await memoir.sessions.list(spaceId, {
...
```typescript
const result = await memoir.sessions.expireIdle(spaceId, {
...
```typescript
try {
  await memoir.sessions.get(spaceId, sessionId);
} catch (error) {
  if (error.code === MemoirErrorCode.SESSION_NOT_FOUND) {
    // Session doesn't exist
  } else if (error.code === MemoirErrorCode.SESSION_EXPIRED) {
    // Session expired
  } else if (error.code === MemoirErrorCode.SESSION_ALREADY_ENDED) {
...
const stats = await memoir.analytics.getAgentStats("unregistered-agent-456");
...
const allMemories = await memoir.memory.list({ memorySpaceId: spaceId });
