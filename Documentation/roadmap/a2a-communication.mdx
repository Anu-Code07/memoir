...
<Callout type="warning" title="Planned Features">
  These A2A communication enhancements are planned for future releases. Basic A2A messaging is fully implemented—see the [A2A Communication API](/api-reference/a2a-communication) for current capabilities.
</Callout>
...
- WebSocket connection from client to Memoir Cloud
...
const inbox = await memoir.memory.search(spaceId, "*", {
  filters: {
    "source.type": "a2a",
    "source.recipient": "agent-123",
  },
...
const sent = await memoir.memory.search(spaceId, "*", {
  filters: {
    "source.type": "a2a",
...
const inbox = await memoir.a2a.getInbox("agent-123", {
  status: "unread",
  limit: 20,
});

const sent = await memoir.a2a.getSent("agent-123", {
  limit: 20,
});

// Mark messages as read
await memoir.a2a.markRead(messageIds);
```
...
```typescript
const analytics = await memoir.analytics.getA2ACommunication("agent-123", {
  period: "last-30-days",
});
...
const suggestion = await memoir.a2a.suggestAgent({
...
const summary = await memoir.a2a.getDailySummary("agent-123", {
...
async function getInbox(agentId: string) {
  return await memoir.memory.search(spaceId, "*", {
    filters: {
      "source.type": "a2a",
...
async function markRead(messageIds: string[]) {
  for (const id of messageIds) {
    await memoir.memory.update(spaceId, id, {
...
async function pollInbox(agentId: string, interval: number = 5000) {
  setInterval(async () => {
    const unread = await memoir.memory.search(spaceId, "*", {
      filters: {
        "source.type": "a2a",
        "source.recipient": agentId,
...
async function getMessageStats(agentId: string) {
  ...
  const received = await memoir.memory.search(spaceId, "*", {
