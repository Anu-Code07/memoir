...
<Callout type="info" title="What is MCP?">
  The **Model Context Protocol** (MCP) is a standard for AI tools to share context. Memoir will provide an MCP server that enables tools like Cursor, Claude Desktop, and custom apps to share memory.
...
<Tabs groupId="install-method">
  ...
  <Step title="Start MCP server">
    <Terminal>memoir mcp start</Terminal>
  </Step>
...
<DataTable
  ...
  rows={[
    ["memoir_remember / /add_memories", "POST", "Store a memory"],
    ["memoir_search / /search_memory", "POST", "Search memories"],
    ["memoir_recall / /list_memories", "GET", "Get context for LLM"],
    ["memoir_get_facts / /get_memory", "GET", "Retrieve extracted facts"],
...
```json
{
  "mcpServers": {
    "memoir-cloud": {
      "url": "https://mcp.memoir.cloud/v1",
      "auth": {
        "type": "bearer",
        "token": "memoir_sk_your_api_key"
      }
    }
  }
}
