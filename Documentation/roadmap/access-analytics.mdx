...
<Callout type="warning" title="Planned Feature">
  The full `memoir.analytics.*` API is planned for a future release. Currently, basic access tracking is built-in to all memories (`accessCount`, `lastAccessed`), and memory space statistics are available via `memoir.memorySpaces.getStats()`. See the [Memory Space Operations API](/api-reference/memory-space-operations) for available methods.
</Callout>
...
```typescript
const memory = await memoir.memory.get(spaceId, memoryId);
...
const popular = await memoir.memory.search(spaceId, "*", {
  sortBy: "accessCount",
  sortOrder: "desc",
  limit: 10,
});
...
const unused = await memoir.memory.list({
...
const recentlyUsed = await memoir.memory.list({
  memorySpaceId: spaceId,
...
The planned `memoir.analytics.*` API will provide:
...
async function findDuplicateCandidates(spaceId: string) {
  const all = await memoir.memory.list({ memorySpaceId: spaceId, limit: 1000 });
...
async function cleanupUnused(spaceId: string) {
  ...
  for (const memory of safeToDelete) {
    await memoir.memory.delete(spaceId, memory.id);
  }
