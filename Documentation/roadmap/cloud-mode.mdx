...
title: Cloud Mode
sidebar_position: 3
description: Managed Memoir services with auto-embeddings, GDPR compliance, and infrastructure (Planned)
---

# Cloud Mode

<Callout type="warning" title="Planned Feature">
  Cloud Mode is a planned managed service offering. Currently, Memoir operates in "Direct Mode" where you manage your own Convex instance, embedding generation, and infrastructure.
</Callout>

<Callout type="info" title="What is Cloud Mode?">
  **Cloud Mode** is a managed service layer that sits between your application and your Convex instance. Your data still resides in your Convex account, but Memoir Cloud handles embeddings, GDPR compliance, pub/sub infrastructure, and automation.
  
  **Architecture:** Your App → Memoir Cloud API → Your Convex Instance
</Callout>
...
await memoir.memory.store(spaceId, {
...
```typescript
import { embed } from "@getmemoir/sdk";
...
const spaces = await memoir.memorySpaces.list({ participantId: "user-123" });

for (const space of spaces) {
  const conversations = await memoir.conversations.list({ participantId: "user-123" });
  for (const conv of conversations) await memoir.conversations.delete(space.id, conv.id);
  
  const memories = await memoir.memory.list({ memorySpaceId: space.id, participantId: "user-123" });
  for (const mem of memories) await memoir.memory.delete(space.id, mem.id);
  
  const facts = await memoir.facts.list({ memorySpaceId: space.id, userId: "user-123" });
  for (const fact of facts) await memoir.facts.delete(space.id, fact.id);
}
...
memoir.a2a.subscribe("agent-123", (message) => {
...
setInterval(async () => {
  const messages = await memoir.memory.search(spaceId, "*", {
    filters: {
      "source.type": "a2a",
...
await memoir.governance.setPolicy(spaceId, {
  vector: {
    maxRecords: 10000,
...
await memoir.governance.enforce(spaceId);
```
...
```typescript
const billing = await memoir.analytics.getAgentBilling("agent-123", {
  period: "monthly",
});
...
```typescript
await memoir.agents.update("agent-123", {
  limits: {
    maxMemories: 50000,
...
await memoir.memory.store(spaceId, { content: "..." });
// Error: Agent limit exceeded (50,001 / 50,000 memories)
```

---

### Memoir-Managed Functions

Deploy Memoir functions to your Convex instance with one command.

```typescript
// Option 1: Use Memoir-hosted functions (Cloud Mode)
const memoir = new Memoir({
  mode: "cloud",
  apiKey: "memoir_sk_...",
});
...
// memoir deploy --convex-url https://your-project.convex.cloud
```
...
<ArchitectureDiagram title="Cloud Mode Architecture">
  ...
  <ArchitectureLayer title="Memoir Cloud API Layer">
