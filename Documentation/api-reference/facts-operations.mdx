...
The Facts API (`memoir.facts.*`) provides structured knowledge storage with versioning, relationships, and temporal validity. Facts are memory-space scoped and can be automatically extracted from conversations or stored manually.
...
- **Automatic graph database sync (v0.29.0+)** - Facts and entities sync automatically when `MEMOIR_GRAPH_SYNC=true`
...
**New in v0.29.0**: Graph sync is now automatic when `MEMOIR_GRAPH_SYNC=true`. The `syncToGraph` option has been removed.
...
// Graph sync is automatic when MEMOIR_GRAPH_SYNC=true
...
const hiveFact = await memoir.facts.store({
...
const enrichedFact = await memoir.facts.store({
  memorySpaceId: "agent-1",
  userId: "user-123",
...
const semanticResult = await memoir.facts.storeWithDedup(
  {
    memorySpaceId: "agent-1",
...
```typescript
// Configure belief revision when creating Memoir instance with LLM
...
if (memoir.facts.hasBeliefRevision()) {
...
Use this method to change the LLM client or belief revision configuration after the Memoir instance has been created.

**Signature:**

```typescript
memoir.facts.configureBeliefRevision(
...
memoir.facts.configureBeliefRevision(newLLMClient);

// Update configuration only
memoir.facts.configureBeliefRevision(undefined, {
...
memoir.facts.configureBeliefRevision(openaiClient, {
  llmResolution: true,
...
```typescript
// TypeScript
memoir.facts.hasBeliefRevision(): boolean

// Python
memoir.facts.has_belief_revision() -> bool
```
...
if (memoir.facts.hasBeliefRevision()) {
  ...
  const result = await memoir.facts.revise(params);
} else {
  // Fall back to deduplication only
  const result = await memoir.facts.storeWithDedup(params, {
    deduplication: { strategy: "structural" }
  });
}
...
// Python
if memoir.facts.has_belief_revision():
    result = await memoir.facts.revise(params)
else:
    result = await memoir.facts.store_with_dedup(params, deduplication="structural")
```
...
```typescript
memoir.facts.revise(params: ReviseParams): Promise<ReviseResult>
```
...
```typescript
memoir.facts.checkConflicts(params: ReviseParams): Promise<ConflictCheckResult>
```
...
if (conflicts.hasConflicts) {
  ...
  if (await userConfirms()) {
    await memoir.facts.revise(params);
  }
}
```
...
const oldFact = await memoir.facts.get("agent-1", "fact-old-color");
...
```typescript
memoir.facts.history(factId: string, limit?: number): Promise<FactChangeEvent[]>
```
...
```typescript
memoir.facts.getChanges(filter: ChangeFilter): Promise<FactChangeEvent[]>
```
...
const userChanges = await memoir.facts.getChanges({
  memorySpaceId: "agent-1",
  userId: "user-123",
  limit: 50,
});
...
const rangeChanges = await memoir.facts.getChanges({
  memorySpaceId: "agent-1",
...
```typescript
memoir.facts.getSupersessionChain(factId: string): Promise<SupersessionChainEntry[]>
```
...
```typescript
memoir.facts.getActivitySummary(
  memorySpaceId: string,
  hours?: number
...
const summary = await memoir.facts.getActivitySummary("agent-1", 24);
...
memoir.facts.configureBeliefRevision(llmClient, {
  slotMatching: {
    enabled: true,
    predicateClasses: {
...
```typescript
const fact = await memoir.facts.get("agent-1", "fact-123");
...
const preferences = await memoir.facts.list({
  memorySpaceId: "agent-1",
  factType: "preference",
  subject: "user-123",
});
...
const uiFacts = await memoir.facts.list({
  memorySpaceId: "agent-1",
  tags: ["ui", "settings"],
  limit: 50,
});
...
const agentFacts = await memoir.facts.list({
  memorySpaceId: "shared-space",
...
const recentHighConfidence = await memoir.facts.list({
  memorySpaceId: "agent-1",
  userId: "user-123",
...
const foodFacts = await memoir.facts.search("agent-1", "food preferences", {
  factType: "preference",
  minConfidence: 80,
...
const recentFacts = await memoir.facts.search("agent-1", "user preferences", {
  userId: "user-123",
...
const participantFacts = await memoir.facts.search(
  "shared-space",
  "user info",
  {
...
const complexSearch = await memoir.facts.search("agent-1", "important facts", {
  userId: "user-123",
  minConfidence: 85,
  sourceType: "conversation",
...
const facts = await memoir.facts.semanticSearch("agent-1", embedding, {
  userId: "user-123",
  minConfidence: 80,
...
const preferences = await memoir.facts.semanticSearch("agent-1", preferenceEmbedding, {
  minConfidence: 70,
...
const updated = await memoir.facts.update("agent-1", "fact-123", {
  confidence: 99,
  tags: ["verified", "ui"],
});

// Mark fact as expiring
const expiring = await memoir.facts.update("agent-1", "fact-456", {
...
await memoir.facts.delete("agent-1", "fact-123");
```
...
const totalPreferences = await memoir.facts.count({
  memorySpaceId: "agent-1",
  factType: "preference",
});
...
const userFactCount = await memoir.facts.count({
  memorySpaceId: "agent-1",
  userId: "user-123",
});
...
const recentHighConfidence = await memoir.facts.count({
  memorySpaceId: "agent-1",
  userId: "user-123",
  minConfidence: 90,
...
const participantFactCount = await memoir.facts.count({
  memorySpaceId: "shared-space",
...
The Facts API now supports **universal filters** across all query operations (`list()`, `search()`, `count()`, `queryBySubject()`, etc.), making it consistent with Memoir's core design philosophy.

### Supported Universal Filters

All fact query operations accept these standard Memoir filters:
...
```typescript
// Same filter patterns work across Memoir
const filters = {
  userId: "user-123",
  createdAfter: new Date("2025-01-01"),
  tags: ["important"],
...
// Use in Memory API
await memoir.memory.list("agent-1", filters);

// Use in Facts API
await memoir.facts.list({ memorySpaceId: "agent-1", ...filters });
```
...
const emailAgentFacts = await memoir.facts.list({
  memorySpaceId: "shared-space",
  participantId: "email-agent",
  factType: "preference",
});

const profileAgentFacts = await memoir.facts.list({
  memorySpaceId: "shared-space",
  participantId: "profile-agent",
  factType: "identity",
});
...
const criticalRecentFacts = await memoir.facts.list({
  memorySpaceId: "agent-1",
  userId: "user-123",
  minConfidence: 90,
  factType: "preference",
...
const highQuality = await memoir.facts.list({
  memorySpaceId: "agent-1",
  minConfidence: 85, // Confidence >= 85
});

// Exact match
const exactFacts = await memoir.facts.list({
  memorySpaceId: "agent-1",
...
// Just preferences
const preferences = await memoir.facts.queryBySubject({
  memorySpaceId: "agent-1",
  subject: "user-123",
  factType: "preference",
});
...
const recentHighConfidence = await memoir.facts.queryBySubject({
  memorySpaceId: "agent-1",
  subject: "user-123",
...
// Hive Mode - filter by participant
const participantFacts = await memoir.facts.queryBySubject({
  memorySpaceId: "shared-space",
  subject: "user-123",
...
const preferences = await memoir.facts.queryByRelationship({
  memorySpaceId: "agent-1",
  subject: "user-123",
  predicate: "prefers",
});
...
// With universal filters
const recentPreferences = await memoir.facts.queryByRelationship({
  memorySpaceId: "agent-1",
  subject: "user-123",
  predicate: "prefers",
  userId: "user-123", // GDPR-friendly
...
// Hive Mode - filter by participant
const agentPreferences = await memoir.facts.queryByRelationship({
  memorySpaceId: "shared-space",
  subject: "user-123",
  predicate: "prefers",
...
```typescript
const history = await memoir.facts.getHistory("agent-1", "fact-123");
...
const jsonExport = await memoir.facts.export({
  memorySpaceId: "agent-1",
  format: "json",
});

// Export preferences as JSON-LD (linked data)
const linkedData = await memoir.facts.export({
  memorySpaceId: "agent-1",
  format: "jsonld",
  factType: "preference",
});
...
// Export as CSV
const csvExport = await memoir.facts.export({
  memorySpaceId: "agent-1",
  format: "csv",
});
```
...
result = await memoir.memory.remember(RememberParams(...))
...
const memories = await memoir.memory.search("agent-1", "user info", {
...
const result = await memoir.memory.forget("agent-1", "mem-123");
...
Memoir v0.15.0 introduces **enriched fact extraction** - a system for extracting facts with rich metadata that dramatically improves semantic search accuracy.
...
const results = await memoir.memory.search(memorySpaceId, query, {
...
When `MEMOIR_GRAPH_SYNC=true` is set in your environment, enriched facts automatically:
...
async function getUserProfile(memorySpaceId: string, userId: string) {
  const allFacts = await memoir.facts.queryBySubject({
    memorySpaceId,
    subject: userId,
  });
...
async function searchWithFactContext(
  ...
  const memories = await memoir.memory.search(memorySpaceId, query, {
    userId,
...
for (const factData of facts) {
  await memoir.facts.store(factData);
}

// Fast: Parallel storage
await Promise.all(facts.map((factData) => memoir.facts.store(factData)));
```
...
const all = await memoir.facts.list({ memorySpaceId: "agent-1", limit: 10000 });
...
const memories = await memoir.memory.search("agent-1", query);
const facts = await Promise.all(
  memories.map((m) => memoir.facts.queryBySubject({ subject: m.userId })),
);

// Efficient: Single enriched query
const enriched = await memoir.memory.search("agent-1", query, {
