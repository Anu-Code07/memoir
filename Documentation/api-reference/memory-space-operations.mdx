...
sidebar_position: 3
description: Complete API reference for memoir.memorySpaces.* namespace - managing memory spaces, participants, and access control.
---
...
This document covers the `memoir.memorySpaces.*` namespace for managing memory spaces, participants, and access control.

## Overview

**Memory spaces** are the fundamental isolation boundary in Memoir. The `memoir.memorySpaces.*` API provides explicit management and registration capabilities beyond the implicit creation that happens automatically when you first use a memorySpaceId.
...
<Callout type="info">
**v0.29.0+**: Graph sync is now automatic when `MEMOIR_GRAPH_SYNC=true`. The `syncToGraph` option has been removed.
</Callout>
...
// Graph sync is automatic when MEMOIR_GRAPH_SYNC=true (v0.29.0+)
```
...
```typescript
memoir.memorySpaces.get(memorySpaceId: string): Promise<MemorySpace | null>
```
...
```typescript
const space = await memoir.memorySpaces.get("user-123-personal");
...
```typescript
const space = await memoir.memorySpaces.get("unknown-space");
...
```typescript
memoir.memorySpaces.list(filters?: ListMemorySpacesFilter): Promise<ListMemorySpacesResult>
```
...
const personal = await memoir.memorySpaces.list({
  type: "personal",
  status: "active",
});
...
```typescript
// Get first 20
const page1 = await memoir.memorySpaces.list({ limit: 20, offset: 0 });

// Get next 20
if (page1.hasMore) {
  const page2 = await memoir.memorySpaces.list({ limit: 20, offset: 20 });
}
```
...
const sorted = await memoir.memorySpaces.list({
  sortBy: "name",
  sortOrder: "asc",
});

// Most recently updated first
const recent = await memoir.memorySpaces.list({
  sortBy: "updatedAt",
  sortOrder: "desc",
});
```
...
```typescript
memoir.memorySpaces.count(filter?: {
  type?: "personal" | "team" | "project" | "custom";
  status?: "active" | "archived";
}): Promise<number>
```
...
```typescript
// Count all spaces
const total = await memoir.memorySpaces.count();

// Count active personal spaces
const activePersonal = await memoir.memorySpaces.count({
  type: "personal",
  status: "active",
});
...
```typescript
memoir.memorySpaces.search(query: string, options?: SearchOptions): Promise<MemorySpace[]>
```
...
```typescript
const results = await memoir.memorySpaces.search("alice", {
  type: "personal",
  status: "active",
  limit: 5,
});
...
<Callout type="info">
**v0.29.0+**: Graph sync is now automatic when `MEMOIR_GRAPH_SYNC=true`. No options parameter needed.
</Callout>
...
```typescript
await memoir.memorySpaces.update("user-123-personal", {
...
```typescript
await memoir.memorySpaces.update("team-engineering-workspace", {
  metadata: {
    lastReview: Date.now(),
...
```typescript
await memoir.memorySpaces.update(
...
await memoir.memorySpaces.addParticipant("user-123-personal", {
  id: "github-copilot",
  type: "ai-tool",
  joinedAt: Date.now(),
});
...
await memoir.memorySpaces.removeParticipant("user-123-personal", "old-tool");
...
await memoir.memorySpaces.updateParticipants("team-workspace", {
...
```typescript
memoir.memorySpaces.findByParticipant(participantId: string): Promise<MemorySpace[]>
```
...
```typescript
memoir.memorySpaces.archive(memorySpaceId: string, options?: ArchiveOptions): Promise<MemorySpace>
```
...
const space = await memoir.memorySpaces.get("project-apollo");
...
await memoir.memorySpaces.archive(`user-${userId}-personal`, {
...
```typescript
memoir.memorySpaces.reactivate(memorySpaceId: string): Promise<MemorySpace>
```
...
await memoir.memorySpaces.reactivate(`user-${userId}-personal`);

const space = await memoir.memorySpaces.get(`user-${userId}-personal`);
...
{/* Note: syncToGraph removed in v0.29.0+ - graph cleanup is automatic when MEMOIR_GRAPH_SYNC=true */}
...
await memoir.memorySpaces.delete("project-old-2023", {
  cascade: true,
...
try {
  await memoir.memorySpaces.delete("user-123-personal", {
...
await memoir.memorySpaces.delete("production-space", {
  cascade: true,
...
```typescript
const stats = await memoir.memorySpaces.getStats("user-123-personal");
...
```typescript
// Last 7 days
const weekStats = await memoir.memorySpaces.getStats("user-123-personal", {
  timeWindow: "7d",
});
...
```typescript
const stats = await memoir.memorySpaces.getStats("team-engineering-workspace", {
  timeWindow: "7d",
...
async function addTeamMember(teamId: string, memberId: string) {
  await memoir.memorySpaces.addParticipant(`team-${teamId}-workspace`, {
    id: `bot-${memberId}`,
    type: "agent",
    joinedAt: Date.now(),
  });
...
async function memorySpaceLifecycle(projectId: string) {
  ...
  // await memoir.memory.remember({ memorySpaceId, ... });

  // 3. Complete project - archive
  await memoir.memorySpaces.archive(memorySpaceId, {
...
async function getMemorySpaceDashboard() {
  ...
  const dashboard = await Promise.all(
    result.spaces.map(async (space) => {
      const stats = await memoir.memorySpaces.getStats(space.memorySpaceId, {
        timeWindow: "7d",
        includeParticipants: true,
      });
...
await memoir.memory.remember({ memorySpaceId: "test-space", ... });
...
await memoir.memorySpaces.archive(memorySpaceId, { reason: "Inactive" });
...
```typescript
try {
  await memoir.memorySpaces.register({ memorySpaceId: "user-123-personal", type: "personal" });
} catch (error) {
...
```typescript
const space = await memoir.memorySpaces.get("nonexistent-space");
...
```typescript
try {
  await memoir.memorySpaces.delete("active-space", {
    cascade: false,
    reason: "Test",
  });
} catch (error) {
...
```typescript
try {
  await memoir.memorySpaces.delete("space-123", {
    cascade: true,
    reason: "Test",
...
**Questions?** Ask in [Discussions](https://github.com/Anu-Code07/memoir/discussions).
