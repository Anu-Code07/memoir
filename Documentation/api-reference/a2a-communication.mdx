...
The A2A Communication API (`memoir.a2a.*`) provides **convenience helpers** for inter-agent communication. It's not a separate storage system - it's syntactic sugar over the standard memory system with `source.type='a2a'`.
...
let conversationId;
const existing = await memoir.conversations.search({
  type: "agent-agent",
...
const msg = await memoir.conversations.addMessage(conversationId, {
  type: "a2a",
  from: "finance-agent",
  to: "hr-agent",
...
await memoir.vector.store("finance-agent", {
...
await memoir.vector.store("hr-agent", {
...
// That's 50+ lines vs 7 lines with memoir.a2a.send()!
```
...
<ComparisonTable
  ...
  items={[
    { feature: "memoir.a2a.send()", values: ["7", "Automatic", "Both agents / Low complexity"] },
    { feature: "memoir.memory.remember()", values: ["~20", "Automatic", "One agent (call 2x) / Medium complexity"] },
    { feature: "Manual (Layer 1+2)", values: ["50+", "Manual", "One agent (call 2x) / High complexity"] },
  ]}
/>

<Callout type="tip" title="Why memoir.a2a.send() is best for A2A">
...
<FlowDiagram title="A2A Architecture">
  ...
  <FlowNode title="Pub/Sub Infrastructure" icon="network" variant="warning">
    Direct Mode: Your Redis/RabbitMQ/NATS — Cloud Mode (planned): Memoir-managed infrastructure
  </FlowNode>
...
async function runAgent(memorySpaceId: string) {
  ...
  redis.on("message", async (channel, message) => {
    ...
    if (notification.type === "a2a-request") {
      // 1. Fetch from Memoir storage
      const request = await memoir.memory.get(
        memorySpaceId,
...
async function manualRequest(
  ...
  // Send via Memoir (stores in database)
  const sent = await memoir.a2a.send({
    from,
    to,
    message,
    metadata: {
...
- Memoir provides **storage APIs** (database operations)
- YOU provide **execution** (agent runners) + **pub/sub** (Redis/RabbitMQ)
- YOU wire them together (subscriptions, notifications)
- Memoir just provides the patterns and storage conventions

### Cloud Mode: Memoir Manages Everything
...
In Cloud Mode, Memoir will provide:
...
const ctx = await memoir.contexts.get(context.id);
...
async function handleIncomingRequests(memorySpaceId: string, agentId: string) {
  ...
  const allA2A = await memoir.vector.list({
    memorySpaceId,
    sourceType: "a2a",
    limit: 100,
  });
  ...
  for (const request of requests) {
    ...
    // Mark as responded
    await memoir.vector.update(memorySpaceId, request.memoryId, {
      metadata: {
        ...request.metadata,
        responded: true,
...
const convo = await memoir.a2a.getConversation("finance-agent", "hr-agent");
...
if (convo.canRetrieveFullHistory) {
  const fullHistory = await memoir.conversations.get(convo.conversationId);
  console.log(
...
const filtered = await memoir.a2a.getConversation("finance-agent", "hr-agent", {
...
const userRelated = await memoir.a2a.getConversation(
  "support-agent",
  "billing-agent",
  {
...
const allA2A = await memoir.memory.search("agent-1", "a2a communication", {
  sourceType: "a2a",
  limit: 100,
});

// Use list() for non-semantic filtering by source type
const allA2AList = await memoir.vector.list({
  memorySpaceId: "agent-1",
  sourceType: "a2a",
  limit: 100,
});
...
const sent = await memoir.vector.list({
  memorySpaceId: "agent-1",
  sourceType: "a2a",
...
const a2aMemories = await memoir.vector.list({
...
```typescript
// Total A2A messages
const total = await memoir.vector.count({
  memorySpaceId: "agent-1",
  sourceType: "a2a",
});
...
```typescript
// User makes request
const userMsg = await memoir.conversations.addMessage("conv-456", {
  role: "user",
...
const a2aResult = await memoir.a2a.send({
  from: "supervisor-agent",
  to: "specialist-agent",
...
const a2aMemory = await memoir.memory.get(
  "specialist-agent",
  a2aResult.receiverMemoryId,
);

// - Context chain
const ctx = await memoir.contexts.get(a2aMemory.metadata.contextId);

// - Original user conversation (via context)
const userConvo = await memoir.conversations.get(
...
for (const memory of oldLowImportance) {
  await memoir.vector.update("agent-1", memory.memoryId, {
...
for (const memory of trivialOld) {
  await memoir.vector.delete("agent-1", memory.memoryId);
}
...
- `memoir.memory.search()` - Semantic or text search
- `memoir.memory.get()` - Retrieve by ID
- `memoir.memory.update()` - Update (creates version)
- `memoir.memory.delete()` - Delete
- `memoir.memory.count()` - Count
...
const results = await memoir.memory.search("agent-2", "important update", {
...
const approvals = await memoir.memory.search("agent-1", "budget approval", {
  sourceType: "a2a",
...
The following features are planned for Memoir Cloud Mode. The SDK provides full A2A functionality today via Direct Mode.
...
for (const agent of directCollaborators) {
  const theirA2A = await memoir.vector.list({
...
```typescript
import { A2AValidationError } from "@getmemoir/sdk/a2a";
...
```typescript
import { A2ATimeoutError } from "@getmemoir/sdk";
