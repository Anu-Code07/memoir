...
sidebar_position: 1
description: Complete guide to navigating and using the Memoir API - a persistent memory system for AI agents built on Convex
---

# API Reference Overview

Welcome to the Memoir API Reference! This guide explains how to navigate the documentation and use Memoir effectively.

## What is Memoir?

Memoir is a **persistent memory system for AI agents** built on Convex. It provides:
...
- **[Memory Operations](/api-reference/memory-operations)** - The main API you'll use
  - Layer 4 convenience (`memoir.memory.*`)
...
- **[Memory Space Operations](/api-reference/memory-space-operations)** - Memory space management
  - Fundamental isolation boundary in Memoir
...
- **[Agent Management](/api-reference/agent-management)** - Optional agent metadata registry (analytics, discovery)
  ...
  - Use `memoir.memorySpaces.*` for production isolation
...
- **[Graph Operations](/api-reference/graph-operations)** - Graph database integration (v0.7.0+)
  ...
  - Automatic sync when `MEMOIR_GRAPH_SYNC=true` (v0.29.0+)
...
→ [Memory Operations](/api-reference/memory-operations#remember) - `memoir.memory.remember()`

**Search agent memories:**
→ [Memory Operations](/api-reference/memory-operations#search) - `memoir.memory.search()`

**Manage user profiles:**
→ [User Operations](/api-reference/user-operations) - `memoir.users.*`

**Delete user data (GDPR):**
→ [User Operations](/api-reference/user-operations#delete) - `memoir.users.delete({ cascade: true })`

**Track multi-agent workflows:**
→ [Context Operations](/api-reference/context-operations) - `memoir.contexts.*`

**Send agent-to-agent messages:**
→ [A2A Communication](/api-reference/a2a-communication) - `memoir.a2a.send()`

**Store shared knowledge:**
→ [Immutable Store](/api-reference/immutable-store-api) - `memoir.immutable.store()`

**Store live data (inventory, config):**
→ [Mutable Store](/api-reference/mutable-store-api) - `memoir.mutable.set()`

**Set retention policies:**
→ [Governance Policies](/api-reference/governance-policies-api) - `memoir.governance.*`

**Manage user sessions:**
→ [Sessions Operations](/api-reference/sessions-operations) - `memoir.sessions.*`

**Store and query facts:**
→ [Facts Operations](/api-reference/facts-operations) - `memoir.facts.*`

**Create versioned documents (artifacts):**
→ [Artifacts Operations](/api-reference/artifacts-operations) - `memoir.artifacts.*`
...
→ [Memory Operations](/api-reference/memory-operations) - `memoir.vector.*`

**Layer 3 (Facts Store):**
→ [Facts Operations](/api-reference/facts-operations) - `memoir.facts.*`

**Layer 4 (Convenience API):**
→ [Memory Operations](/api-reference/memory-operations) - `memoir.memory.*`
...
Memoir separates **storage** (immutable source) from **search** (optimized index):
...
>
  ...
  <FlowNode
    ...
    variant="default"
  >
    memoir.memory.* � Primary developer interface
  </FlowNode>
</FlowDiagram>

<Callout type="tip">
**Most developers use Layer 4** (`memoir.memory.*`) - it handles Layers 1, 2, and 3 automatically.
</Callout>
...
const conversation = await memoir.conversations.get('conv-456');
...
await memoir.memory.updateMany(memorySpaceId, filters, updates);
await memoir.memory.deleteMany(memorySpaceId, filters);
...
```typescript
// v1
await memoir.memory.store('user-123-personal', { content: 'Password is Blue', ... });

// v2 (v1 preserved in history)
await memoir.memory.update('user-123-personal', memoryId, { content: 'Password is Red' });
...
All Memoir operations are async:
...
try {
  ...
  if (error instanceof MemoirError) {
