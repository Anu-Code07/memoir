...
sidebar_position: 7
description: Complete API reference for configuring retention, purging, and governance rules across all Memoir layers.
---
...
Complete API reference for configuring retention, purging, and governance rules across all Memoir layers.

## Overview

The Governance Policies API provides centralized control over data retention, purging, and compliance rules across all Memoir storage layers.
...
await memoir.governance.setAgentOverride("enterprise-memory-space", {
  sessions: {
    lifecycle: {
...
await memoir.governance.setAgentOverride("banking-memory-space", {
  sessions: {
    lifecycle: {
...
await memoir.vector.update('agent-1', memoryId, {...});
...
const spaceResult = await memoir.governance.enforce({
...
```typescript
import { GovernanceValidationError } from "@getmemoir/sdk";
...
const basePolicy = await memoir.governance.getTemplate("GDPR");
...
const impact = await memoir.governance.simulate(newPolicy);
...
1. Global defaults (Memoir defaults)
