...
- **GDPR-enabled** - Allows `memoir.users.delete(userId, { cascade: true })` to find and delete this record
...
<Callout type="info">
Validation errors throw `ImmutableValidationError` (exported from `@getmemoir/sdk/immutable`). Backend errors are wrapped in standard `Error` objects.
</Callout>
...
```typescript
const history = await memoir.immutable.getHistory("policy", "max-refund");
...
const policy2 = await memoir.immutable.getAtTimestamp(
  "policy",
  "max-refund",
...
const userRecords = await memoir.immutable.list({
  userId: "user-123",
  limit: 20,
});

// List all records (up to default limit)
const allRecords = await memoir.immutable.list();
```
...
const allResults = await memoir.immutable.search({
...
```typescript
// Total KB articles
const total = await memoir.immutable.count({ type: "kb-article" });

// Count records for a specific user
const userCount = await memoir.immutable.count({
  userId: "user-123",
});

// Count all immutable records
const allCount = await memoir.immutable.count();
```
...
const result = await memoir.immutable.purge("kb-article", "old-article");
...
const userPurge = await memoir.immutable.purgeMany({
  userId: "user-123",
});
...
```typescript
memoir.immutable.purgeVersions(
  type: string,
  id: string,
  keepLatest: number
...
const result = await memoir.immutable.purgeVersions(
  "kb-article",
  "guide-123",
  20,
);
...
await memoir.immutable.purgeVersions("policy", "refund-policy", 5);
```
...
const current = await memoir.immutable.get("kb-article", "guide-1");
...
const article = await memoir.immutable.store({
  type: "kb-article",
  id: "refund-guide",
...
const results = await memoir.memory.search("support-agent", "refund policy");

// 4. Retrieve full article via immutableRef
if (results[0].immutableRef) {
  const fullArticle = await memoir.immutable.get(
...
const policy = await memoir.immutable.store({
  type: "policy",
  id: "refund-max",
  data: { value: 5000 },
});
...
await memoir.vector.store("policy-agent", {
...
await memoir.immutable.purge("kb-article", "outdated-guide");

// Purge by type or user
await memoir.immutable.purgeMany({
  type: "agent-reasoning",
});

// Clean up old versions (keep latest 20)
await memoir.immutable.purgeVersions("kb-article", "guide-123", 20);
```
...
const guide = await memoir.immutable.get("kb-article", "troubleshooting-login");
```
...
// Agents check policy
const policy = await memoir.immutable.get("policy", "refund-window");
...
const auditLogs = await memoir.immutable.list({
  type: "audit-log",
  limit: 100,
});
...
const userFacts = await memoir.immutable.list({
  type: "fact",
  userId: "user-123",
});

// Fact → Conversation (via conversationRef in data)
const fact = await memoir.immutable.get("fact", "fact-abc123");
if (fact?.data.conversationRef) {
  const sourceConvo = await memoir.conversations.get(
...
- Live/frequently changing data (use `memoir.mutable.*`)
- Searchable memories (use `memoir.vector.*` referencing immutable)
