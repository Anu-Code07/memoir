...
<Callout type="info">
**Architecture:** Memoir uses **Memory Spaces** as the fundamental isolation boundary. Multiple agents/tools can share one memory space (Hive Mode), or operate in separate spaces (Collaboration Mode). The Agent Registry is an optional metadata layer on top of this architecture for tracking agent information.
</Callout>
...
const agent = await memoir.agents.get("my-agent");
...
const troubleshooters = await memoir.agents.search({
  capabilities: ["troubleshooting"],
});

// Find recently registered agents (last 30 days)
const newAgents = await memoir.agents.search({
...
// Find by name
const found = await memoir.agents.search({
...
const teamAgents = await memoir.agents.list({
  metadata: { team: "support" },
});

// WARNING: offset + metadata may produce unexpected results
// const paginated = await memoir.agents.list({
//   metadata: { team: "alpha" },
...
// Count by status
const activeCount = await memoir.agents.count({
  status: "active",
});
...
const supportAgents = await memoir.agents.list({
  metadata: { team: "support" },
});
...
if (!(await memoir.agents.exists("support-agent"))) {
  await memoir.agents.register({
    id: "support-agent",
    name: "Support Bot",
  });
...
const updated = await memoir.agents.update("support-agent", {
  metadata: {
...
await memoir.agents.update("audit-agent", {
  config: {
...
await memoir.agents.configure("support-agent", {
...
const preview = await memoir.agents.unregister("agent-123", {
  cascade: true,
  dryRun: true,
});
...
const agent = await memoir.agents.get("agent-123");
...
const result = await memoir.agents.unregister("agent-xyz", {
...
<ComparisonTable
  headers={["Feature", "memoir.users.delete()", "memoir.agents.unregister()"]}
  items={[
...
await memoir.agents.unregister("agent-123", { cascade: true });
...
const agents = await memoir.agents.search({
  capabilities: ["troubleshooting", "billing"],
});

// Find agents with ALL capabilities
const specialists = await memoir.agents.search({
...
const team = await memoir.agents.search({
...
const active = await memoir.agents.search({
  sortBy: "lastActive",
  sortOrder: "desc",
  limit: 10,
});
...
<Callout type="note">
**PLANNED FEATURE:** A dedicated `memoir.analytics.getAgentStats()` API for retrieving statistics for unregistered agents is planned for a future release. Currently, stats are only computed for registered agents via the agents API.
</Callout>
...
const agent = await memoir.agents.get("agent-1");
console.log(
...
// Search
await memoir.agents.search(filters);

// Count
await memoir.agents.count(filters);

// List
await memoir.agents.list({ ...filters, limit: 50 });

// Update many
await memoir.agents.updateMany(filters, { metadata: { updated: true } });

// Unregister many
await memoir.agents.unregisterMany(filters);

// Export
await memoir.agents.export({ filters, format: "json" });
```
...
const cascaded = await memoir.agents.unregisterMany(
  { status: "archived" },
...
const preview = await memoir.agents.unregisterMany(
...
const withStats = await memoir.agents.export({
...
<FlowDiagram title="Agent Lifecycle">
  ...
  <FlowNode title="2. Registered (Optional)" icon="database" variant="secondary">
    memoir.agents.register() — Enables analytics, team organization, agent discovery
  </FlowNode>
  <FlowNode title="3. Configured" icon="settings" variant="warning">
    memoir.agents.configure() — Custom retention rules, team-specific settings
  </FlowNode>
  <FlowNode title="4. Unregistered (Optional)" icon="user" variant="success">
    memoir.agents.unregister() — Can still use with simple ID afterward
  </FlowNode>
</FlowDiagram>
...
<Callout type="info">
**Cloud Mode (Under Development)**: These enhanced agent management features will be available when the managed Memoir Cloud service launches. The SDK provides full technical capability today.
</Callout>
...
for (const agent of agents) {
  await memoir.agents.register(agent);
}

// Find agent for specific task
const technicalAgents = await memoir.agents.search({
  capabilities: ["troubleshooting"],
});
...
const agentMemories = await memoir.memory.search({
  memorySpaceId: "agent-1-space",
  query: "*",
});

// Agent → Contexts (via memory space)
const agentContexts = await memoir.contexts.list({
  memorySpaceId: "agent-1-space",
});

// Agent → Agent (collaboration graph via A2A)
const collaborations = await memoir.memory.search({
  memorySpaceId: "agent-1-space",
  query: "*",
  sourceType: "a2a",
});
