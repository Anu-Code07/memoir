...
The User Operations API (`memoir.users.*`) provides user profile management that enables **memories to follow users across tools**.
...
const memories = await memoir.memory.recall({
  memorySpaceId: 'user-123-space', 
...
```typescript
import { CypherGraphAdapter } from "@memoir-platform/sdk/graph";
...
```typescript
// Convenience
await memoir.users.get("user-123");

// vs Equivalent
await memoir.immutable.get("user", "user-123");
```

### Under the Hood

User profiles are stored in **`memoir.immutable.*`** with `type='user'`:
...
<ComparisonTable
  headers={["Feature", "memoir.users.*", "memoir.immutable.*", "memoir.mutable.*", "memoir.vector.*"]}
  items={[
...
- **ONLY `memoir.users.*` has GDPR cascade** - deletes from ALL stores with userId
- `memoir.users.*` is a wrapper over `memoir.immutable.*` with `type='user'`
...
const users = await memoir.users.list({
...
```typescript
import { Memoir, createAuthContext } from "@memoir-platform/sdk";
...
const profile = await memoir.users.get("user-123");
...
- `MemoirError('INVALID_USER_ID')` - User ID is empty or invalid
...
```typescript
const user = await memoir.users.update("user-123", {
...
const preview = await memoir.users.delete("user-123", {
  cascade: true,
...
Granular layer control is planned for Memoir Cloud. Currently, `cascade: true` deletes from ALL layers. The options below are not yet implemented.
</Callout>

```typescript
// 🚧 PLANNED - Memoir Cloud: Only delete mutable data (like sessions, cache)
const result = await memoir.users.delete("user-123", {
  cascade: true,
...
// - memoir.conversations.delete() for each conversation
// - memoir.immutable.purge() for each record
// - memoir.mutable.delete() for each key
// - memoir.vector.delete() for each memory
// - memoir.facts.delete() for each fact
...
- `MemoirError('USER_NOT_FOUND')` - User profile doesn't exist
...
```typescript
const users = await memoir.users.search({ limit: 10 });
...
```typescript
const page1 = await memoir.users.list({ limit: 10, offset: 0 });
...
const recentSignups = await memoir.users.count({
  createdAfter: Date.now() - 30 * 24 * 60 * 60 * 1000,
});
console.log(`New users (30 days): ${recentSignups}`);

// Count users active this week
const activeThisWeek = await memoir.users.count({
...
```typescript
const result = await memoir.users.updateMany(["user-1", "user-2", "user-3"], {
...
const alexResult = await memoir.users.updateMany(
  { displayName: "alex" },
  { data: { verified: true } },
);
```
...
const result = await memoir.users.deleteMany(["user-1", "user-2", "user-3"]);
...
const testUsers = await memoir.users.deleteMany(
...
const jsonExport = await memoir.users.export({ format: "json" });
console.log(jsonExport);

// Export with limit as CSV
const csvExport = await memoir.users.export({
  format: "csv",
  filters: { limit: 100 },
});
```
...
const recentExport = await memoir.users.export({
  format: "json",
  filters: {
    createdAfter: Date.now() - 30 * 24 * 60 * 60 * 1000,
    sortBy: "createdAt",
...
const augustProfile = await memoir.users.getAtTimestamp(
  "user-123",
...
const janProfile = await memoir.users.getAtTimestamp(
  "user-123",
  new Date("2025-01-01"),
);
const octProfile = await memoir.users.getAtTimestamp(
  "user-123",
  new Date("2025-10-01"),
);
...
```typescript
// Existing profile
const existing = await memoir.users.get("user-123");
...
async function getCompleteUserContext(
  ...
  // Get profile
  const profile = await memoir.users.get(userId);

  // Get memories using recall
  const memoryResult = await memoir.memory.recall({
    memorySpaceId,
    query,
    userId,
    limit: 10,
...
const recentUsers = await memoir.users.list({
...
async function ensureUserProfile(userId: string, message: string) {
  // Get or create
  let user = await memoir.users.get(userId);

  if (!user) {
    user = await memoir.users.update(userId, {
...
// Initialize Memoir with tenant-scoped auth
const memoir = new Memoir({
  convexUrl: process.env.CONVEX_URL!,
...
const tenantData = await memoir.users.export({
  format: "json",
  includeMemories: true,
  includeConversations: true,
});
...
await memoir.users.update("alex", {
  displayName: "Alex Johnson",
  role: "admin",
});
...
// Export tenant users
const exportData = await memoir.users.export({
  format: "json",
  includeConversations: true,
  includeMemories: true,
});
...
async function handleGDPRDeletion(userId: string) {
  ...
  const userData = await memoir.users.export({
    format: "json",
    includeMemories: true,
    includeConversations: true,
  });
...
Users are universal graph nodes - everything in Memoir can link to a user via the `userId` field:
...
```typescript
const result = await memoir.users.delete("user-123", { cascade: true });
...
```typescript
import { UserValidationError } from "@memoir-platform/sdk";

try {
  await memoir.users.update("", { name: "Test" }); // Empty userId
} catch (error) {
...
```typescript
import { CascadeDeletionError } from "@memoir-platform/sdk";
