...
All timestamps in the Memoir SDK are **Unix timestamps in milliseconds** (not JavaScript `Date` objects):

```typescript
// SDK returns timestamps as numbers (Unix ms)
const memory = await memoir.vector.get(memorySpaceId, memoryId);
...
await memoir.vector.getAtTimestamp(
  memorySpaceId,
  memoryId,
...
The Memory Operations API is organized into **namespaces** corresponding to Memoir's complete architecture:

```typescript
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// Layer 1: Three ACID Stores (Immutable Sources of Truth)
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
memoir.conversations.*   // Layer 1a: Conversations (memorySpace-scoped)
memoir.immutable.*       // Layer 1b: Shared immutable (NO memorySpace - TRULY shared)
memoir.mutable.*         // Layer 1c: Shared mutable (NO memorySpace - TRULY shared)

// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// Layer 2: Vector Index (memorySpace-scoped, Searchable)
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
memoir.vector.*          // Vector memory operations (memorySpace-scoped)

// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// Layer 3: Facts Store (memorySpace-scoped, Versioned) - NEW
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
memoir.facts.*           // LLM-extracted facts (memorySpace-scoped)

// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// Layer 4: Convenience API (Wrapper over L1a + L2 + L3)
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
memoir.memory.*          // Primary interface (recommended)

// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// Additional APIs
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
memoir.memorySpaces.*    // Memory space management (Hive/Collaboration)
memoir.users.*           // User profiles (shared across all spaces)
memoir.contexts.*        // Context chains (cross-space support)
memoir.a2a.*             // Inter-space messaging (Collaboration Mode)
memoir.governance.*      // Retention policies
memoir.graph.*           // Graph database integration
```

**Complete Architecture:**

<ArchitectureDiagram title="Memoir Four-Layer Architecture" caption="Data flows from Layer 1 (source of truth) through Layer 4 (convenience API)">
...
>
  <ArchitectureBox title="LLM-Extracted Facts">
    60-90% token savings • memoir.facts.* • Enables infinite context
  </ArchitectureBox>
</ArchitectureLayer>
...
>
  <ArchitectureBox title="memoir.memory.*">
...
**`memoir.memory.*` (Layer 4)** - Recommended for most use cases. Provides `remember()` / `recall()` for full orchestration, and `search()` / `get()` for quick retrieval.
</Callout>

<DataTable
  headers={["Namespace", "Layer", "Description"]}
  rows={[
    ["`memoir.memory.*`", "Layer 4", "**START HERE** - Full orchestration (recommended)"],
    ["`memoir.conversations.*`", "Layer 1a", "Direct ACID conversation access"],
    ["`memoir.vector.*`", "Layer 2", "Direct vector index control"],
    ["`memoir.facts.*`", "Layer 3", "Direct fact operations"],
    ["`memoir.immutable.*`", "Layer 1b", "Shared knowledge (NO memorySpace - TRULY shared)"],
    ["`memoir.mutable.*`", "Layer 1c", "Live mutable data (NO memorySpace - TRULY shared)"],
    ["`memoir.users.*`", "—", "User profiles (shared across ALL spaces + GDPR cascade)"],
    ["`memoir.governance.*`", "—", "Retention policies for all layers"],
  ]}
/>
...
```typescript
// Stores with userId can be deleted via memoir.users.delete(userId, { cascade: true })
await memoir.conversations.addMessage(convId, { userId: 'user-123', ... });
await memoir.immutable.store({ type: 'feedback', id: 'fb-1', userId: 'user-123', ... });
await memoir.mutable.set('sessions', 'sess-1', data, 'user-123');
await memoir.vector.store('user-123-personal', { userId: 'user-123', ... });

// One call deletes from ALL stores
await memoir.users.delete('user-123', { cascade: true });
```
...
```typescript
// When initializing Memoir with auth context, tenantId is auto-injected
...
await memoir.memory.remember({...});           // tenantId: 'tenant-acme'
await memoir.conversations.create({...});      // tenantId: 'tenant-acme'
await memoir.facts.store({...});               // tenantId: 'tenant-acme'
await memoir.immutable.store({...});           // tenantId: 'tenant-acme'
await memoir.mutable.set(...);                 // tenantId: 'tenant-acme'

// Queries are automatically filtered by tenant
const memories = await memoir.memory.search('user-space', 'query');
...
### Layer 1: memoir.conversations.\* (ACID)

```typescript
// Managing immutable conversation threads
await memoir.conversations.create({ type: 'user-agent', participants: {...} });
await memoir.conversations.addMessage(conversationId, message);
await memoir.conversations.get(conversationId);
await memoir.conversations.getHistory(conversationId, options);
// Returns raw messages, no Vector index involved
```

### Layer 2: memoir.vector.\* (Vector Index)

```typescript
// Managing searchable knowledge index
await memoir.vector.store(memorySpaceId, vectorInput); // Must provide conversationRef manually
await memoir.vector.get(memorySpaceId, memoryId);
await memoir.vector.search(memorySpaceId, query, options);
await memoir.vector.update(memorySpaceId, memoryId, updates);
// Direct Vector operations, you manage conversationRef
```

### Layer 4: memoir.memory.\* (Convenience API)

```typescript
// High-level operations that manage both layers
await memoir.memory.remember(params); // Stores in ACID + creates Vector index
await memoir.memory.get(memorySpaceId, memoryId, { includeConversation: true });
await memoir.memory.search(memorySpaceId, query, { enrichConversation: true });
...
<FlowDiagram title="Manual Flow" caption="You manage the linking between layers">
  <FlowNode title="Layer 1: memoir.conversations.addMessage()" icon="database" variant="primary">
    Returns: msg.id
  </FlowNode>
  <FlowNode title="Layer 2: memoir.vector.store()" icon="search" variant="warning">
...
<FlowDiagram title="Automatic Flow" caption="Single call handles everything">
  <FlowNode title="Layer 4: memoir.memory.remember()" icon="layers" variant="success">
...
const msg = await memoir.conversations.addMessage("conv-456", {
  role: "user",
...
const memory = await memoir.vector.store("user-123-personal", {
...
**Use `memoir.memory.*` to handle both layers automatically:**
...
// 1. memoir.conversations.addMessage() × 2  (ACID Layer 1)
// 2. memoir.vector.store() × 2              (Vector Layer 2)
...
const memory = await memoir.memory.store("user-123-personal", {
  content: "Agent initialized successfully",
  source: { type: "system" },
...
### Layer 1: memoir.conversations.\* Operations
...
### Layer 2: memoir.vector.\* Operations
...
### Layer 4: memoir.memory.\* Operations (Convenience API)
...
<ComparisonTable
  headers={["Operation", "Layer 2 (memoir.vector.*)", "Layer 4 (memoir.memory.*)"]}
  items={[
...
- Most operations are thin wrappers around `memoir.vector.*`
- Convenience for not having to remember namespaces
- Use `memoir.vector.*` directly if you prefer explicit control

---

## Core Operations (Layer 4: memoir.memory.\*)

<Callout type="note">
Layer 4 operations are convenience wrappers that orchestrate across all layers. For direct control, use Layer 1 (`memoir.conversations.*`), Layer 2 (`memoir.vector.*`), and Layer 3 (`memoir.facts.*`) separately.
</Callout>
...
<Callout type="info">
**v0.29.0+**: Graph sync is now automatic when `MEMOIR_GRAPH_SYNC=true` is set in your environment. The `syncToGraph` option has been removed from all APIs.
</Callout>
```
...
```typescript
// Configure Memoir with belief revision
const memoir = new Memoir({
...
<Callout type="note">
Fine-grained control over individual pipeline stages (slot matching, LLM resolution) is available through the `memoir.facts.revise()` API directly. The `remember()` integration uses a batteries-included approach - full pipeline when enabled, deduplication-only when disabled.
</Callout>
...
Set `MEMOIR_RECALL_LIMIT_MEMORIES`, `MEMOIR_RECALL_LIMIT_FACTS`, etc. in your environment for project-wide defaults. See [Configuration](/getting-started/configuration#recall-limits-configuration) for all environment variables.
</Callout>
...
<Callout type="tip">
**v0.30.0+**: When `embedding` is configured at SDK init (or `MEMOIR_EMBEDDING=true`), embeddings are automatically generated from the `query`. Manual embedding is still supported for override.
</Callout>
...
const sharedContext = await memoir.memory.recall({
...
interface StreamingOptions {
  // Graph sync (v0.29.0+): Automatic when MEMOIR_GRAPH_SYNC=true
...
```typescript
// Configure Memoir with embedding + LLM at init (v0.30.0+)
const memoir = new Memoir({
  convexUrl: process.env.CONVEX_URL!,
...
```typescript
try {
  const result = await memoir.memory.rememberStream(params, {
    partialFailureHandling: "store-partial",
    generateResumeToken: true,
  ...
  if (error instanceof ResumableStreamError) {
    ...
    const resumed = await memoir.memory.rememberStream({
      ...params,
...
```typescript
const enriched = await memoir.memory.search(
  "user-123-personal",
  "user preferences",
  {
    // Embedding auto-generated!
...
const vectorResults = await memoir.vector.search(
  "user-123-personal",
  query,
  options,
);
...
const results = await memoir.memory.search("user-123-personal", query, options);

// Layer 4 enriched (Vector + ACID)
const enriched = await memoir.memory.search("user-123-personal", query, {
  ...options,
...
- `MemoirError('INVALID_AGENT_ID')` - Agent ID is invalid
- `MemoirError('INVALID_EMBEDDING_DIMENSION')` - Embedding dimension mismatch
- `MemoirError('CONVEX_ERROR')` - Database error
...
```typescript
const memory = await memoir.memory.get("user-123-personal", "mem_abc123");
...
```typescript
const enriched = await memoir.memory.get("user-123-personal", "mem_abc123", {
...
const vectorMem = await memoir.vector.get("user-123-personal", "mem_abc123");

// Layer 4 default (same as Layer 2)
const mem = await memoir.memory.get("user-123-personal", "mem_abc123");
...
- `MemoirError('INVALID_AGENT_ID')` - Agent ID is invalid
- `MemoirError('MEMORY_NOT_FOUND')` - Memory doesn't exist
- `MemoirError('PERMISSION_DENIED')` - Agent doesn't own this memory
...
interface MemoryEntry {
  ...
  memoryId: string; // Memoir memory ID (use this for API calls)
  memorySpaceId: string;
...
const result = await memoir.memory.store("user-123-personal", {
...
- `MemoirError('INVALID_AGENT_ID')` - Agent ID is invalid
- `MemoirError('INVALID_CONTENT')` - Content is empty or too large
- `MemoirError('INVALID_IMPORTANCE')` - Importance not in 0-100 range
- `MemoirError('CONVEX_ERROR')` - Database error
...
interface UpdateMemoryOptions {
  // Note: syncToGraph option removed in v0.29.0+ - sync is automatic when MEMOIR_GRAPH_SYNC=true
}
```
...
- `MemoirError('MEMORY_NOT_FOUND')` - Memory doesn't exist
- `MemoirError('PERMISSION_DENIED')` - Agent doesn't own this memory
- `MemoirError('INVALID_UPDATE')` - Update data is invalid
...
await memoir.memory.updateMany(
  "agent-1",
  {
...
- `MemoirError('INVALID_FILTERS')` - Filters are malformed
- `MemoirError('NO_MEMORIES_MATCHED')` - No memories match filters
...
interface DeleteMemoryOptions {
  // Note: syncToGraph option removed in v0.29.0+ - graph cleanup is automatic when MEMOIR_GRAPH_SYNC=true
}
```
...
```typescript
const result = await memoir.memory.delete("user-123-personal", "mem_abc123");
...
// Use memoir.conversations.get(conversationId) to retrieve original
```
...
await memoir.vector.delete("user-123-personal", "mem_abc123");

// Layer 4 (same as Layer 2, but preserves ACID)
await memoir.memory.delete("user-123-personal", "mem_abc123");
// Vector deleted, ACID preserved

// Layer 4 forget() (delete from both - see below)
await memoir.memory.forget("user-123-personal", "mem_abc123", {
...
const result = await memoir.memory.forget("user-123-personal", "mem_abc123", {
  deleteConversation: true,
});
...
```typescript
// Preview deletion
const preview = await memoir.memory.deleteMany(
  "agent-1",
  {
    importance: { $lte: 30 },
    accessCount: { $lte: 1 },
...
if (preview.wouldDelete < 100) {
  const result = await memoir.memory.deleteMany("user-123-personal", {
    importance: { $lte: 30 },
    accessCount: { $lte: 1 },
    createdBefore: new Date(Date.now() - 90 * 24 * 60 * 60 * 1000),
  });
...
- `MemoirError('INVALID_FILTERS')` - Filters are malformed
- `MemoirError('DELETION_CANCELLED')` - User cancelled confirmation
...
```typescript
// Total memories
const total = await memoir.memory.count("user-123-personal");

// Count critical memories
const critical = await memoir.memory.count("user-123-personal", {
  importance: { $gte: 90 },
});

// Count for specific user
const userCount = await memoir.memory.count("user-123-personal", {
  userId: "user-123",
});

// Complex filter count
const oldUnused = await memoir.memory.count("user-123-personal", {
  createdBefore: new Date(Date.now() - 90 * 24 * 60 * 60 * 1000),
  accessCount: { $lte: 1 },
  importance: { $lte: 30 },
});
...
const userMemories = await memoir.memory.list({
  memorySpaceId: "user-123-personal",
  userId: "user-123",
  limit: 100,
});
...
const conversationMemories = await memoir.memory.list({
  memorySpaceId: "user-123-personal",
...
// With enriched facts
const enrichedMemories = await memoir.memory.list({
  memorySpaceId: "user-123-personal",
  enrichFacts: true,
  limit: 25,
});
...
- `MemoirError('INVALID_FILTERS')` - Filters are malformed
- `MemoirError('INVALID_PAGINATION')` - Invalid limit/offset
...
const userData = await memoir.memory.export("user-123-personal", {
  userId: "user-123",
  format: "json",
...
const criticalBackup = await memoir.memory.export("user-123-personal", {
  importance: { $gte: 90 },
  format: "json",
...
- `MemoirError('INVALID_FORMAT')` - Format not supported
- `MemoirError('EXPORT_FAILED')` - File write error
...
const result = await memoir.memory.archive("user-123-personal", "mem_abc123");
...
- `MemoirError('INVALID_FILTERS')` - Filters are malformed
- `MemoirError('ARCHIVE_FAILED')` - Archive operation failed
...
- `MemoirError('MEMORY_NOT_FOUND')` - Memory doesn't exist
- `MemoirError('MEMORY_NOT_ARCHIVED')` - Memory is not in archived state
- `MemoirError('PERMISSION_DENIED')` - Memory space doesn't own this memory
...
```typescript
// Get version 1
const v1 = await memoir.memory.getVersion("user-123-personal", "mem_abc123", 1);
...
- `MemoirError('MEMORY_NOT_FOUND')` - Memory doesn't exist
- `MemoirError('VERSION_NOT_FOUND')` - Version doesn't exist
...
- `MemoirError('MEMORY_NOT_FOUND')` - Memory doesn't exist
- `MemoirError('INVALID_TIMESTAMP')` - Timestamp is invalid
...
await memoir.agents.configure("temp-agent", {
...
Memory entries participate in the Memoir graph through references:
...
const workflowMemories = await memoir.memory.search("user-123-personal", "*", {
  metadata: { contextId: "ctx-001" },
});

// Trace memory to source conversation (via conversationRef edge)
const enriched = await memoir.memory.get("user-123-personal", memoryId, {
...
for (const agent of agents) {
  const userMemories = await memoir.memory.search(agent.id, "*", {
    userId: "user-123",
  });
}
```
...
```typescript
import { VectorValidationError } from "@memoir-ai/sdk/vector";

try {
  await memoir.vector.store(memorySpaceId, input);
} catch (error) {
...
**Questions?** Ask in [GitHub Discussions](https://github.com/Anu-Code07/memoir/discussions).
