...
sidebar_position: 3
description: Complete API reference for authentication context and multi-tenancy integration with Memoir SDK.
---
...
<FlowDiagram title="Auth Context Flow">
  ...
  <FlowNode title="3. Initialize Memoir SDK" icon="layers" variant="warning">
    Pass auth context to Memoir — auto-injects to ALL operations
  </FlowNode>
...
Create an authentication context for use with Memoir SDK.
...
```typescript
import { createAuthContext } from "@getmemoir/sdk";
...
## Integration with Memoir SDK

### Initialization

Pass auth context when creating Memoir instance:
...
const sessions = await memoir.sessions.list({ status: "active" });
...
await memoirA.memory.remember({ memorySpaceId: "shared", ... });
await memoirB.memory.search("shared", "anything");
// Returns: [] (different tenant)
```

### Which Layers Support tenantId?

**ALL Memoir layers automatically support tenant isolation:**

<APITable
  parameters={[
    {
      name: "Layer 1a",
      type: "memoir.conversations.*",
      required: false,
      description: "Auto-filtered by tenantId"
    },
    {
      name: "Layer 1b",
      type: "memoir.immutable.*",
      required: false,
      description: "Auto-filtered by tenantId"
    },
    {
    ...
    {
      name: "Contexts",
      type: "memoir.contexts.*",
      required: false,
      description: "Auto-filtered by tenantId"
    },
    {
      name: "Sessions",
      type: "memoir.sessions.*",
      required: false,
      description: "Auto-filtered by tenantId"
    },
    {
...
const memoir = new Memoir({ convexUrl, auth });
...
await memoir.sessions.touch("current-session");
...
const memoir = new Memoir({ convexUrl, auth: serviceAuth });
...
Create new Memoir instance per request:
...
Reuse Memoir instance for entire app:
...
// Global instance
export const memoir = new Memoir({ convexUrl, auth });

// Use throughout app
await memoir.memory.remember({...});
await memoir.memory.search(...);
```
...
async function switchOrg(userId: string, newOrgId: string) {
  ...
  return new Memoir({ convexUrl, auth });
}

// Usage
const memoirOrgA = await switchOrg("user-123", "org-a");
const memoirOrgB = await switchOrg("user-123", "org-b");
...
await memoir.users.update("user-123", { displayName: "Alice" });

// Note: You CAN update other users, but some operations validate
await memoir.users.update("user-456", { displayName: "Bob" });
...
```typescript
import { AuthContextError } from "@getmemoir/sdk";
...
const memoir1 = new Memoir({ convexUrl, auth: extractAuthContext(req) });
const memoir2 = new Memoir({ convexUrl, auth: extractAuthContext(req) });
```
...
Auth context is immutable once set. To change context, create a new Memoir instance.
...
```typescript
// When you create Memoir with auth
const memoir = new Memoir({
  convexUrl,
  auth: createAuthContext({
    userId: "user-123",
