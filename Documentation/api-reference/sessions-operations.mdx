...
sidebar_position: 8
description: Complete API reference for session lifecycle management and multi-session support in Memoir.
---
...
The Sessions API (`memoir.sessions.*`) provides native session management for user-facing agentic platforms. Sessions track user interactions, manage activity timeouts, and provide an isolated context for conversations.
...
```typescript
memoir.sessions.create(params: CreateSessionParams): Promise<Session>
```
...
```typescript
memoir.sessions.get(sessionId: string): Promise<Session | null>
```
...
```typescript
const session = await memoir.sessions.get("session-xyz");
if (session) {
...
```typescript
memoir.sessions.touch(sessionId: string): Promise<void>
```
...
```typescript
memoir.sessions.end(sessionId: string): Promise<void>
```
...
const tenantResult = await memoir.sessions.endAll("user-123", {
  tenantId: "tenant-abc",
});
...
```typescript
memoir.sessions.list(filters: SessionFilters): Promise<Session[]>
```
...
const tenantSessions = await memoir.sessions.list({
  tenantId: "tenant-abc",
  status: "active",
  limit: 100,
});
...
const spaceSessions = await memoir.sessions.list({
...
```typescript
memoir.sessions.count(filters: SessionFilters): Promise<number>
```
...
```typescript
const activeSessions = await memoir.sessions.count({
  tenantId: "tenant-abc",
  status: "active",
});
...
```typescript
memoir.sessions.getActive(userId: string): Promise<Session[]>
```
...
```typescript
const sessions = await memoir.sessions.getActive("user-123");
...
```typescript
memoir.sessions.expireIdle(options?: ExpireSessionsOptions): Promise<{ expired: number }>
```
...
const customResult = await memoir.sessions.expireIdle({
  tenantId: "tenant-abc",
...
```typescript
import { createAuthContext } from "@memoir-platform/sdk";
...
// Initialize Memoir with auth context
...
async function handleUserConnection(
  ...
  const existingSessions = await memoir.sessions.getActive(userId);
...
async function getActiveSessions(userId: string) {
  const sessions = await memoir.sessions.getActive(userId);
...
async function getSessionStats(tenantId: string) {
  const [active, idle, ended] = await Promise.all([
    memoir.sessions.count({ tenantId, status: "active" }),
    memoir.sessions.count({ tenantId, status: "idle" }),
    memoir.sessions.count({ tenantId, status: "ended" }),
  ]);
...
```typescript
import { SessionValidationError } from "@memoir-platform/sdk";
...
```typescript
try {
  await memoir.sessions.touch("non-existent-session");
} catch (error) {
...
**PLANNED:** Session-specific error codes (`MemoirErrorCode.SESSION_NOT_FOUND`, `MemoirErrorCode.SESSION_EXPIRED`, `MemoirErrorCode.SESSION_ALREADY_ENDED`) are planned for a future release to provide more structured error handling.
