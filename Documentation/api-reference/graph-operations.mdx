...
sidebar_position: 6
description: Complete API reference for graph database integration, multi-hop queries, and knowledge graph operations in Memoir.
---
...
The Graph Operations API (`@getmemoir/sdk/graph`) provides advanced graph database capabilities for Memoir, enabling multi-hop relationship queries, knowledge discovery, and cross-layer context enrichment.
...
**Important:** As of v0.29.0, graph synchronization is **automatic** when `MEMOIR_GRAPH_SYNC=true` is set in your environment variables.

The `syncToGraph` option has been removed from all APIs. Graph sync is now controlled entirely by:
1. Setting `MEMOIR_GRAPH_SYNC=true` in your environment
2. Providing valid graph credentials (e.g., `NEO4J_URI`, `NEO4J_USERNAME`, `NEO4J_PASSWORD`)

#### Automatic Sync Behavior

```typescript
// All operations auto-sync to graph when MEMOIR_GRAPH_SYNC=true
...
await memoir.vector.store(memorySpaceId, data);
// Memory node synced automatically!

// Conversations are automatically synced
await memoir.conversations.create(input);
// Conversation node synced automatically!
```

#### Disabling Graph Sync

To disable graph sync, simply don't set `MEMOIR_GRAPH_SYNC` or set it to `false`:

```bash
# Disable graph sync (default)
MEMOIR_GRAPH_SYNC=false
...
const memory = await memoir.vector.store(memorySpaceId, data);
...
```typescript
const context = await memoir.contexts.get(contextId);
...
```typescript
const fact = await memoir.facts.get(memorySpaceId, factId);
...
```typescript
const memorySpace = await memoir.memorySpaces.get(memorySpaceId);
...
```typescript
import { ensureUserNode } from "@getmemoir/sdk/graph";
...
```typescript
import { ensureAgentNode } from "@getmemoir/sdk/graph";
...
```typescript
import { ensureParticipantNode } from "@getmemoir/sdk/graph";
...
```typescript
import { ensureEntityNode } from "@getmemoir/sdk/graph";
...
```typescript
import { ensureEnrichedEntityNode } from "@getmemoir/sdk/graph";
...
// Worker stops automatically when memoir.close() is called
memoir.close();
```
...
```typescript
import { GraphSyncWorker } from "@getmemoir/sdk/graph";
...
// Graph cleanup is automatic when MEMOIR_GRAPH_SYNC=true (v0.29.0+)
...
```typescript
import { deleteImmutableFromGraph } from "@getmemoir/sdk/graph";
...
```typescript
import { deleteMutableFromGraph } from "@getmemoir/sdk/graph";
...
```typescript
import { syncFactSupersession } from "@getmemoir/sdk/graph";
...
```typescript
import { updateFactGraphStatus } from "@getmemoir/sdk/graph";
...
```typescript
import { syncFactUpdateInPlace } from "@getmemoir/sdk/graph";
...
```typescript
import { syncFactRevision } from "@getmemoir/sdk/graph";
...
```typescript
import { getFactSupersessionChainFromGraph } from "@getmemoir/sdk/graph";
...
```typescript
import { removeFactSupersessionRelationships } from "@getmemoir/sdk/graph";
...
```typescript
import { initializeGraphSchema } from "@getmemoir/sdk/graph";
...
```typescript
import { verifyGraphSchema } from "@getmemoir/sdk/graph";
...
```typescript
import { dropGraphSchema } from "@getmemoir/sdk/graph";
...
Graph synchronization is now automatic when `MEMOIR_GRAPH_SYNC=true` is set.
</Note>

All Memoir APIs automatically sync to the graph database when:
1. `MEMOIR_GRAPH_SYNC=true` is set in environment variables
2. Valid graph credentials are configured

### Conversations API

```typescript
// Automatically syncs when MEMOIR_GRAPH_SYNC=true
await memoir.conversations.create(input);
await memoir.conversations.addMessage(input);
...
await memoir.vector.delete(memorySpaceId, memoryId);
```
...
await memoir.facts.update(memorySpaceId, factId, updates);
await memoir.facts.delete(memorySpaceId, factId);
```
...
await memoir.contexts.update(contextId, updates);
await memoir.contexts.delete(contextId);
```
...
```typescript
// Automatically syncs when MEMOIR_GRAPH_SYNC=true
await memoir.memorySpaces.register(params);
```
...
await memoir.memory.forget(memorySpaceId, memoryId, {
  deleteConversation: true,
});
```
...
```typescript
import { initialGraphSync } from "@getmemoir/sdk/graph";
...
await memoir.vector.store(data);
...
for (const record of related.records) {
  const fullFact = await memoir.facts.get(
    memorySpaceId,
