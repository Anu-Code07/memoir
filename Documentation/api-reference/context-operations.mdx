...
sidebar_position: 5
description: Complete API reference for context chain management and multi-agent workflow coordination in Memoir.
---
...
The Context Operations API (`memoir.contexts.*`) provides methods for managing hierarchical workflows where agents collaborate on complex tasks. Context chains track task delegation, shared state, and workflow evolution across multiple agents.
...
```typescript
// Graph sync happens automatically when MEMOIR_GRAPH_SYNC=true
const root = await memoir.contexts.create({
  purpose: "Process customer refund request",
  memorySpaceId: "supervisor-agent-space",
  userId: "user-123",
...
// Context is automatically synced to graph if MEMOIR_GRAPH_SYNC=true
...
```typescript
const ctx = await memoir.contexts.get("ctx-1234567890-abc123");
...
```typescript
const chain = await memoir.contexts.get("ctx-1234567890-child", {
  includeChain: true,
});
...
```typescript
const enriched = await memoir.contexts.get("ctx-1234567890-abc123", {
  includeChain: true,
...
await memoir.contexts.update("ctx-1234567890-abc123", {
  status: "completed",
  data: {
    result: "success",
...
<Callout type="info">
**v0.29.0+**: Graph deletion with orphan cleanup is now automatic when `MEMOIR_GRAPH_SYNC=true` is set in your environment. The `syncToGraph` option has been removed from all APIs.
</Callout>
...
await memoir.contexts.delete("ctx-1234567890-abc123");

// Delete context and all descendants
const result = await memoir.contexts.delete("ctx-1234567890-root", {
...
const orphanResult = await memoir.contexts.delete("ctx-1234567890-parent", {
  orphanChildren: true,
});
...
const active = await memoir.contexts.search({
  memorySpaceId: "finance-agent-space",
  status: "active",
});

// Find contexts for a user
const userContexts = await memoir.contexts.search({
  userId: "user-123",
});

// Find all root contexts
const roots = await memoir.contexts.search({
  depth: 0,
});
```
...
const all = await memoir.contexts.list();

// List contexts for a memory space
const spaceContexts = await memoir.contexts.list({
  memorySpaceId: "finance-agent-space",
  limit: 50,
});

// List active root workflows
const activeRoots = await memoir.contexts.list({
  status: "active",
...
```typescript
// Total contexts
const total = await memoir.contexts.count();

// Count by status
const active = await memoir.contexts.count({ status: "active" });
const completed = await memoir.contexts.count({ status: "completed" });

// Count for a memory space
const agentContexts = await memoir.contexts.count({
  memorySpaceId: "finance-agent-space",
});

// Count active contexts for a user
const userActive = await memoir.contexts.count({
  userId: "user-123",
  status: "active",
});
```
...
```typescript
const chain = await memoir.contexts.getChain("ctx-1234567890-child");
...
const root = await memoir.contexts.getRoot("ctx-1234567890-nested");
...
```typescript
// Get direct children
const children = await memoir.contexts.getChildren("ctx-1234567890-root");

// Get only active children
const activeChildren = await memoir.contexts.getChildren(
  "ctx-1234567890-root",
  {
    status: "active",
  },
...
const allDescendants = await memoir.contexts.getChildren(
  "ctx-1234567890-root",
  {
    recursive: true,
  },
...
```typescript
memoir.contexts.findOrphaned(): Promise<Context[]>
```
...
for (const ctx of orphaned) {
  await memoir.contexts.delete(ctx.contextId);
}
```
...
```typescript
memoir.contexts.addParticipant(
  contextId: string,
...
```typescript
memoir.contexts.getByConversation(
  conversationId: string
): Promise<Context[]>
```
...
const v1 = await memoir.contexts.getVersion("ctx-1234567890-abc123", 1);
...
```typescript
const history = await memoir.contexts.getHistory("ctx-1234567890-abc123");
...
```typescript
memoir.contexts.getAtTimestamp(
  contextId: string,
  timestamp: Date
): Promise<ContextVersion | null>
```
...
const msg = await memoir.conversations.addMessage("conv-789", {
  role: "user",
...
const chain = await memoir.contexts.get("ctx-1234567890-finance", {
  includeChain: true,
  includeConversation: true,
});
...
async function checkWorkflowComplete(rootContextId: string) {
  const chain = await memoir.contexts.getChain(rootContextId);
  ...
  if (allComplete && chain.current.status !== "completed") {
    ...
    await memoir.contexts.update(rootContextId, {
      status: "completed",
...
const results = await memoir.memory.recall({
  memorySpaceId: "finance-agent-space",
  userId: "user-123",
...
const workflow = await memoir.contexts.create({
...
await memoir.contexts.addParticipant(workflow.contextId, "finance-agent-space");

// All agents can see who's involved
const ctx = await memoir.contexts.get(workflow.contextId);
...
const active = await memoir.contexts.search({ status: "active" });
...
```typescript
// When work starts
await memoir.contexts.update(contextId, {
  status: "active",
...
const chain = await memoir.contexts.get(contextId, {
...
const roots = await memoir.contexts.search({ depth: 0 });

// Find all child nodes of a parent
const children = await memoir.contexts.search({ parentId: "ctx-001" });

// Find entire workflow tree (all nodes with same rootId)
const workflowTree = await memoir.contexts.search({ rootId: "ctx-root" });

// Find workflows triggered by conversation (traverse conversationRef edge)
const workflowsFromConvo = await memoir.contexts.search({
...
```typescript
// Create context (graph sync is automatic when MEMOIR_GRAPH_SYNC=true)
await memoir.contexts.create(params);
