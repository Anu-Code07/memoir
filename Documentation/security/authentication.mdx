...
sidebar_position: 1
description: Understanding authentication context in Memoir for user tracking, multi-tenancy, and session management
---

# Authentication

Understanding authentication context in Memoir for user tracking, multi-tenancy, and session management.

## Overview

<Callout type="info" title="Key Concept">
  Auth context is **just data** — not a protocol or SDK. Extract it from your auth system (whatever that is) and pass it to Memoir.
</Callout>

<FlowDiagram 
  title="Authentication Flow" 
  caption="Your auth system → Memoir SDK → Auto-injected to all operations"
>
  <FlowNode 
...
app.post("/chat", (req, res) => {
  const memoir = new Memoir({ convexUrl, auth: req.auth });
  // Use memoir...
});
```
...
async function validateAuth(auth: AuthContext) {
  const user = await memoir.users.get(auth.userId);
