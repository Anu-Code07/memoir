...
title: Core Concepts
sidebar_position: 4
description: Understand memory spaces, layers, and the Memoir architecture
---

# Core Concepts

Understanding these core concepts will help you make the most of Memoir. This guide covers the fundamental building blocks of the system.

<Callout type="info" title="Memoir Cloud Coming Soon">
  Memoir SDK is available now for self-hosted deployments. **Memoir Cloud**—a managed service with enhanced analytics, automatic scaling, and zero-config setup—is coming soon.
</Callout>
...
A **memory space** is the fundamental isolation boundary in Memoir. Think of it as a private namespace where memories, facts, and conversations are stored.
...
<Terminal>memoir spaces create user-123-personal --type personal --name "Alice's Personal Space"</Terminal>

<Terminal>memoir spaces stats user-123-personal</Terminal>

<Terminal>memoir spaces agents user-123-personal</Terminal>
...
Memoir supports two architectural patterns for multi-agent/multi-tool systems:
...
const memories = await memoir.memory.search("user-123-personal", "preferences");
...
const fullContext = await memoir.contexts.get(context.id, {
  includeChain: true,
...
In Memoir, a **memory** is a piece of information stored in a memory space for later retrieval.
...
```typescript
await memoir.vector.store("support-bot-space", {
...
await memoir.memory.update("user-123-personal", memoryId, {
  content: "User's address is 456 Oak Ave, Seattle",
});

// Both versions are preserved!
const memory = await memoir.memory.get("user-123-personal", memoryId);
...
Memoir uses a granular 0-100 importance scale for precise prioritization:
...
<Callout type="info" title="Bring Your Own Embeddings">
  The Memoir SDK does not generate embeddings—you bring your own provider (OpenAI, Cohere, local models, etc.).
</Callout>
...
// Memoir SDK stores and searches
...
<Terminal>memoir users list</Terminal>

<Terminal>memoir users get user-123</Terminal>
...
async function respondToUser(userMessage: string, memorySpaceId: string) {
  ...
  const relevantContext = await memoir.memory.search(
    memorySpaceId,
    userMessage,
    {
...
const fullContext = await memoir.contexts.get(financeContext.id, {
  includeChain: true,
...
<Callout type="note" title="Enhanced Analytics Coming Soon">
  Advanced analytics dashboards with `memoir.analytics.*` APIs are planned for Memoir Cloud. The SDK provides basic memory space statistics today via `memoir.memorySpaces.getStats()`.
</Callout>
...
```typescript
const memory = await memoir.memory.get("user-123-personal", "mem_123");
...
<Callout type="info" title="Planned Feature">
  Advanced analytics methods like `memoir.analytics.findUnusedMemories()` and `memoir.analytics.findHotMemories()` are planned for Memoir Cloud. See [Access Analytics roadmap](/roadmap/access-analytics) for details.
</Callout>

```typescript
// For now, you can query memories manually:
const memories = await memoir.memory.search("user-123-personal", "", {
  limit: 1000,
});
...
<Terminal>memoir memory stats --space user-123-personal</Terminal>
...
<FlowDiagram
  ...
  caption="How Memoir orchestrates memory from input to retrieval"
>
  <FlowNode
    ...
    variant="default"
  >
    Call memoir.memory.remember() or memoir.memory.recall()
  </FlowNode>
  <FlowNode
    title="Memoir SDK"
    icon="brain"
...
<Callout type="info" title="Memoir Does the Heavy Lifting">
  You call `remember()` or `recall()`—Memoir handles orchestration, fact extraction, vector indexing, and multi-layer retrieval automatically.
</Callout>
...
Memoir supports optional graph database integration for advanced knowledge discovery, multi-hop queries, and relationship-based reasoning.
...
// 3. Initialize Memoir with graph
...
// 4. Use Memoir normally - graph syncs automatically!
```
...
<Callout type="info" title="Convex as Source of Truth">
  Graph is for **discovery and traversal**. Always write to Convex first, then sync to graph. Memoir works perfectly without graph—add it when you need multi-hop reasoning or knowledge graphs.
</Callout>
