...
sidebar_position: 6
description: Integrate Memoir with your existing authentication system
---

# Auth Integration (2 Minutes)

Quick guide: Integrate Memoir with your existing authentication system.

## The Simple Truth

**Memoir works with whatever auth system you already use.** No special integration, no proprietary protocol, no heavy lifting.

**Your auth system** → Extract `userId` → **Pass to Memoir** → Done.
...
<Callout type="info">
  Memoir is **framework-agnostic** — it works with any auth system that provides a user ID.
</Callout>
...
<Tabs>
  <TabItem value="clerk" label="Clerk">
    <Steps>
      ...
      <Step title="Configure Memoir with Clerk">
        ```typescript
        import { Memoir, createAuthContext } from '@getmemoir/sdk';
...
**The pattern is always the same** — just map your auth data to Memoir's simple structure.
...
await memoir.memory.remember({...}); // tenantId auto-injected
```
...
<Steps>
  <Step title="Auto-Injection">
    ...
    ```typescript
    // You write:
    await memoir.memory.remember({ memorySpaceId: "space-1", ... });
  ...
  <Step title="Auto-Filtering">
    ...
    ```typescript
    // You write:
    const memories = await memoir.memory.search("space-1", "query");
...
<Callout type="warning" title="Server-Side Only">
  Always create the Memoir client on the **server side**. Never expose your Convex URL or auth context to the client.
</Callout>
...
<Tabs>
  ...
  <TabItem value="express" label="Express.js">
    ```typescript
    // middleware/memoir.ts
    import { Memoir, createAuthContext } from '@getmemoir/sdk';
    
    export function memoirMiddleware(req, res, next) {
      req.memoir = new Memoir({
        convexUrl: process.env.CONVEX_URL!,
        auth: createAuthContext({
          userId: req.user.id,
    ...
    app.post("/chat", async (req, res) => {
      await req.memoir.memory.remember({...});
    });
    ```
  </TabItem>
...
Memoir works without auth context for development and testing:
...
<Accordion>
  <AccordionItem title="Do I need to install provider-specific packages?" defaultOpen={true}>
    **No!** Memoir doesn't require any auth provider SDKs. Use whatever you already have.
  </AccordionItem>
  <AccordionItem title="What if I change auth providers?">
    Just update your extraction code. Memoir doesn't care where `userId` comes from.
  </AccordionItem>
  ...
  <AccordionItem title="What about RBAC/permissions?">
    Store in `metadata` field. Memoir provides data isolation, you handle authorization logic.
  </AccordionItem>
...
<QuickNav>
  <QuickNavItem 
    ...
    description="Get Memoir running in minutes"
