---
id: introduction
title: Introduction to Memoir
sidebar_position: 1
description: Learn what Memoir is and how it provides persistent memory for AI agents
---

## Welcome to Memoir

Memoir is a plug'n'play persistent memory system for AI agents, powered by Convex. It brings enterprise-grade memory capabilities to any AI application, allowing your agents to remember, learn, and build context over time.
...
Memoir solves all these problems with a single, unified system. Get started in 2 commands:
...
import { Memoir, createAuthContext } from "@getmemoir/sdk";
...
<Callout type="tip" title="Auth Integration">
  Memoir is **framework-agnostic** - it works with whatever auth system you already use. Just extract `userId` and optionally `tenantId`, no complex integration needed. See [Authentication](/security/authentication).
</Callout>
...
Memoir leverages [Convex](https://convex.dev), a reactive backend platform that provides:
...
**Flexible Deployment**: Memoir works with Convex however you run it:
...
Unlike vector databases that lock you into their ecosystem, Memoir is **embedding-agnostic**:
...
```typescript
// Memoir doesn't care - just store it (Layer 2 for system memories)
...
Memoir is licensed under FSL-1.1-Apache-2.0 (Functional Source License):

- Free for internal use and applications that use Memoir
...
Memoir is built on these core principles:

- **Developer Experience First** - Intuitive APIs, full TypeScript support, clear docs, helpful errors
- **Progressive Enhancement** - Start simple, add complexity when needed
- **Embedding-Agnostic** - You choose your embedding model; Memoir just stores and retrieves
...
<FlowDiagram 
  ...
  caption="Your app connects directly to your Convex instance via the Memoir SDK"
>
  <FlowNode 
    ...
    variant="primary"
  >
    Your code integrates with the Memoir SDK
  </FlowNode>
  <FlowNode 
    title="Memoir SDK (Open Source)" 
    icon="package" 
    variant="warning"
  >
...
<FlowDiagram 
  ...
  caption="Memoir Cloud adds management tools while your data stays in your Convex instance"
>
  <FlowNode 
    title="Your Application" 
    icon="app" 
    variant="primary"
  >
    Your code uses the same Memoir SDK API
  </FlowNode>
  <FlowNode 
    title="Memoir SDK (Same API!)" 
    icon="package" 
    variant="warning"
  >
...
<Callout type="note">
  In both modes, your data lives in your Convex instance. Memoir Cloud adds management tools without moving your data.
</Callout>

## Use Cases

Memoir powers a wide range of AI applications:
...
<Callout type="info" title="What Memoir Is Not">
  Memoir is **not** an LLM, vector database, or Convex replacement. It's a complete memory system built on top of Convex that works with any AI framework. Your data stays in your Convex instance.
</Callout>
