...
Memoir's graph integration extends your memory system with relationship-aware capabilities. Connect Neo4j or Memgraph to enable multi-hop queries, knowledge graphs, and cross-layer context enrichment.
...
// All Memoir operations now sync automatically!
await memoir.memory.remember(params); // → Graph updated
await memoir.facts.store(params); // → Graph updated
await memoir.contexts.create(params); // → Graph updated
```
...
Graph synchronization is now automatic when `MEMOIR_GRAPH_SYNC=true` is set in your environment. The `syncToGraph` option has been deprecated and removed.

```typescript
// Graph sync happens automatically for all operations when MEMOIR_GRAPH_SYNC=true
await memoir.vector.store(data);  // → Auto-syncs to graph
await memoir.facts.store(params); // → Auto-syncs to graph (including entities)
...
<Callout type="info">
**v0.29.0 Breaking Change**: The `syncToGraph` option has been removed from all API methods. Graph sync is now controlled exclusively via the `MEMOIR_GRAPH_SYNC` environment variable.
</Callout>
...
```typescript
// Delete memory - orphans are cleaned up automatically (when MEMOIR_GRAPH_SYNC=true)
...
// Convex write succeeds → then auto-syncs to graph when MEMOIR_GRAPH_SYNC=true
...
const fullConversation = await memoir.conversations.get(conversationId);
```
...
try {
  ...
  if (error instanceof GraphAuthenticationError) {
    ...
    // Memoir works fine without graph!
  }
}
```
