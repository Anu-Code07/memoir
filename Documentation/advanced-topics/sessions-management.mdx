...
Memoir provides built-in session management through the `memoir.sessions.*` API. Sessions track user interactions, manage activity timeouts, and provide isolated context for conversations.
...
await memoir.sessions.endAll("user-123", {
  tenantId: "tenant-abc",
});
```
