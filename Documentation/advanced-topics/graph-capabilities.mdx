...
Navigate relationships and traverse knowledge graphs using Memoir's document-oriented architecture.
...
Memoir is built on Convex, a **document-oriented database**, but provides **graph-like querying** through references and relationships. We call this **Graph-Lite** - you get graph capabilities without running a separate graph database.
...
Every entity in Memoir is a graph node:
...
<FlowDiagram title="Memoir Implicit Graph Structure">
...
const chain = await memoir.contexts.get("ctx-child", {
...
const conversation = await memoir.a2a.getConversation(
  "finance-agent",
  "hr-agent",
  {
...
const directConnections = await memoir.vector.search("finance-agent", "*", {
  sourceType: "a2a",
  limit: 100,
});
...
for (const space of connectedSpaces) {
  const theirConnections = await memoir.vector.search(space, "*", {
    sourceType: "a2a",
    limit: 100,
  });
...
async function getUserDataGraph(userId: string) {
  const graph = {
    user: await memoir.users.get(userId),
    conversations: await memoir.conversations.list({ userId }),
    contexts: await memoir.contexts.list({ userId }),
    memories: [] as any[],
    facts: [] as any[]
  };
  ...
  for (const space of spaces.spaces) {
    const spaceMemories = await memoir.vector.search(space.memorySpaceId, '*', {
      userId,
      limit: 100
    });
...
async function traceAuditTrail(conversationId: string) {
  ...
  for (const context of contexts) {
    ...
    const children = await memoir.contexts.getChildren(context.id, {
      recursive: true,
    });
    ...
    const contextMemories = await memoir.memory.search("*", "*", {
...
async function findRelatedFacts(factId: string) {
  // Get the source fact
  const fact = await memoir.immutable.get("fact", factId);
  ...
  for (const entity of fact.data.entities) {
    const facts = await memoir.immutable.search(entity, {
      type: "fact",
      limit: 10,
    });
...
async function buildMemorySpaceNetwork() {
  ...
  for (const space of spaces.spaces) {
    const collaborations = await memoir.vector.search(space.memorySpaceId, '*', {
      sourceType: 'a2a',
      limit: 1000
    });
...
const convos = await memoir.conversations.list({ userId }); // 20ms
...
const memorySpace = await memoir.memorySpaces.get(contexts[0].memorySpaceId); // 15ms
...
const graphResult = await memoir.graph.traverse(...);
const convexResult = await memoirTraverse(...);
...
async function getHistoricalGraph(timestamp: Date) {
  ...
  const a2aMessages = await memoir.memory.search("*", "*", {
    source: { type: "a2a" },
    createdBefore: timestamp,
...
async function getWeightedCollaborationGraph() {
  ...
  for (const space of spaces.spaces) {
    const a2a = await memoir.vector.search(space.memorySpaceId, "*", {
      sourceType: "a2a",
      limit: 1000,
    });
...
const chain = await memoir.contexts.get(contextId, { includeChain: true });

// Avoid: Manual traversal when built-in exists
const context = await memoir.contexts.get(contextId);
const parent = context.parentId
  ? await memoir.contexts.get(context.parentId)
  : null;
const grandparent = parent?.parentId
  ? await memoir.contexts.get(parent.parentId)
  : null;
...
const root = await memoir.contexts.get(rootId);
for (const childId of root.childIds) {
  const child = await memoir.contexts.get(childId); // N queries!
}
```
...
const chain = await memoir.contexts.get(complexContextId, {
  includeChain: true,
});
...
const agents = await memoir.graph.traverse({
