...
const fullContext = await memoir.contexts.get(childContext.id, {
  includeChain: true,
});
...
const customerCtx = await memoir.contexts.create({
...
```typescript
const fullContext = await memoir.contexts.get(financeCtx.id, {
  includeChain: true,
});
...
```typescript
// Get single context
const ctx = await memoir.contexts.get(contextId);

// Get with full chain
const fullCtx = await memoir.contexts.get(contextId, {
  includeChain: true,
});
...
const urgent = await memoir.contexts.search({
  data: { importance: { $gte: 80 } },
  status: "active",
});

// Find user's contexts
const userCtx = await memoir.contexts.search({
  userId: "user-123",
  sortBy: "createdAt",
  sortOrder: "desc",
});
...
const chain = await memoir.contexts.get(financeApproval.id, {
  includeChain: true,
});
```
...
```typescript
// Vague - hard to trace
await memoir.contexts.create({ purpose: "Process request" });
...
await memoir.contexts.update(id, { status: "active" });
// ... do work ...
await memoir.contexts.update(id, { 
  status: "completed",
  completedAt: Date.now(),
});
```
