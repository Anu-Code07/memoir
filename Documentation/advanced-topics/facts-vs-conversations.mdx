...
Memoir supports three storage strategies:
...
async function intelligentRetrieval(
  ...
  const vectorSummaries = await memoir.memory.search(memorySpaceId, query, {
    embedding: await embed(query),
    userId,
...
async function buildAdaptiveContext(
  ...
  context.facts = await memoir.memory.search(memorySpaceId, query, {
    embedding: await embed(query),
    userId,
    contentType: "fact",
    limit: 10,
  ...
  if (complexity === "moderate") {
    context.summaries = await memoir.memory.search(memorySpaceId, query, {
      embedding: await embed(query),
      userId,
  ...
  if (complexity === "complex") {
    ...
    for (const fact of topFacts) {
      if (fact.conversationRef && fact.metadata.importance >= 80) {
        const convo = await memoir.conversations.get(
          fact.conversationRef.conversationId,
        );
...
const relevantFacts = await memoir.memory.search("support-agent", query, {
  userId: customerId,
  contentType: "fact",
  limit: 5,
});
...
const context = await memoir.memory.search("personal-assistant", query, {
  userId,
  contentType: "fact",
...
const prefs = await memoir.memory.search('code-assistant', 'coding style', {
  userId: devId,
  contentType: 'fact',
...
async function migrateToFacts(memorySpaceId: string) {
  ...
  for (const conversation of conversations) {
    try {
      ...
      const messages = await memoir.conversations.getHistory(
...
const rawMemories = await memoir.memory.list("agent-1", {
  contentType: "raw",
...
for (const memory of rawMemories.memories) {
  ...
  const facts = await memoir.memory.search("agent-1", "*", {
  ...
  if (facts.length > 0) {
    ...
    await memoir.memory.delete("agent-1", memory.id);
  }
}
...
async function measureRetrievalPerformance() {
  ...
  for (const query of queries) {
    ...
    await memoir.memory.search("agent-1", query, {
      contentType: "raw",
      userId,
    });
...
- Memoir-unique compliance advantage (ACID audit trails)
...
**Remember:** Memoir's ACID layer means you always have raw conversations as backup, even if you only index facts in vector layer. This is a unique advantage - other memory systems typically discard raw data to save storage.
