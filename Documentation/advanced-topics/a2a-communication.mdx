...
const inbox = await memoir.memory.search("hr-agent", "*", {
...
const sent = await memoir.memory.search("finance-agent", "*", {
...
const ctx = await memoir.contexts.get(context.id, { includeChain: true });
```
...
async function handleRequests(agentId: string) {
  ...
  const requests = await memoir.memory.search(agentId, "*", {
...
const allA2A = await memoir.memory.search("agent-1", "*", {
  source: { type: "a2a" },
});

// Messages from specific agent
const fromFinance = await memoir.memory.search("hr-agent", "*", {
  source: { type: "a2a" },
  metadata: { fromAgent: "finance-agent" },
});

// High-importance A2A
const urgent = await memoir.memory.search("agent-1", "*", {
  source: { type: "a2a" },
  minImportance: 85,
});

// Semantic search across A2A
const budgetMsgs = await memoir.memory.search("agent-1", "budget approval", {
