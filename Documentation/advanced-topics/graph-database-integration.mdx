...
Comprehensive guide for integrating a graph database with Memoir for advanced relationship queries.

## Overview

This guide covers everything you need to add graph database capabilities to Memoir: from deciding if you need it, to choosing the right database, to setup and integration.
...
- Memoir SDK installed and working
...
- **Enterprise:** Memoir Graph-Premium (fully managed, zero DevOps)
...
- For Memoir integration: Likely fine (user runs Neo4j)
...
# Verify it's running
docker ps
# Should see: memoir-neo4j (ports 7474, 7687)
...
# Verify it's running
docker ps
# Should see: memoir-memgraph (ports 7688, 3001)
...
- Neo4j: `neo4j` / `memoir-dev-password`
- Memgraph: `memgraph` / `memoir-dev-password`
...
## Section 5: Syncing Memoir to Graph

### Strategy 1: Automatic Sync (Recommended)

Use Memoir's built-in graph sync:
...
// Use Memoir normally - graph syncs automatically!
...
// Sync a context
const context = await memoir.contexts.get("ctx-001");
const nodeId = await syncContextToGraph(context, graphAdapter);

// Sync a memory
const memory = await memoir.vector.get("agent-1", "mem-123");
await syncMemoryToGraph(memory, graphAdapter);

// Sync a fact
const fact = await memoir.facts.get("agent-1", "fact-456");
...
for (const record of related.records) {
  const conversation = await memoir.conversations.get(record.conversationId);
...
async function verifySync(contextId: string) {
  const convexContext = await memoir.contexts.get(contextId);
