...
```typescript
import { createMemoirMemoryAsync } from '@memoir-platform/vercel-ai-provider';
...
await memoir.facts.update('user-space', factId, {
...
const history = await memoir.facts.getHistory('user-space', factId);
...
const chain = await memoir.facts.getSupersessionChain(factId);
