...
**This is NOT a Drop-In Auth Integration.** Memoir does NOT include pre-built authentication provider integrations. There is no `npm install @getmemoir/auth-clerk` or automatic Auth0 setup. This guide provides **generic patterns** for integrating Memoir with your existing authentication system.
</Callout>
...
Memoir provides the **AuthContext API** - a simple, framework-agnostic interface:
...
Memoir accepts a simple AuthContext object. You extract user info from your auth system:

```typescript
// YOUR auth system (any provider)
const session = await yourAuthSystem.getCurrentUser();

// Create Memoir auth context
...
const extracted = {
  ...
  // ... map your auth data to Memoir fields
};
```
...
<Steps>
  ...
  <Step>
    **Initialize Memoir with auth context** → `new Memoir({ auth, ... })`
  </Step>
  <Step>
...
Memoir supports full SaaS multi-tenant isolation via `tenantId`:
...
const adminMemoir = new Memoir({ convexUrl, auth: adminAuth });

// Query across all tenants
const allUsers = await adminMemoir.users.list({
...
Create new Memoir instance per request for tenant isolation:
...
app.post("/api/chat", async (req, res) => {
  ...
  // New Memoir instance per request
  const memoir = new Memoir({
    convexUrl: process.env.CONVEX_URL!,
...
Reuse Memoir instance for single-user applications:
...
await memoir.users.getOrCreate("user-123", {
...
const session = await memoir.sessions.getOrCreate(userId, metadata);
```
...
await memoir.users.delete("user-to-delete", {
...
const memoir = new Memoir({ convexUrl, auth: adminAuth });
...
async function switchOrganization(userId: string, newOrgId: string) {
  ...
  // New Memoir instance for new tenant context
...
it("should store memory with auth context", async () => {
  ...
  const memoir = new Memoir({ convexUrl: testUrl, auth });
