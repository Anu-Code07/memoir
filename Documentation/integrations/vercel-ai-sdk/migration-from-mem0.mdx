# Migration from mem0 to Memoir

<Callout type="info"> **Last Updated**: 2026-01-08 </Callout>

Guide for migrating from mem0's Vercel AI SDK integration to Memoir.
...
const result = await streamText({
  model: memoirMemory(openai("gpt-5-nano")),
  messages,
});
```
...
### 2. Install Memoir Provider

<Terminal>
npm uninstall @mem0/vercel-ai-provider
npm install @getmemoir/vercel-ai-provider
</Terminal>
...
<DataTable
  headers={["mem0", "Memoir"]}
  rows={[
    ["`mem0('model')`", "`memoirMemory(provider('model'))`"],
    ["`mem0.retrieveMemories()`", "`memoirMemory.search()`"],
    ["`mem0.addMemories()`", "`memoirMemory.remember()`"],
    ["`mem0.getMemories()`", "`memoirMemory.getMemories()`"],
  ]}
/>
...
<DataTable
  headers={["mem0 Feature", "Memoir Equivalent", "Notes"]}
  rows={[
...
**Important**: `agentId` is a separate required field from `memorySpaceId`. While `agent_id` from mem0 maps to `memorySpaceId` (for memory isolation), Memoir also requires `agentId` to identify the agent participant in user-agent conversations.

## Required Configuration Fields

When migrating, ensure your `createMemoirMemory()` config includes:
