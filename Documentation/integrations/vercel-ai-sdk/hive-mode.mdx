...
```typescript
// Next.js App
const webAppMemory = createMemoirMemory({
  convexUrl: process.env.CONVEX_URL!,
...
// Cursor (if using SDK)
const cursorMemory = createMemoirMemory({
  convexUrl: process.env.CONVEX_URL!,
...
const claudeMemory = createMemoirMemory({
  convexUrl: process.env.CONVEX_URL!,
  memorySpaceId: "shared-workspace", // Same space
  userId: currentUser.id,
...
```typescript
// Agent A
const agentA = createMemoirMemory({
...
// Agent B
const agentB = createMemoirMemory({
  memorySpaceId: "team-workspace",
  userId: "user-123",
...
const calendarMemories = await memoirMemory.search('meetings', {
  participantId: 'calendar-tool'
});

// Filter facts by participant
const emailFacts = await memoir.facts.list({
