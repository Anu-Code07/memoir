# Getting Started with Memoir for Vercel AI SDK

<Callout type="info"> **Last Updated**: 2026-01-08 </Callout>

Complete guide to adding persistent memory to your Next.js application using Memoir SDK v0.21.0+.
...
The **fastest way** to understand Memoir is with our interactive quickstart demo. This is a complete Next.js app with:
...
2. **Watch the Layer Flow Diagram** show data flowing through all 7 Memoir layers in real-time
...
[View Quickstart Documentation](https://github.com/Anu-Code07/memoir/tree/main/packages/vercel-ai-provider/quickstart)
...
Follow the wizard to set up Convex with Memoir backend automatically.
...
export async function POST(req: Request) {
  ...
  // Use createMemoirMemoryAsync for graph memory support
...
<Callout type="info">
Use `createMemoirMemoryAsync` (with `await`) instead of `createMemoirMemory` when you need graph memory support or want automatic configuration from environment variables.
</Callout>
...
- `MEMOIR_FACT_EXTRACTION` (optional)
- `MEMOIR_GRAPH_SYNC` (optional)
