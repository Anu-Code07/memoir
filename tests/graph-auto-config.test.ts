...
describe("Graph Auto-Configuration", () => {
  ...
  describe("Two-Gate Approach - Gate 2 (MEMOIR_GRAPH_SYNC)", () => {
    ...
    it("does NOT auto-configure when MEMOIR_GRAPH_SYNC is 'false'", async () => {
      process.env.MEMOIR_GRAPH_SYNC = "false";
      process.env.NEO4J_URI = "bolt://localhost:7687";
    ...
    it("does NOT auto-configure when MEMOIR_GRAPH_SYNC is empty string", async () => {
      process.env.MEMOIR_GRAPH_SYNC = "";
      process.env.NEO4J_URI = "bolt://localhost:7687";
  ...
  describe("Two-Gate Approach - Gate 1 (Connection Credentials)", () => {
    it("warns when only MEMOIR_GRAPH_SYNC is set (no URI)", async () => {
      ...
      expect(warnSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          "MEMOIR_GRAPH_SYNC=true but no graph database URI found",
        ),
      );
  ...
  describe("Memoir.create() Factory", () => {
    it("creates Memoir instance without graph when auto-config fails", async () => {
      process.env.MEMOIR_GRAPH_SYNC = "true";
      process.env.NEO4J_URI = "bolt://localhost:9999"; // Non-existent port
      ...
      expect(memoir.getGraphSyncWorker()).toBeUndefined();
    ...
    it("creates Memoir without graph when MEMOIR_GRAPH_SYNC is not set", async () => {
