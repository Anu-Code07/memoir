/**
 * Memoir SDK - Context Operations Tests
 *
...
describe("Context Operations API", () => {
  ...
  describe("Versioning Operations", () => {
    it("tracks versions on updates (getVersion, getHistory, getAtTimestamp)", async () => {
      ...
      const v1Initial = await memoir.contexts.getVersion(context.contextId, 1);
      ...
      await memoir.contexts.update(context.contextId, {
        status: "active",
        data: { value: "v2" },
      });
      ...
      // Test getVersion
      const v1 = await memoir.contexts.getVersion(context.contextId, 1);
      const v2 = await memoir.contexts.getVersion(context.contextId, 2);
      const v3 = await memoir.contexts.getVersion(context.contextId, 3);
      ...
      // Test getHistory
      const history = await memoir.contexts.getHistory(context.contextId);
      ...
      // Test getAtTimestamp
      const atV2 = await memoir.contexts.getAtTimestamp(
        context.contextId,
      ...
      const current = await memoir.contexts.getAtTimestamp(
        context.contextId,
        new Date(),
      );
    ...
    it("returns null for non-existent version number", async () => {
      ...
      const version = await memoir.contexts.getVersion(context.contextId, 999);
    ...
    it("returns empty array for getHistory on non-existent context", async () => {
      const history = await memoir.contexts.getHistory(
    ...
    it("returns null for getVersion on non-existent context", async () => {
      const version = await memoir.contexts.getVersion(
        "ctx-9999999999-nonexistent",
        1,
      );
    ...
    it("returns null for getAtTimestamp before context creation", async () => {
      ...
      const result = await memoir.contexts.getAtTimestamp(
        context.contextId,
        beforeCreate,
      );
    ...
    it("returns correct version for getAtTimestamp at exact update time", async () => {
      ...
      const updated = (await memoir.contexts.get(context.contextId)) as any;
  ...
  describe("Bulk Operations", () => {
    it("updates many contexts with filters", async () => {
      ...
      const result = await memoir.contexts.updateMany(
        {
          userId: "user-1",
          status: "active",
        },
      ...
      for (const ctx of contexts) {
        ...
        const currentVersion = await memoir.contexts.getVersion(
          ctx.contextId,
          2,
        );
    ...
    it("deletes many contexts with cascade", async () => {
      ...
      const result = await memoir.contexts.deleteMany(
        {
          memorySpaceId: TEST_MEMORY_SPACE,
          status: "completed",
        },
      ...
      // Verify all deleted
      const remaining = await memoir.contexts.list({
        memorySpaceId: TEST_MEMORY_SPACE,
      });
  ...
  describe("Participant Management", () => {
    it("removes participant from context", async () => {
      ...
      // Add participants
      await memoir.contexts.addParticipant(context.contextId, "agent-1");
      await memoir.contexts.addParticipant(context.contextId, "agent-2");

      let updated = (await memoir.contexts.get(context.contextId)) as any;
      expect(updated?.participants).toContain("agent-1");
      expect(updated?.participants).toContain("agent-2");

      // Remove one participant
      await memoir.contexts.removeParticipant(context.contextId, "agent-1");

      updated = (await memoir.contexts.get(context.contextId)) as any;
    ...
    it("throws error when removing participant from non-existent context", async () => {
      ...
      await expect(
        memoir.contexts.removeParticipant(
          "ctx-9999999999-nonexistent",
          "agent-1",
        ),
  ...
  describe("Query Operations", () => {
    it("finds contexts by conversation ID", async () => {
      ...
      const results = await memoir.contexts.getByConversation(conversationId);
    ...
    it("finds orphaned contexts", async () => {
      ...
      const childExists = await memoir.contexts.get(child.contextId);
  ...
  describe("Export Operations", () => {
    it("exports contexts to JSON", async () => {
      ...
      const result = await memoir.contexts.export(
        { userId: "export-user" },
        { format: "json" },
      );
