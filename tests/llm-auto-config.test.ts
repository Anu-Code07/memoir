...
import { Memoir, LLMConfig } from "../src";
...
describe("LLM Auto-Configuration", () => {
  ...
  describe("Two-Gate Approach", () => {
    it("does NOT auto-configure when only OPENAI_API_KEY is set (no opt-in)", () => {
      ...
      const result = (Memoir as any).autoConfigureLLM();

      expect(result).toBeUndefined();
    });

    it("does NOT auto-configure when only MEMOIR_FACT_EXTRACTION is set (no API key)", () => {
      ...
      expect(warnSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          "MEMOIR_FACT_EXTRACTION=true but no API key found",
        ),
      );
    ...
    it("does NOT auto-configure when MEMOIR_FACT_EXTRACTION is 'false'", () => {
    ...
    it("does NOT auto-configure when MEMOIR_FACT_EXTRACTION is empty string", () => {
      process.env.MEMOIR_FACT_EXTRACTION = "";
      process.env.OPENAI_API_KEY = "sk-test-key";
  ...
  describe("Explicit Config Override", () => {
    it("explicit MemoirConfig.llm takes priority over env vars", () => {
      ...
      // The actual priority logic is: config.llm ?? Memoir.autoConfigureLLM()
