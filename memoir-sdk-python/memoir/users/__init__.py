...
from ..errors import CascadeDeletionError, MemoirError, ErrorCode
from ..types import (
    AuthContext,
    DeleteUserOptions,
    ExportUsersOptions,
...
class UsersAPI:
    ...
    async def get(self, user_id: str) -> Optional[UserProfile]:
        ...
        Example:
            >>> user = await memoir.users.get('user-123')
            >>> if user:
    ...
    async def update(self, user_id: str, data: Dict[str, Any]) -> UserProfile:
        ...
        Example:
            >>> updated = await memoir.users.update(
            ...     'user-123',
            ...     {
    ...
    async def search(self, filters: Optional[ListUsersFilter] = None) -> List[UserProfile]:
        ...
        Example:
            ...
            >>> recent_users = await memoir.users.search(ListUsersFilter(
            ...     created_after=int((time.time() - 30 * 24 * 60 * 60) * 1000),
            ...     limit=100
            ... ))
            >>>
            >>> # Search by displayName (client-side filter)
            >>> alex_users = await memoir.users.search(ListUsersFilter(
            ...     display_name='alex',
            ...     limit=50
            ... ))
            >>>
            >>> # Search with sorting
            >>> sorted_users = await memoir.users.search(ListUsersFilter(
            ...     sort_by='updatedAt',
            ...     sort_order='desc'
            ... ))
        """
    ...
    async def list(self, filters: Optional[ListUsersFilter] = None) -> ListUsersResult:
        ...
        Example:
            ...
            >>> result = await memoir.users.list(ListUsersFilter(limit=50))
            >>> print(f"Found {result.total} users, showing {len(result.users)}")
            >>>
            >>> # With date filters and sorting
            >>> recent = await memoir.users.list(ListUsersFilter(
            ...
            >>>
            >>> # Pagination
            >>> page2 = await memoir.users.list(ListUsersFilter(limit=10, offset=10))
        """
        # Client-side validation
        validate_list_users_filter(filters)
    ...
    async def count(self, filters: Optional[Dict[str, Any]] = None) -> int:
        ...
        Example:
            >>> total = await memoir.users.count()
        """
        # Client-side validation
    ...
    async def get_or_create(
        ...
        Example:
            >>> user = await memoir.users.get_or_create(
            ...     'user-123',
    ...
    async def export(self, options: Optional[ExportUsersOptions] = None) -> str:
        ...
        Example:
            >>> # Basic JSON export
            >>> json_str = await memoir.users.export(ExportUsersOptions(format='json'))
            >>>
            >>> # Export with version history
            >>> with_history = await memoir.users.export(ExportUsersOptions(
            ...     format='json',
            ...     include_version_history=True
            ... ))
            >>>
            >>> # Full GDPR export with conversations and memories
            >>> gdpr_export = await memoir.users.export(ExportUsersOptions(
            ...     format='json',
    ...
    async def get_version(
        ...
        Example:
            >>> v1 = await memoir.users.get_version('user-123', 1)
        """
        # Client-side validation
        validate_user_id(user_id)
    ...
    async def get_history(self, user_id: str) -> List[UserVersion]:
        ...
        Example:
            >>> history = await memoir.users.get_history('user-123')
        """
        # Client-side validation
        validate_user_id(user_id)
    ...
    async def get_at_timestamp(
        ...
        Example:
            >>> august_profile = await memoir.users.get_at_timestamp(
