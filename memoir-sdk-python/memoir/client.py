"""
Memoir SDK - Main Client

Main entry point for the Memoir SDK providing access to all memory operations.
"""
...
class Memoir:
    ...
    Example:
        >>> config = MemoirConfig(convex_url="https://your-deployment.convex.cloud")
    ...
    def _auto_configure_llm() -> Optional[LLMConfig]:
        ...
        """
        fact_extraction_enabled = os.environ.get("MEMOIR_FACT_EXTRACTION") == "true"
    ...
    async def _auto_configure_graph() -> Optional[GraphConfig]:
        ...
        """
        graph_sync_enabled = os.environ.get("MEMOIR_GRAPH_SYNC") == "true"
        ...
        if neo4j_uri:
            try:
                ...
                warnings.warn(
                    "[Memoir] neo4j package not installed. "
                    "Run: pip install memoir[graph]",
                    UserWarning,
                    stacklevel=2,
                )
                return None
            except Exception as e:
                print(f"[Memoir] Failed to connect to Neo4j: {e}")
                return None
        ...
        if memgraph_uri:
            try:
                ...
                print(f"[Memoir] Failed to connect to Memgraph: {e}")
                return None
    ...
    @classmethod
    async def create(cls, config: MemoirConfig) -> "Memoir":
        """
        ...
        Use this instead of `Memoir()` when you want environment-based auto-config.

        Example:
            >>> # With env vars: MEMOIR_GRAPH_SYNC=true, NEO4J_URI=bolt://localhost:7687
            >>> memoir = await Memoir.create(MemoirConfig(convex_url=os.getenv("CONVEX_URL")))
            >>> # Graph is automatically connected and sync worker started

        Args:
            config: Memoir configuration (explicit config takes priority over env vars)

        Returns:
            Fully configured Memoir instance
        """
    ...
    def __init__(self, config: MemoirConfig) -> None:
        """
        Initialize Memoir SDK.

        Args:
            config: Memoir configuration including Convex URL and optional graph config
        ...
        self._llm_config: Optional[LLMConfig] = config.llm or Memoir._auto_configure_llm()
    ...
    def get_resilience(self) -> ResilienceLayer:
        ...
        Example:
            >>> # Check system health
            >>> is_healthy = memoir.get_resilience().is_healthy()
            >>>
            >>> # Get current metrics
            >>> metrics = memoir.get_resilience().get_metrics()
