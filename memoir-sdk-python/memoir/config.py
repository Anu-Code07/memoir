...
def resolve_fact_extraction_model(
    ...
    if config_model:
        return config_model
    env_model = os.environ.get("MEMOIR_FACT_EXTRACTION_MODEL")
    if env_model:
        return env_model
...
def resolve_conflict_resolution_model(
    ...
    if config_model:
        return config_model
    env_model = os.environ.get("MEMOIR_CONFLICT_RESOLUTION_MODEL")
    if env_model:
        return env_model
    env_model = os.environ.get("MEMOIR_FACT_EXTRACTION_MODEL")
    if env_model:
...
def resolve_embedding_model(config_model: Optional[str] = None) -> str:
    ...
    if config_model:
        return config_model
    env_model = os.environ.get("MEMOIR_EMBEDDING_MODEL")
    if env_model:
        return env_model
...
def get_recall_defaults() -> ResolvedRecallLimits:
    ...
    return ResolvedRecallLimits(
        memories=_parse_env_int(os.environ.get("MEMOIR_RECALL_LIMIT_MEMORIES"), 20),
        facts=_parse_env_int(os.environ.get("MEMOIR_RECALL_LIMIT_FACTS"), 15),
        graph_hops=_parse_env_int(os.environ.get("MEMOIR_RECALL_GRAPH_HOPS"), 2),
        graph_entities_per_hop=_parse_env_int(
            os.environ.get("MEMOIR_RECALL_GRAPH_ENTITIES_PER_HOP"), 5
        ),
        graph_results_per_entity=_parse_env_int(
            os.environ.get("MEMOIR_RECALL_GRAPH_RESULTS_PER_ENTITY"), 3
        ),
        total=_parse_env_int(os.environ.get("MEMOIR_RECALL_LIMIT_TOTAL"), 30),
    )
