"""
Memoir SDK - Error Handling

All error types and error codes used throughout the Memoir SDK.
"""
...
class MemoirError(Exception):
    """Base exception for all Memoir errors with structured error codes."""

    def __init__(self, code: str, message: str = "", details: Optional[Any] = None) -> None:
        """
        Initialize Memoir error.
...
class ErrorCode:
    """Error codes used throughout Memoir SDK."""
...
def is_memoir_error(error: Any) -> bool:
    """Type guard for MemoirError."""
    return isinstance(error, MemoirError)
