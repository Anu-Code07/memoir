...
def _parse_facts_response(content: str) -> Optional[List[ExtractedFact]]:
    ...
    try:
        ...
        if not isinstance(facts_data, list):
            print("[Memoir LLM] Invalid facts response format - not a list")
            return None
        ...
        print(f"[Memoir LLM] Failed to parse facts response: {e}")
        return None
    except Exception as e:
        print(f"[Memoir LLM] Error parsing facts: {e}")
        return None
...
class OpenAIClient(LLMClient):
    ...
    async def extract_facts(
        ...
        try:
            ...
            model = (
                self.config.model
                or os.environ.get("MEMOIR_FACT_EXTRACTION_MODEL")
            ...
            if not content:
                print("[Memoir LLM] OpenAI returned empty response")
                return None

            return _parse_facts_response(content)

        except ImportError:
            print("[Memoir LLM] OpenAI SDK not installed. Run: pip install openai")
            return None
        except Exception as e:
            print(f"[Memoir LLM] OpenAI extraction failed: {e}")
            return None
    ...
    async def complete(
        ...
        *,
        ...
        content = response.choices[0].message.content
        if not content:
            raise Exception("[Memoir LLM] OpenAI returned empty response")
...
class AnthropicClient(LLMClient):
    ...
    async def extract_facts(
        ...
        try:
            ...
            if not text_block:
                print("[Memoir LLM] Anthropic returned no text content")
                return None
            ...
            print(
                "[Memoir LLM] Anthropic SDK not installed. Run: pip install anthropic"
            )
            return None
        except Exception as e:
            print(f"[Memoir LLM] Anthropic extraction failed: {e}")
            return None
    ...
    async def complete(
        ...
        *,
        ...
        if not text_block:
            raise Exception("[Memoir LLM] Anthropic returned no text content")
...
def create_llm_client(config: "LLMConfig") -> Optional[LLMClient]:
    ...
    if config.provider == "openai":
        ...
        print("[Memoir LLM] Custom provider requires extract_facts function in config")
        return None
    else:
        print(f"[Memoir LLM] Unknown provider: {config.provider}")
        return None
