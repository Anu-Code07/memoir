"""
Memoir SDK - Graph Sync Worker

Real-time reactive worker for syncing Memoir entities to graph database.
"""
...
class GraphSyncWorker:
    ...
    Example:
        ...
        >>> from memoir.graph.worker import GraphSyncWorker
        >>>
        >>> adapter = CypherGraphAdapter()
        >>> await adapter.connect(config)
        >>>
        >>> worker = GraphSyncWorker(memoir, adapter)
        >>> await worker.start()
        >>>
        >>> # Check health
    ...
    async def _fetch_queue_items(self, limit: int) -> List[SyncQueueItem]:
        ...
        try:
            ...
            result = await self._memoir._client.query(
    ...
    async def _mark_synced(self, queue_id: str) -> None:
        ...
        try:
            await self._memoir._client.mutation(
