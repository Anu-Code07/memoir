...
async def initial_graph_sync(
    ...
    Example:
        >>> from memoir.graph.adapters import CypherGraphAdapter
        >>>
        ...
        ... ))
        >>>
        >>> result = await initial_graph_sync(memoir, adapter, BatchSyncOptions(
    ...
    try:
        ...
        memory_spaces_result = await _sync_memory_spaces(
            memoir, adapter, limits.memory_spaces, opts.on_progress
        )
        ...
        contexts_result = await _sync_contexts(
            memoir, adapter, sync_rels, limits.contexts, opts.on_progress
        )
        ...
        conversations_result = await _sync_conversations(
            memoir, adapter, sync_rels, limits.conversations, opts.on_progress
        )
        ...
        memories_result = await _sync_memories(
            memoir, adapter, sync_rels, limits.memories, opts.on_progress
        )
        ...
        facts_result = await _sync_facts(
            memoir, adapter, sync_rels, limits.facts, opts.on_progress
        )
        ...
        users_result = await _sync_users(memoir, adapter, limits.users, opts.on_progress)
        result.users = users_result["stats"]
        result.errors.extend(users_result["errors"])

        # Phase 7: Sync Agents
        print("📦 Phase 7: Syncing Agents...")
        agents_result = await _sync_agents(memoir, adapter, limits.agents, opts.on_progress)
...
async def _sync_memory_spaces(
    ...
    try:
        ...
        memory_spaces_result = await memoir.memory_spaces.list(
...
async def _sync_contexts(
    ...
    try:
        ...
        effective_limit = min(limit, 1000)
        contexts = await memoir.contexts.list(ListContextsFilter(limit=effective_limit))
...
async def _sync_conversations(
    ...
    try:
        ...
        conversations_result = await memoir.conversations.list(
    ...
    try:
        ...
        for memory_space in memory_spaces:
            ...
            try:
                ...
                memories = await memoir.vector.list(
    ...
    try:
        ...
        for memory_space in memory_spaces:
            ...
            try:
                ...
                facts = await memoir.facts.list(ListFactsFilter(
...
async def _sync_users(
    ...
    try:
        # List all users
        users_result = await memoir.users.list(ListUsersFilter(limit=limit))
...
async def _sync_agents(
    ...
    try:
        ...
        agents = await memoir.agents.list()
