...
async def collab_spaces(module_memoir_client, test_run_context):
...
@pytest.mark.asyncio
async def test_each_organization_has_separate_data_by_default(memoir_client, collab_spaces):
    """
    ...
    org_a_result = await memoir_client.vector.list(memory_space_id=org_a_space)
    ...
    org_b_result = await memoir_client.vector.list(memory_space_id=org_b_space)
...
@pytest.mark.asyncio
async def test_shares_workflow_context_across_organizations(memoir_client, collab_spaces):
    """
    ...
    shared_context = await memoir_client.contexts.create(
        ContextInput(
    ...
    updated = await memoir_client.contexts.grant_access(
        shared_context.id,
        org_b_space,
        "read-only",
    )
    ...
    org_a_view = await memoir_client.contexts.get(shared_context.id)
...
@pytest.mark.asyncio
async def test_creates_child_contexts_in_different_spaces(memoir_client, collab_spaces):
    """
    ...
    org_a_task = await memoir_client.contexts.create(
        ContextInput(
    ...
    org_b_task = await memoir_client.contexts.create(
        ContextInput(
...
@pytest.mark.asyncio
async def test_contexts_can_grant_selective_access(memoir_client, collab_spaces):
    """
    ...
    private_context = await memoir_client.contexts.create(
        ContextInput(
...
@pytest.mark.asyncio
async def test_can_grant_different_scopes(memoir_client, collab_spaces):
    """
    ...
    context = await memoir_client.contexts.create(
        ContextInput(
    ...
    await memoir_client.contexts.grant_access(
        context.id,
        org_b_space,
        "read-only",
    )
...
@pytest.mark.asyncio
async def test_partner_organizations_collaborate_on_joint_campaign(memoir_client, collab_spaces):
    """
    ...
    campaign = await memoir_client.contexts.create(
        ContextInput(
    ...
    chain_a = await memoir_client.contexts.get_chain(org_a_task.id)
    chain_b = await memoir_client.contexts.get_chain(org_b_task.id)
    ...
    org_a_facts_result = await memoir_client.facts.list(
    ...
    org_b_facts_result = await memoir_client.facts.list(ListFactsFilter(memory_space_id=org_b_space))
...
@pytest.mark.asyncio
async def test_coordinates_tasks_across_organizations(memoir_client, collab_spaces):
    """
...
@pytest.mark.asyncio
async def test_shares_only_context_not_underlying_data(memoir_client, collab_spaces):
    """
    ...
    shared_ctx = await memoir_client.contexts.get(context.id)

    assert shared_ctx.data.get("projectName") == "Joint Initiative"

    # But Org B CANNOT see Org A's facts
    org_b_fact_view_result = await memoir_client.facts.list(ListFactsFilter(memory_space_id=org_b_space))
...
@pytest.mark.asyncio
async def test_demonstrates_hive_vs_collaboration_difference(memoir_client, collab_spaces):
    """
    ...
    hive_facts_result = await memoir_client.facts.list(ListFactsFilter(memory_space_id=hive_space))
    ...
    shared_project = await memoir_client.contexts.create(
        ContextInput(
    ...
    # Facts stay isolated
    x_facts_result = await memoir_client.facts.list(ListFactsFilter(memory_space_id=company_x))
    x_facts = x_facts_result if isinstance(x_facts_result, list) else x_facts_result.get("facts", [])

    y_facts_result = await memoir_client.facts.list(ListFactsFilter(memory_space_id=company_y))
    ...
    context = await memoir_client.contexts.get(shared_project.id)
