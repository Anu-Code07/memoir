...
from memoir.types import CreateShareInput
...
async def test_share_link_with_defaults(
    ...
    result = await memoir_client.conversations.share(
        CreateShareInput(
...
@pytest.mark.asyncio
async def test_revoke_share(memoir_client, test_memory_space_id, test_user_id):
    ...
    share_result = await memoir_client.conversations.share(
        CreateShareInput(
            conversation_id=conv.conversation_id,
            grant_type="link",
        )
    ...
    revoke_result = await memoir_client.conversations.revoke_share(
...
@pytest.mark.asyncio
async def test_list_shares(memoir_client, test_memory_space_id, test_user_id):
    ...
    await memoir_client.conversations.share(
    ...
    shares = await memoir_client.conversations.list_shares(conv.conversation_id)
...
async def test_list_shares_filter_by_status(
    ...
    share1 = await memoir_client.conversations.share(
        CreateShareInput(conversation_id=conv.conversation_id, grant_type="link")
    )
    await memoir_client.conversations.revoke_share(share1.share_id)
    ...
    active_shares = await memoir_client.conversations.list_shares(
...
@pytest.mark.asyncio
async def test_get_share(memoir_client, test_memory_space_id, test_user_id):
    ...
    retrieved = await memoir_client.conversations.get_share(share_result.share_id)
...
@pytest.mark.asyncio
async def test_get_share_not_found(memoir_client):
    """Test retrieving a non-existent share."""
    result = await memoir_client.conversations.get_share("share-nonexistent")
...
async def test_check_share_access_link_share(
    ...
    access = await memoir_client.conversations.check_share_access(
        conversation_id=conv.conversation_id,
        user_id="random-user",
    )
