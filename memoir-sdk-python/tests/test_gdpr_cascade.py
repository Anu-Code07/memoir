...
from memoir import DeleteUserOptions, RememberParams
...
@pytest.mark.asyncio
async def test_cascade_delete_user_profile_layer(memoir_client, test_user_id, cleanup_helper):
    """
    ...
    user_before = await memoir_client.users.get(test_user_id)
    ...
    user_after = await memoir_client.users.get(test_user_id)
...
@pytest.mark.asyncio
async def test_cascade_delete_conversations_layer(memoir_client, test_ids, cleanup_helper):
    """
    ...
    from memoir import ConversationParticipants, CreateConversationInput
    ...
    conv = await memoir_client.conversations.get(conversation_id)
...
@pytest.mark.asyncio
async def test_cascade_delete_memories_layer(memoir_client, test_ids, cleanup_helper):
    """
    ...
    # Create user
    await memoir_client.users.update(user_id, {"displayName": "Memory Cascade Test"})
    ...
    count_before = await memoir_client.vector.count(
        memory_space_id,
        user_id=user_id,
    )
    ...
    # Delete user with cascade
    delete_result = await memoir_client.users.delete(
        user_id,
        DeleteUserOptions(cascade=True),
    )
    ...
    count_after = await memoir_client.vector.count(
        memory_space_id,
        user_id=user_id,
    )
...
@pytest.mark.asyncio
async def test_cascade_with_verification(memoir_client, ctx, cleanup_helper):
    """
    ...
    await memoir_client.users.update(user_id, {"displayName": "Verify Test"})
...
@pytest.mark.asyncio
async def test_cascade_dry_run(memoir_client, test_ids, cleanup_helper):
    """
    ...
    # Create user with data
    await memoir_client.users.update(user_id, {"displayName": "Dry Run Test"})
    ...
    count_before = await memoir_client.vector.count(memory_space_id, user_id=user_id)
    ...
    count_after = await memoir_client.vector.count(memory_space_id, user_id=user_id)
...
@pytest.mark.asyncio
async def test_cascade_all_layers(memoir_client, test_ids, cleanup_helper):
    """
    ...
    await memoir_client.users.update(user_id, {"displayName": "All Layers Test"})
