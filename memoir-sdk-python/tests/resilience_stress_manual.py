...
def print_metrics(memoir: Memoir) -> None:
    """Print current resilience metrics."""
    metrics = memoir.get_resilience_metrics()
    print("\n📊 Current Metrics:")
    print(
...
async def test_burst_rate_limiting():
    ...
    async def make_call(i: int):
        ...
        try:
            await memoir.memory.search(test_space_id, f"test query {i}")
            results["succeeded"] += 1
...
async def test_concurrency_saturation():
    ...
    # Cleanup
    try:
        await memoir.memory_spaces.delete(test_space_id)
    except Exception:
        pass
...
async def test_circuit_breaker_trip_and_recovery():
    ...
    for i in range(7):
        try:
            ...
            await memoir.memory.search("", f"query {i}")
    ...
    for i in range(3):
        start = time.time()
        try:
            await memoir.memory.search("test-space", f"query {i}")
    ...
    for i in range(3):
        try:
            await memoir.memory.search(test_space_id, f"recovery query {i}")
...
async def test_priority_queue_under_load():
    ...
    async def low_priority_call(i: int):
        try:
            await memoir.memory.search(test_space_id, f"low query {i}")
...
async def test_graceful_shutdown():
    ...
    async def make_call(i: int):
        try:
            await memoir.memory.search(test_space_id, f"shutdown query {i}")
            return "success"
        except Exception:
            return "failed"
    ...
    await memoir.shutdown(timeout_s=10.0)
