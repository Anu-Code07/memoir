...
- Gate 2: MEMOIR_GRAPH_SYNC must be set to 'true'
...
from memoir.client import Memoir
from memoir.types import MemoirConfig, GraphConfig
...
class TestGraphAutoConfiguration:
    ...
    async def test_no_auto_config_when_only_uri_set(self):
        ...
        # MEMOIR_GRAPH_SYNC is NOT set

        result = await Memoir._auto_configure_graph()
    ...
    async def test_no_auto_config_when_only_graph_sync_set(self):
        ...
        os.environ["MEMOIR_GRAPH_SYNC"] = "true"
        # No URI set
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            assert result is None
            assert len(w) == 1
            assert "MEMOIR_GRAPH_SYNC=true but no graph database URI found" in str(w[0].message)
    ...
    async def test_no_auto_config_when_graph_sync_false(self):
        """MEMOIR_GRAPH_SYNC='false' should NOT trigger auto-config."""
        os.environ["MEMOIR_GRAPH_SYNC"] = "false"
        os.environ["NEO4J_URI"] = "bolt://localhost:7687"
    ...
    async def test_no_auto_config_when_graph_sync_empty(self):
        """MEMOIR_GRAPH_SYNC='' should NOT trigger auto-config."""
        os.environ["MEMOIR_GRAPH_SYNC"] = ""
        os.environ["NEO4J_URI"] = "bolt://localhost:7687"
...
class TestMemoirCreateFactory:
    """Test Memoir.create() factory method."""
    ...
    async def test_create_without_graph_config(self):
        """Memoir.create() works without graph config."""
        config = MemoirConfig(convex_url="http://127.0.0.1:3210")

        memoir = await Memoir.create(config)

        assert memoir is not None
        assert memoir.graph_adapter is None
    ...
    async def test_create_uses_explicit_graph_config(self):
        ...
        explicit_config = MemoirConfig(
            convex_url="http://127.0.0.1:3210",
            graph=GraphConfig(adapter=mock_adapter),
        )

        memoir = await Memoir.create(explicit_config)

        assert memoir is not None
        assert memoir.graph_adapter is mock_adapter
