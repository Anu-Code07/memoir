...
- Gate 2: MEMOIR_FACT_EXTRACTION must be set to 'true'
...
from memoir.types import LLMConfig
...
class TestLLMAutoConfiguration:
    ...
    def test_no_auto_config_when_only_api_key_set(self):
        ...
        # MEMOIR_FACT_EXTRACTION is NOT set

        result = Memoir._auto_configure_llm()
    ...
    def test_no_auto_config_when_only_fact_extraction_set(self):
        ...
        os.environ["MEMOIR_FACT_EXTRACTION"] = "true"
        # No API key set
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            assert result is None
            assert len(w) == 1
            assert "MEMOIR_FACT_EXTRACTION=true but no API key found" in str(w[0].message)
    ...
    def test_no_auto_config_when_fact_extraction_false(self):
        """MEMOIR_FACT_EXTRACTION='false' should NOT trigger auto-config."""
        os.environ["MEMOIR_FACT_EXTRACTION"] = "false"
        os.environ["OPENAI_API_KEY"] = "sk-test-key"
    ...
    def test_no_auto_config_when_fact_extraction_empty(self):
        """MEMOIR_FACT_EXTRACTION='' should NOT trigger auto-config."""
        os.environ["MEMOIR_FACT_EXTRACTION"] = ""
        os.environ["OPENAI_API_KEY"] = "sk-test-key"
    ...
    def test_no_auto_config_when_fact_extraction_any_other_value(self):
        """MEMOIR_FACT_EXTRACTION='yes' (not 'true') should NOT trigger auto-config."""
        os.environ["MEMOIR_FACT_EXTRACTION"] = "yes"
        os.environ["OPENAI_API_KEY"] = "sk-test-key"
...
class TestExplicitConfigOverride:
    """Test that explicit MemoirConfig.llm takes priority over env vars."""
