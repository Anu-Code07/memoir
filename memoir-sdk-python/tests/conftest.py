...
from memoir import Memoir, MemoirConfig  # noqa: E402
...
# Memoir client fixture (function scope for test isolation)
@pytest.fixture(scope="function")
async def memoir_client(test_config) -> AsyncGenerator[Memoir, None]:
    """
    Memoir client fixture for tests.

    Creates a fresh Memoir instance for each test function.
    """
...
@pytest.fixture(scope="function")
async def cleanup_helper(memoir_client):
    """TestCleanup helper for tests that need cleanup."""
    return TestCleanup(memoir_client)
...
def test_run_context(request) -> TestRunContext:
    ...
    Example:
        def test_something(test_run_context, memoir_client):
            user_id = test_run_context.user_id("alice")
            await memoir_client.users.update(user_id, {"name": "Alice"})
    """
...
@pytest.fixture(scope="module")
async def module_memoir_client(test_config) -> AsyncGenerator[Memoir, None]:
    """
    Module-scoped Memoir client for fixtures that need module-level persistence.

    Use this for module-scoped fixtures like scoped_cleanup. For individual
    test functions, use the function-scoped memoir_client fixture instead.
    """
    convex_url = test_config["convex_url"]
    client = Memoir(MemoirConfig(convex_url=convex_url))
...
@pytest.fixture(scope="module")
async def scoped_cleanup(module_memoir_client, test_run_context) -> AsyncGenerator[ScopedCleanup, None]:
    """
    ...
    Note: Uses module_memoir_client (module-scoped) instead of memoir_client
    (function-scoped) to avoid pytest scope mismatch errors.

    Example:
        @pytest.fixture(scope="module")
        async def setup_teardown(scoped_cleanup, memoir_client, test_run_context):
            # Setup: create test data
            yield
            # Teardown: cleanup only this run's data
            await scoped_cleanup.cleanup_all()
    """
    cleanup = ScopedCleanup(module_memoir_client, test_run_context)
    yield cleanup
...
def ctx(test_run_context) -> TestRunContext:
    ...
    Shorter name for use in test functions:
        def test_something(ctx, memoir_client):
