...
from memoir.types import AgentFilters, AgentRegistration, ContextInput, RegisterMemorySpaceParams
...
class TestContextStateTransitions:
    ...
    async def test_context_transition(self, memoir_client, base_id, test_user_id, from_status, to_status):
    ...
    async def test_context_count_reflects_transition(
        self, memoir_client, base_id, test_user_id, from_status, to_status
    ):
        ...
        # Get initial counts
        before_from_count = await memoir_client.contexts.count(
            memory_space_id=space_id, status=from_status
        )
        before_to_count = await memoir_client.contexts.count(
            memory_space_id=space_id, status=to_status
        )
        ...
        await memoir_client.contexts.update(ctx.id, updates={"status": to_status})

        # Get final counts
        after_from_count = await memoir_client.contexts.count(
            memory_space_id=space_id, status=from_status
        )
        after_to_count = await memoir_client.contexts.count(
            memory_space_id=space_id, status=to_status
        )
    ...
    async def test_data_preservation_through_transitions(
        self, memoir_client, base_id, test_user_id
    ):
        ...
        await memoir_client.contexts.update(ctx.id, updates={"status": "blocked"})
        blocked = await memoir_client.contexts.get(ctx.id)
        assert blocked.data == original_data

        await memoir_client.contexts.update(ctx.id, updates={"status": "completed"})
        completed = await memoir_client.contexts.get(ctx.id)
        assert completed.data == original_data

        await memoir_client.contexts.update(ctx.id, updates={"status": "active"})
        reactivated = await memoir_client.contexts.get(ctx.id)
    ...
    async def test_completed_at_set_on_completion(
        ...
        completed = await memoir_client.contexts.update(
    ...
    async def test_parent_child_preserved_through_transitions(
        ...
        # Transition parent
        await memoir_client.contexts.update(parent.id, updates={"status": "completed"})

        # Verify child still references parent
        child_after = await memoir_client.contexts.get(child.id)
        assert child_after.parent_id == parent.id

    async def test_rapid_state_transitions(self, memoir_client, base_id, test_user_id):
        ...
        final = await memoir_client.contexts.get(ctx.id)
    ...
    async def test_all_statuses_independently(
        ...
        for ctx, status in contexts:
            status_list = await memoir_client.contexts.list(memory_space_id=space_id, status=status)
            assert any(c.id == ctx.id for c in status_list)

        # Transition each to different status
        await memoir_client.contexts.update(contexts[0][0].id, updates={"status": "completed"})
        await memoir_client.contexts.update(contexts[1][0].id, updates={"status": "active"})
        await memoir_client.contexts.update(contexts[2][0].id, updates={"status": "blocked"})
        await memoir_client.contexts.update(contexts[3][0].id, updates={"status": "cancelled"})

        # Verify new states
        ctx0 = await memoir_client.contexts.get(contexts[0][0].id)
        ctx1 = await memoir_client.contexts.get(contexts[1][0].id)
        ctx2 = await memoir_client.contexts.get(contexts[2][0].id)
        ctx3 = await memoir_client.contexts.get(contexts[3][0].id)
...
class TestMemorySpaceStateTransitions:
    ...
    async def test_active_to_archived_transition(self, memoir_client, base_id):
        """Test transition from active to archived."""
        import time
        space_id = f"{base_id}-space-archive-{int(time.time() * 1000)}"

        # Register as active
        space = await memoir_client.memory_spaces.register(
        ...
        active_list = await memoir_client.memory_spaces.list(status="active")
        assert any(s.memory_space_id == space_id for s in active_list.spaces)

        # Archive
        archived = await memoir_client.memory_spaces.archive(space_id)
        assert archived.status == "archived"

        # Verify in archived list
        archived_list = await memoir_client.memory_spaces.list(status="archived")
        assert any(s.memory_space_id == space_id for s in archived_list.spaces)

        # Verify NOT in active list
        active_list_after = await memoir_client.memory_spaces.list(status="active")
        assert not any(
            s.memory_space_id == space_id for s in active_list_after.spaces
        )

    async def test_archived_to_active_via_reactivate(self, memoir_client, base_id):
        ...
        await memoir_client.memory_spaces.archive(space_id)
        ...
        # Reactivate
        reactivated = await memoir_client.memory_spaces.reactivate(space_id)
    ...
    async def test_count_reflects_archive_transition(self, memoir_client, base_id):
        ...
        before_archive = await memoir_client.memory_spaces.get(space_id)
        ...
        after_archive = await memoir_client.memory_spaces.get(space_id)
    ...
    async def test_metadata_preserved_through_archive_cycle(
        ...
        # Archive
        await memoir_client.memory_spaces.archive(space_id, reason="Project completed")

        archived = await memoir_client.memory_spaces.get(space_id)
        assert archived.metadata.get("project_name") == "Test Project"
        assert archived.metadata.get("owner") == "team-alpha"

        # Reactivate
        await memoir_client.memory_spaces.reactivate(space_id)

        reactivated = await memoir_client.memory_spaces.get(space_id)
        assert reactivated.metadata.get("project_name") == "Test Project"
        assert reactivated.metadata.get("owner") == "team-alpha"

    async def test_participants_preserved_through_archive(self, memoir_client, base_id):
        ...
        space = await memoir_client.memory_spaces.get(space_id)
    ...
    async def test_archived_space_queryable_and_updatable(
        ...
        await memoir_client.memory_spaces.update(
            space_id, updates={"metadata": {"note": "Updated while archived"}}
        )

        updated = await memoir_client.memory_spaces.get(space_id)
        assert updated.metadata.get("note") == "Updated while archived"

    async def test_archive_with_reason_stores_metadata(self, memoir_client, base_id):
        ...
        archived = await memoir_client.memory_spaces.archive(
...
class TestAgentStateTransitions:
    ...
    async def test_agent_transition(
        self, memoir_client, from_status, to_status
    ):
        ...
        agent = await memoir_client.agents.register(
            AgentRegistration(
                id=agent_id,
        ...
        if from_status != "active":
            await memoir_client.agents.update(agent_id, updates={"status": from_status})

        # Then transition to to_status
        updated = await memoir_client.agents.update(agent_id, updates={"status": to_status})
        assert updated.status == to_status

        # Verify in list
        agent_list = await memoir_client.agents.list(AgentFilters(status=to_status))
        assert any(a.id == agent_id for a in agent_list)

        # Verify NOT in from_status list
        old_list = await memoir_client.agents.list(AgentFilters(status=from_status))
        assert not any(a.id == agent_id for a in old_list)

    async def test_inactive_agent_preserves_capabilities(self, memoir_client):
        ...
        # Deactivate
        await memoir_client.agents.update(agent_id, updates={"status": "inactive"})

        inactive = await memoir_client.agents.get(agent_id)
        assert inactive.status == "inactive"
        assert inactive.metadata.get("capabilities") == ["code", "analysis", "testing"]

        # Reactivate
        await memoir_client.agents.update(agent_id, updates={"status": "active"})

        reactivated = await memoir_client.agents.get(agent_id)
        assert reactivated.status == "active"
...
class TestCrossEntityStateEffects:
    ...
    async def test_archiving_space_preserves_contexts(
        ...
        context_after = await memoir_client.contexts.get(ctx.id)
    ...
    async def test_multiple_statuses_in_same_space(
        ...
        active = await memoir_client.contexts.create(
            ContextInput(
                memory_space_id=space_id,
                user_id=user_id,
        ...
        completed = await memoir_client.contexts.create(
            ContextInput(
                memory_space_id=space_id,
                user_id=user_id,
        ...
        blocked = await memoir_client.contexts.create(
            ContextInput(
                memory_space_id=space_id,
                user_id=user_id,
        ...
        # Verify each in correct status list
        active_list = await memoir_client.contexts.list(
            memory_space_id=space_id, status="active"
        )
        completed_list = await memoir_client.contexts.list(
            memory_space_id=space_id, status="completed"
        )
        blocked_list = await memoir_client.contexts.list(memory_space_id=space_id, status="blocked")
...
class TestStateTransitionEdgeCases:
    ...
    async def test_concurrent_transitions_handled(
        ...
        await asyncio.gather(
            memoir_client.contexts.update(ctx.id, updates={"status": "completed"}),
            memoir_client.contexts.update(ctx.id, updates={"status": "blocked"}),
            memoir_client.contexts.update(ctx.id, updates={"status": "cancelled"}),
    ...
    async def test_transition_with_data_update(self, memoir_client, base_id, test_user_id):
    ...
    async def test_all_status_values_exhaustive(
        ...
        for status in CONTEXT_STATUSES:
            ...
            # Verify retrievable
            retrieved = await memoir_client.contexts.get(ctx.id)
    ...
    async def test_archived_space_with_data_reactivatable(
        ...
        from memoir.types import CreateConversationInput, StoreMemoryInput
        ...
        mem = await memoir_client.vector.store(
            space_id,
            StoreMemoryInput(
                content="Test memory",
                content_type="raw",
        ...
        conv_archived = await memoir_client.conversations.get(conv.conversation_id)
        mem_archived = await memoir_client.vector.get(space_id, mem.memory_id)
        ...
        conv_reactivated = await memoir_client.conversations.get(conv.conversation_id)
        mem_reactivated = await memoir_client.vector.get(space_id, mem.memory_id)
...
class TestInvalidTransitions:
    ...
    async def test_invalid_status_rejected(self, memoir_client, base_id, test_user_id):
        ...
        with pytest.raises(Exception):
            await memoir_client.contexts.update(
...
class TestBatchStateTransitions:
    ...
    async def test_multiple_contexts_transition_simultaneously(
        ...
        for ctx in contexts:
            updated = await memoir_client.contexts.get(ctx.id)
            assert updated.status == "completed"
    ...
    async def test_multiple_spaces_transition_independently(self, memoir_client, base_id):
        ...
        await memoir_client.memory_spaces.archive(spaces[0].memory_space_id)
        await memoir_client.memory_spaces.archive(spaces[1].memory_space_id)

        # Verify states
        space0 = await memoir_client.memory_spaces.get(spaces[0].memory_space_id)
        space1 = await memoir_client.memory_spaces.get(spaces[1].memory_space_id)
        space2 = await memoir_client.memory_spaces.get(spaces[2].memory_space_id)
