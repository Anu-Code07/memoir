...
class TestConversationsFilterParametrized:
    ...
    @pytest.mark.asyncio
    async def test_list_filters_by_type(self, memoir_client, conv_type):
        ...
        if conv_type == "user-agent":
            target_conv = await memoir_client.conversations.create(
                CreateConversationInput(
    ...
    @pytest.mark.asyncio
    async def test_count_filters_by_type(self, memoir_client, conv_type):
    ...
    @pytest.mark.asyncio
    async def test_search_filters_by_type(self, memoir_client, conv_type):
        ...
        if conv_type == "user-agent":
            noise_conv = await memoir_client.conversations.create(
                CreateConversationInput(
                    memory_space_id=space_id,
                    type="agent-agent",
                    participants=ConversationParticipants(
...
class TestConversationsFilterEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_list_both_types_exist(self, memoir_client):
        """Test that both conversation types can coexist in same memory space."""
        space_id = generate_test_memory_space_id()

        # Create user-agent conversation
        ua_conv = await memoir_client.conversations.create(
            CreateConversationInput(
                memory_space_id=space_id,
                type="user-agent",
        ...
        aa_conv = await memoir_client.conversations.create(
            CreateConversationInput(
                memory_space_id=space_id,
                type="agent-agent",
                participants=ConversationParticipants(
        ...
        # List user-agent
        ua_result = await memoir_client.conversations.list(
        ...
        # List agent-agent
        aa_result = await memoir_client.conversations.list(
            ListConversationsFilter(type="agent-agent", memory_space_id=space_id)
        )
    ...
    @pytest.mark.asyncio
    async def test_combine_type_with_user_id_filter(self, memoir_client):
        """Test combining type filter with userId filter."""
        space_id = generate_test_memory_space_id()
        target_user = "combine-user"

        # Create user-agent conversation with target user
        _target_conv = await memoir_client.conversations.create(
            CreateConversationInput(
                memory_space_id=space_id,
                type="user-agent",
    ...
    @pytest.mark.asyncio
    async def test_combine_type_with_memory_space_filter(self, memoir_client):
        ...
        _conv_space_1 = await memoir_client.conversations.create(
            CreateConversationInput(
    ...
    async def test_count_all_vs_filtered(self, memoir_client):
        ...
        ua_count = await memoir_client.conversations.count(
    ...
    @pytest.mark.asyncio
    async def test_search_empty_results_for_wrong_type(self, memoir_client):
