...
from memoir import RegisterMemorySpaceParams
from memoir.memory_spaces import MemorySpaceValidationError
...
@pytest.mark.asyncio
async def test_register_missing_memory_space_id(memoir_client):
...
@pytest.mark.asyncio
async def test_register_invalid_memory_space_id_format(memoir_client):
...
@pytest.mark.asyncio
async def test_register_missing_type(memoir_client):
    ...
    with pytest.raises(MemorySpaceValidationError) as exc_info:
        ...
        await memoir_client.memory_spaces.register(params)
    assert "type is required" in str(exc_info.value)


@pytest.mark.asyncio
async def test_register_invalid_type(memoir_client):
...
@pytest.mark.asyncio
async def test_register_invalid_participant_structure(memoir_client):
...
@pytest.mark.asyncio
async def test_register_duplicate_participant_ids(memoir_client):
...
@pytest.mark.asyncio
async def test_register_invalid_name_length(memoir_client):
...
@pytest.mark.asyncio
async def test_get_empty_memory_space_id(memoir_client):
    """Should raise on empty memory_space_id."""
    with pytest.raises(MemorySpaceValidationError) as exc_info:
        await memoir_client.memory_spaces.get("")
    assert "memory_space_id" in str(exc_info.value)


@pytest.mark.asyncio
async def test_list_invalid_type(memoir_client):
    """Should raise on invalid type."""
    with pytest.raises(MemorySpaceValidationError) as exc_info:
        await memoir_client.memory_spaces.list(type="invalid")
    assert "Invalid type" in str(exc_info.value)
...
async def test_list_invalid_status(memoir_client):
    ...
    with pytest.raises(MemorySpaceValidationError) as exc_info:
        await memoir_client.memory_spaces.list(status="deleted")
...
async def test_list_invalid_limit(memoir_client):
    ...
    with pytest.raises(MemorySpaceValidationError) as exc_info:
        await memoir_client.memory_spaces.list(limit=0)
    assert "limit" in str(exc_info.value)


@pytest.mark.asyncio
async def test_list_invalid_offset_negative(memoir_client):
    ...
    from memoir.types import ListMemorySpacesFilter
    ...
    result = await memoir_client.memory_spaces.list(filter=filter_obj)
...
async def test_list_invalid_sort_by(memoir_client):
    ...
    """
    from memoir.types import ListMemorySpacesFilter
    from memoir.errors import MemoirError
    ...
    with pytest.raises(MemoirError):
        await memoir_client.memory_spaces.list(filter=filter_obj)


@pytest.mark.asyncio
async def test_list_invalid_sort_order(memoir_client):
...
@pytest.mark.asyncio
async def test_search_empty_query(memoir_client):
    """Should raise on empty query."""
    with pytest.raises(MemorySpaceValidationError) as exc_info:
        await memoir_client.memory_spaces.search("")
    assert "query" in str(exc_info.value).lower()


@pytest.mark.asyncio
async def test_search_whitespace_query(memoir_client):
...
@pytest.mark.asyncio
async def test_update_empty_memory_space_id(memoir_client):
    """Should raise on empty memory_space_id."""
    with pytest.raises(MemorySpaceValidationError) as exc_info:
        await memoir_client.memory_spaces.update("", {"name": "Test"})
    assert "memory_space_id" in str(exc_info.value)


@pytest.mark.asyncio
async def test_update_no_updates_provided(memoir_client):
    """Should raise on no updates provided."""
    with pytest.raises(MemorySpaceValidationError) as exc_info:
        await memoir_client.memory_spaces.update("valid-id", {})
...
async def test_update_invalid_status(memoir_client):
    """Should raise on invalid status."""
    with pytest.raises(MemorySpaceValidationError) as exc_info:
        await memoir_client.memory_spaces.update("valid-id", {"status": "deleted"})
    assert "Invalid status" in str(exc_info.value)


@pytest.mark.asyncio
async def test_update_participants_empty_memory_space_id(memoir_client):
    """Should raise on empty memory_space_id."""
    with pytest.raises(MemorySpaceValidationError) as exc_info:
        await memoir_client.memory_spaces.update_participants(
...
@pytest.mark.asyncio
async def test_update_participants_no_updates(memoir_client):
    """Should raise when no updates provided."""
    with pytest.raises(MemorySpaceValidationError) as exc_info:
        await memoir_client.memory_spaces.update_participants("valid-id")
    assert "At least one" in str(exc_info.value)


@pytest.mark.asyncio
async def test_archive_empty_memory_space_id(memoir_client):
    """Should raise on empty memory_space_id."""
    with pytest.raises(MemorySpaceValidationError) as exc_info:
        await memoir_client.memory_spaces.archive("")
    assert "memory_space_id" in str(exc_info.value)


@pytest.mark.asyncio
async def test_reactivate_empty_memory_space_id(memoir_client):
    """Should raise on empty memory_space_id."""
    with pytest.raises(MemorySpaceValidationError) as exc_info:
        await memoir_client.memory_spaces.reactivate("")
    assert "memory_space_id" in str(exc_info.value)


@pytest.mark.asyncio
async def test_get_stats_empty_memory_space_id(memoir_client):
    """Should raise on empty memory_space_id."""
    with pytest.raises(MemorySpaceValidationError) as exc_info:
        await memoir_client.memory_spaces.get_stats("")
    assert "memory_space_id" in str(exc_info.value)


@pytest.mark.asyncio
async def test_count_invalid_type(memoir_client):
    """Should raise on invalid type."""
    with pytest.raises(MemorySpaceValidationError) as exc_info:
        await memoir_client.memory_spaces.count(type="PERSONAL")
    assert "Invalid type" in str(exc_info.value)


@pytest.mark.asyncio
async def test_delete_empty_memory_space_id(memoir_client):
    """Should raise on empty memory_space_id."""
    with pytest.raises(MemorySpaceValidationError) as exc_info:
        await memoir_client.memory_spaces.delete("")
    assert "memory_space_id" in str(exc_info.value)
...
@pytest.mark.asyncio
async def test_add_participant_empty_memory_space_id(memoir_client):
    """Should raise on empty memory_space_id for add_participant."""
    with pytest.raises(MemorySpaceValidationError) as exc_info:
        await memoir_client.memory_spaces.add_participant(
...
@pytest.mark.asyncio
async def test_add_participant_invalid_participant(memoir_client):
    """Should raise on invalid participant structure."""
    with pytest.raises(MemorySpaceValidationError) as exc_info:
        await memoir_client.memory_spaces.add_participant(
...
@pytest.mark.asyncio
async def test_remove_participant_empty_memory_space_id(memoir_client):
    """Should raise on empty memory_space_id for remove_participant."""
    with pytest.raises(MemorySpaceValidationError) as exc_info:
        await memoir_client.memory_spaces.remove_participant("", "user-123")
    assert "memory_space_id" in str(exc_info.value)
...
async def test_remove_participant_empty_participant_id(memoir_client):
    ...
    with pytest.raises(MemorySpaceValidationError) as exc_info:
        await memoir_client.memory_spaces.remove_participant("valid-space", "")
    assert "participantId" in str(exc_info.value)


@pytest.mark.asyncio
async def test_find_by_participant_empty_participant_id(memoir_client):
    """Should raise on empty participant_id for find_by_participant."""
    with pytest.raises(MemorySpaceValidationError) as exc_info:
        await memoir_client.memory_spaces.find_by_participant("")
    assert "participantId" in str(exc_info.value)


@pytest.mark.asyncio
async def test_get_stats_invalid_time_window(memoir_client):
    """Should raise on invalid time window."""
    from memoir.types import GetMemorySpaceStatsOptions
...
@pytest.mark.asyncio
async def test_register_memory_space(memoir_client, test_ids):
    """
    ...
    result = await memoir_client.memory_spaces.register(
        RegisterMemorySpaceParams(
    ...
    # Cleanup
    await memoir_client.memory_spaces.delete(memory_space_id)


@pytest.mark.asyncio
async def test_register_shared_memory_space(memoir_client, test_ids):
    """
...
@pytest.mark.asyncio
async def test_register_with_tenant_id(memoir_client, test_ids):
    """
...
@pytest.mark.asyncio
async def test_get_memory_space(memoir_client, test_ids):
    """
    ...
    # Get space
    retrieved = await memoir_client.memory_spaces.get(memory_space_id)
...
async def test_get_nonexistent_returns_none(memoir_client):
    ...
    Port of: memorySpaces.test.ts - get tests
    """
    result = await memoir_client.memory_spaces.get("space-does-not-exist")

    assert result is None


@pytest.mark.asyncio
async def test_get_with_tenant_isolation(memoir_client, test_ids):
    """
...
@pytest.mark.asyncio
async def test_list_memory_spaces(memoir_client, test_ids):
    """
    ...
    # List spaces
    result = await memoir_client.memory_spaces.list(limit=100)
...
@pytest.mark.asyncio
async def test_list_filter_by_type(memoir_client, test_ids):
    """
    ...
    result = await memoir_client.memory_spaces.list(type="personal", limit=100)
    ...
    # Cleanup
    await memoir_client.memory_spaces.delete(personal_id)
    await memoir_client.memory_spaces.delete(shared_id)
...
@pytest.mark.asyncio
async def test_delete_memory_space(memoir_client, test_ids):
    """
...
@pytest.mark.asyncio
async def test_count_memory_spaces(memoir_client, test_ids):
    """
    ...
    # Count spaces
    count = await memoir_client.memory_spaces.count()
...
@pytest.mark.asyncio
async def test_get_memory_space_stats(memoir_client, test_ids, cleanup_helper):
    """
    ...
    await memoir_client.vector.store(memory_space_id, create_test_memory_input())

    # Get stats
    stats = await memoir_client.memory_spaces.get_stats(memory_space_id)
...
@pytest.mark.asyncio
async def test_list_backward_compatible_kwargs(memoir_client, test_ids):
    """
    ...
    result = await memoir_client.memory_spaces.list(
        type="personal",
        status="active",
        limit=10,
        offset=0
