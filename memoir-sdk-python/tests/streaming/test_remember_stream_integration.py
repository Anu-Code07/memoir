...
Tests that validate ACTUAL data storage across all Memoir layers:
...
neo4j = pytest.importorskip("neo4j", reason="neo4j not installed (install with: pip install memoir[graph])")

from memoir import Memoir
from memoir.graph.adapters.cypher import CypherGraphAdapter
from memoir.memory.streaming_types import StreamingOptions
...
@pytest.fixture
async def memoir_with_graph():
    """Memoir client with graph adapter"""
    ...
    from memoir import MemoirConfig, GraphConfig
    client = Memoir(
        MemoirConfig(
...
class TestRememberStreamIntegration:
    ...
    @pytest.mark.asyncio
    async def test_basic_stream_stores_in_convex(self, memoir_with_graph):
        """
        ...
        # Stream response
        result = await memoir_with_graph.memory.remember_stream(
            {
        ...
        conversation = await memoir_with_graph.conversations.get(conversation_id)
    ...
    @pytest.mark.asyncio
    async def test_stream_creates_graph_nodes(self, memoir_with_graph):
        """
        ...
        graph_adapter = memoir_with_graph.graph_adapter
    ...
    @pytest.mark.asyncio
    async def test_stream_metrics_are_accurate(self, memoir_with_graph):
        """
    ...
    @pytest.mark.asyncio
    async def test_progressive_storage_creates_partial_memory(self, memoir_with_graph):
        """
    ...
    @pytest.mark.asyncio
    async def test_streaming_hooks_are_called(self, memoir_with_graph):
        """
    ...
    @pytest.mark.asyncio
    async def test_performance_insights_detect_bottlenecks(self, memoir_with_graph):
        """
...
class TestGraphSyncValidation:
    ...
    @pytest.mark.asyncio
    async def test_progressive_graph_sync_creates_and_updates_node(self, memoir_with_graph):
        """
...
class TestErrorRecoveryValidation:
    ...
    @pytest.mark.asyncio
    async def test_failed_stream_rolls_back_cleanly(self, memoir_with_graph):
        """
        ...
        with pytest.raises(Exception):  # Don't match message - error format may vary
            await memoir_with_graph.memory.remember_stream(
                {
                    "memorySpaceId": memory_space_id,
                    "conversationId": conversation_id,
        ...
        memories = await memoir_with_graph.vector.list(memory_space_id, limit=100)
    ...
    @pytest.mark.asyncio
    async def test_resume_token_generation(self, memoir_with_graph):
        """
...
class TestMultiDatabaseValidation:
    ...
    @pytest.mark.asyncio
    async def test_data_consistency_across_convex_and_graph(self, memoir_with_graph):
        """
        ...
        convex_memory = await memoir_with_graph.vector.get(memory_space_id, memory_id)
