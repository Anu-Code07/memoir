...
from memoir.types import ListFactsFilter, RecallParams, RememberOptions, RememberParams
...
@pytest.fixture
async def memoir_client(test_context: Dict[str, str]) -> AsyncGenerator[Any, None]:
    """Create a Memoir client for testing."""
    from memoir import Memoir
    from memoir.types import MemoirConfig, LLMConfig, RegisterMemorySpaceParams, RememberParams, RememberOptions
    ...
    config = MemoirConfig(convex_url=convex_url)
    ...
    memoir = Memoir(config)

    # Register memory space for tests
    await memoir.memory_spaces.register(
...
class TestSubjectFactTypeMatching:
    ...
    def test_pipeline_result_has_subject_type_matching(self) -> None:
        ...
        from memoir.facts.belief_revision import PipelineStageResult, ReviseResult
...
class TestBatteriesIncludedMode:
    ...
    async def test_revise_available_without_llm(self) -> None:
        ...
        try:
            ...
            assert memoir.facts.has_belief_revision() is True
        finally:
            await memoir.close()
...
class TestChatbotBeliefSystemE2E:
    ...
    @pytest.fixture
    async def e2e_memoir(self) -> AsyncGenerator[Dict[str, Any], None]:
        """Create Memoir client with LLM for E2E tests."""
        from memoir import Memoir
        from memoir.types import MemoirConfig, LLMConfig, RegisterMemorySpaceParams, RememberParams, RememberOptions
    ...
    async def test_turn1_extracts_name_and_color_facts(
        self, e2e_memoir: Dict[str, Any]
    ) -> None:
        """Turn 1: User states name and color -> 2 facts ADDed."""
        memoir = e2e_memoir["memoir"]
        memory_space_id = e2e_memoir["memory_space_id"]
        user_id = e2e_memoir["user_id"]
        agent_id = e2e_memoir["agent_id"]

        # Simulate user message
        user_message = "My name is Nicholas and my favorite color is blue"
        conversation_id = e2e_memoir["conversation_id"]
        ...
        stored_facts = await memoir.facts.list(
            ListFactsFilter(
                memory_space_id=memory_space_id,
                user_id=user_id,
    ...
    async def test_turn2_recall_does_not_create_duplicates(
        ...
        facts_before = await memoir.facts.list(
            ListFactsFilter(
                memory_space_id=memory_space_id,
                user_id=user_id,
                include_superseded=False,
        ...
        recall_result = await memoir.memory.recall(
            RecallParams(
                memory_space_id=memory_space_id,
        ...
        facts_after = await memoir.facts.list(
            ListFactsFilter(
                memory_space_id=memory_space_id,
                user_id=user_id,
                include_superseded=False,
    ...
    async def test_turn3_supersedes_color_preference(
        ...
        active_facts = await memoir.facts.list(
            ListFactsFilter(
                memory_space_id=memory_space_id,
                user_id=user_id,
                include_superseded=False,
        ...
        all_facts = await memoir.facts.list(
            ListFactsFilter(
                memory_space_id=memory_space_id,
                user_id=user_id,
    ...
    async def test_full_conversation_flow(self, e2e_memoir: Dict[str, Any]) -> None:
        ...
        turn1_result = await memoir.memory.remember(
            RememberParams(
                memory_space_id=memory_space_id,
        ...
        facts_after_turn1 = await memoir.facts.list(
            ListFactsFilter(
                memory_space_id=memory_space_id,
                user_id=user_id,
                include_superseded=False,
        ...
        recall = await memoir.memory.recall(
            RecallParams(
                memory_space_id=memory_space_id,
                query="What is my favorite color?",
                user_id=user_id,
        ...
        facts_after_turn2 = await memoir.facts.list(
            ListFactsFilter(
                memory_space_id=memory_space_id,
                user_id=user_id,
                include_superseded=False,
        ...
        turn3_result = await memoir.memory.remember(
            RememberParams(
                memory_space_id=memory_space_id,
