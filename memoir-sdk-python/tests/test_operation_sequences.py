...
class TestVectorMemorySequences:
    ...
    async def test_full_crud_sequence(self, memoir_client):
        ...
        after_create = await memoir_client.vector.get(space_id, created.memory_id)
        ...
        after_update = await memoir_client.vector.get(space_id, created.memory_id)
        assert after_update.content == "Updated content"
        assert after_update.version == 2

        # STEP 5: Delete
        await memoir_client.vector.delete(space_id, created.memory_id)

        # STEP 6: Get (validate delete)
        after_delete = await memoir_client.vector.get(space_id, created.memory_id)
        assert after_delete is None

    async def test_list_reflects_each_operation(self, memoir_client):
        """Test list reflects state after each operation."""
        space_id = generate_test_id("seq-vector-list-")

        # Initial list
        list0 = await memoir_client.vector.list(memory_space_id=space_id)
        count0 = len(list0)

        # Create
        mem1 = await memoir_client.vector.store(
            space_id,
            StoreMemoryInput(
                content="Memory 1",
                content_type="raw",
        ...
        list1 = await memoir_client.vector.list(memory_space_id=space_id)
        ...
        list2 = await memoir_client.vector.list(memory_space_id=space_id)
        assert len(list2) == count0 + 2

        # Update first
        await memoir_client.vector.update(space_id, mem1.memory_id, updates={"content": "Updated 1"})

        list3 = await memoir_client.vector.list(memory_space_id=space_id)
        ...
        # Delete first
        await memoir_client.vector.delete(space_id, mem1.memory_id)

        list4 = await memoir_client.vector.list(memory_space_id=space_id)
        assert len(list4) == count0 + 1

    async def test_count_matches_list_length(self, memoir_client):
        """Test count matches list.length after each operation."""
        space_id = generate_test_id("seq-vector-count-")

        # Initial count
        count0 = await memoir_client.vector.count(memory_space_id=space_id)
        list0 = await memoir_client.vector.list(memory_space_id=space_id)
        ...
        count1 = await memoir_client.vector.count(memory_space_id=space_id)
        list1 = await memoir_client.vector.list(memory_space_id=space_id)
        assert count1 == len(list1)
        assert count1 == count0 + 1

        # Update doesn't change count
        await memoir_client.vector.update(space_id, mem.memory_id, updates={"content": "Updated"})

        count2 = await memoir_client.vector.count(memory_space_id=space_id)
        assert count2 == count1

        # Delete
        await memoir_client.vector.delete(space_id, mem.memory_id)

        count3 = await memoir_client.vector.count(memory_space_id=space_id)
...
class TestConversationSequences:
    ...
    async def test_conversation_full_sequence(self, memoir_client):
        ...
        # STEP 2: Get (validate create)
        after_create = await memoir_client.conversations.get(created.conversation_id)
        ...
        after_message = await memoir_client.conversations.get(created.conversation_id)
        ...
        # STEP 6: Get (validate delete)
        after_delete = await memoir_client.conversations.get(created.conversation_id)
        assert after_delete is None

    async def test_count_reflects_message_additions(self, memoir_client):
...
class TestFactsSequences:
    ...
    async def test_version_chain_sequence(self, memoir_client):
        """Test store→update→update→delete validates versions."""
        space_id = generate_test_id("seq-facts-")

        # STEP 1: Store v1
        v1 = await memoir_client.facts.store(
            StoreFactParams(
                memory_space_id=space_id,
                fact="Version 1",
                fact_type="knowledge",
        ...
        # STEP 2: Update to v2
        v2 = await memoir_client.facts.update(
        ...
        v1_after = await memoir_client.facts.get(space_id, v1.fact_id)
        assert v1_after.superseded_by is not None

        # STEP 4: Update to v3
        v3 = await memoir_client.facts.update(space_id, v2.fact_id, updates={"confidence": 90})

        assert v3.version == 3

        # STEP 5: Get v2 (should be superseded)
        v2_after = await memoir_client.facts.get(space_id, v2.fact_id)
        assert v2_after.superseded_by is not None

    async def test_list_excludes_superseded(self, memoir_client):
        ...
        list1 = await memoir_client.facts.list(ListFactsFilter(memory_space_id=space_id))
        assert any(f.fact_id == v1.fact_id for f in list1)

        # Update (creates v2, supersedes v1)
        v2 = await memoir_client.facts.update(space_id, v1.fact_id, updates={"fact": "Updated"})

        list2 = await memoir_client.facts.list(ListFactsFilter(memory_space_id=space_id))
...
class TestContextSequences:
    ...
    async def test_context_full_lifecycle(self, memoir_client, ctx):
        ...
        # STEP 2: Get (validate create)
        after_create = await memoir_client.contexts.get(created.id)
        ...
        # STEP 6: Get (validate delete)
        after_delete = await memoir_client.contexts.get(created.id)
        assert after_delete is None
...
class TestImmutableSequences:
    ...
    async def test_version_sequence(self, memoir_client):
        """Test store→getVersion→getHistory validates versioning."""
        import time
        imm_id = f"immutable-seq-{int(time.time() * 1000)}"

        # Store v1
        from memoir.types import ImmutableEntry
        v1 = await memoir_client.immutable.store(
        ...
        v1_retrieved = await memoir_client.immutable.get_version(
        ...
        # Store v2
        v2 = await memoir_client.immutable.store(
        ...
        # Get latest
        latest = await memoir_client.immutable.get(type="document", id=imm_id)
        assert latest.version == 2
        assert latest.data.get("title") == "V2"

        # Get history
        history = await memoir_client.immutable.get_history(type="document", id=imm_id)
...
class TestMutableSequences:
    ...
    async def test_mutable_full_sequence(self, memoir_client):
        ...
        # STEP 1: Set
        await memoir_client.mutable.set(ns, key, {"count": 0, "status": "initial"})

        # STEP 2: Get (validate set)
        after_set = await memoir_client.mutable.get(ns, key)
        ...
        # STEP 4: Get (validate update)
        after_update = await memoir_client.mutable.get(ns, key)
        # Mutable returns record
        assert after_update is not None

        # STEP 5: Delete
        await memoir_client.mutable.delete(ns, key)

        # STEP 6: Get (validate delete)
        after_delete = await memoir_client.mutable.get(ns, key)
        assert after_delete is None
...
class TestMemorySpaceSequences:
    ...
    async def test_memory_space_lifecycle(self, memoir_client):
        ...
        # STEP 4: Delete
        await memoir_client.memory_spaces.delete(space_id, cascade=True)

        after_delete = await memoir_client.memory_spaces.get(space_id)
        assert after_delete is None
...
class TestUserProfileSequences:
    ...
    async def test_user_profile_evolution(self, memoir_client):
        ...
        # STEP 2: Get
        after_create = await memoir_client.users.get(user_id)
        ...
        # STEP 4: Delete
        await memoir_client.users.delete(user_id, DeleteUserOptions(cascade=True))

        # STEP 5: Get (validate delete)
        after_delete = await memoir_client.users.get(user_id)
        assert after_delete is None
...
class TestConcurrentOperationSequences:
    ...
    async def test_parallel_creates(self, memoir_client, ctx):
        ...
        # All retrievable
        for mem in results:
            retrieved = await memoir_client.vector.get(space_id, mem.memory_id)
            assert retrieved is not None

    async def test_parallel_updates_no_interference(self, memoir_client):
...
class TestCrossLayerSequences:
    ...
    async def test_conversation_to_context_chain(self, memoir_client):
        ...
        conv_check = await memoir_client.conversations.get(conv.conversation_id)
        mem_check = await memoir_client.vector.get(space_id, mem.memory_id)
        fact_check = await memoir_client.facts.get(space_id, fact.fact_id)
        ctx_check = await memoir_client.contexts.get(ctx.id)

        assert all([conv_check, mem_check, fact_check, ctx_check])

    async def test_remember_forget_cleanup(self, memoir_client):
        ...
        # Get memories
        mem1 = await memoir_client.vector.get(space_id, result.memories[0].memory_id)
        assert mem1 is not None

        # Forget first memory
        await memoir_client.memory.forget(space_id, result.memories[0].memory_id)

        after_forget = await memoir_client.vector.get(
...
class TestBulkOperationSequences:
    ...
    async def test_delete_many_complete_removal(self, memoir_client):
        ...
        list_before = await memoir_client.vector.list(memory_space_id=space_id)
        ...
        # Verify count after
        # Count doesn't support tags filter
        list_after = await memoir_client.vector.list(memory_space_id=space_id)
...
class TestErrorRecoverySequences:
    ...
    async def test_failed_update_preserves_state(self, memoir_client):
        ...
        after = await memoir_client.vector.get(space_id, mem.memory_id)
...
class TestComplexWorkflows:
    ...
    async def test_complete_user_journey(self, memoir_client):
        ...
        # Remember
        remembered = await memoir_client.memory.remember(
            RememberParams(
                memory_space_id=space_id,
                conversation_id=conv.conversation_id,
        ...
        user_check = await memoir_client.users.get(user_id)
        conv_check = await memoir_client.conversations.get(conv.conversation_id)
        mem_check = await memoir_client.vector.get(
    ...
    async def test_cascade_delete_complete_cleanup(self, memoir_client):
