...
from memoir import RememberParams, SearchOptions
...
async def test_openai_stores_multiple_facts_with_real_embeddings(
    memoir_client, test_memory_space_id, test_user_id, test_agent_id, test_conversation_id, cleanup_helper
):
    """
...
async def test_openai_recalls_facts_using_semantic_search(
    ...
    for search in searches:
        ...
        results = await retry_async(
            lambda q=search["query"], emb=query_embedding: memoir_client.memory.search(
...
async def test_openai_enriches_search_results_with_conversation_context(
    ...
    # Wrap search with retry logic to handle server errors during parallel execution
    results = await retry_async(
        lambda emb=password_embedding: memoir_client.memory.search(
            test_memory_space_id,
            "password",
            SearchOptions(
                embedding=emb,
...
async def test_openai_validates_summarization_quality(
    memoir_client, test_memory_space_id, test_user_id, test_conversation_id, cleanup_helper
):
    """
...
async def test_openai_similarity_scores_are_realistic(
    ...
    results = await retry_async(
        lambda emb=query_embedding: memoir_client.memory.search(
            test_memory_space_id,
