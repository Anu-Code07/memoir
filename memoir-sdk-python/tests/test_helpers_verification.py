...
@pytest.mark.asyncio
async def test_cleanup_conversations(memoir_client, test_ids):
    ...
    # Verify it exists
    result = await memoir_client.conversations.get(conv_id)
...
@pytest.mark.asyncio
async def test_cleanup_memories(memoir_client, test_ids):
    ...
    # Verify it exists
    result = await memoir_client.vector.get(memory_space_id, mem_id)
    assert result is not None
...
@pytest.mark.asyncio
async def test_cleanup_facts(memoir_client, test_ids):
...
@pytest.mark.asyncio
async def test_cleanup_users(memoir_client, cleanup_helper):
    """Test cleanup helper can purge users."""
    user_id = generate_test_user_id()

    # Create test user
    await memoir_client.users.update(user_id, {"name": "Test User for Cleanup"})
...
@pytest.mark.asyncio
async def test_cleanup_memory_space(memoir_client, test_ids):
    ...
    await memoir_client.conversations.create(conv_input)

    # Create test memory (already returns StoreMemoryInput)
    memory_input = create_test_memory_input()
    await memoir_client.vector.store(memory_space_id, memory_input)
...
@pytest.mark.asyncio
async def test_cleanup_all(memoir_client, test_ids, cleanup_helper):
    ...
    user_id = generate_test_user_id()
    await memoir_client.users.update(user_id, {"name": "Test User"})
...
@pytest.mark.asyncio
async def test_validate_conversation_storage(memoir_client, test_ids):
    ...
    # Validate storage
    validation = await validate_conversation_storage(memoir_client, conv_id)
...
@pytest.mark.asyncio
async def test_validate_memory_storage(memoir_client, test_ids):
    ...
    # Validate storage
    validation = await validate_memory_storage(memoir_client, memory_space_id, mem_id)
    ...
    # Cleanup
    await memoir_client.vector.delete(memory_space_id, mem_id)


@pytest.mark.asyncio
async def test_validate_user_storage(memoir_client):
    """Test storage validation for users."""
    user_id = generate_test_user_id()

    # Create user
    await memoir_client.users.update(user_id, {"name": "Storage Test User"})

    # Validate storage
    validation = await validate_user_storage(memoir_client, user_id)
...
@pytest.mark.asyncio
async def test_validate_storage_with_expected_data(memoir_client, test_ids):
    ...
    validation = await validate_conversation_storage(
        memoir_client, conv_id, expected
    )
...
@pytest.mark.asyncio
async def test_direct_convex_client_access(memoir_client, direct_convex_client, test_ids):
...
@pytest.mark.asyncio
async def test_all_helpers_summary(memoir_client, cleanup_helper, test_ids, embeddings_available_fixture):
