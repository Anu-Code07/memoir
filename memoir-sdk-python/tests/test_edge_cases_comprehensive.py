...
@pytest.mark.asyncio
async def test_handles_very_long_content_10kb_plus(memoir_client, ctx):
    ...
    memory = await memoir_client.vector.store(
        memory_space_id,
        StoreMemoryInput(
    ...
    stored = await memoir_client.vector.get(memory_space_id, memory.memory_id)
...
@pytest.mark.asyncio
async def test_handles_very_long_fact_statements_10kb_plus(memoir_client, ctx):
...
@pytest.mark.asyncio
async def test_handles_long_array_of_tags_100_plus_tags(memoir_client, ctx):
...
@pytest.mark.asyncio
async def test_handles_very_long_participant_id(memoir_client, ctx):
...
@pytest.mark.asyncio
async def test_handles_unicode_emoji_in_content(memoir_client, ctx):
...
@pytest.mark.asyncio
async def test_handles_special_characters_in_ids(memoir_client, ctx):
    ...
    immutable = await memoir_client.immutable.store(
        ImmutableEntry(
            type=immutable_type,
            id=special_id,
            data={"value": "test"},
    ...
    retrieved = await memoir_client.immutable.get(immutable_type, special_id)
    assert retrieved.id == special_id


@pytest.mark.asyncio
async def test_handles_newlines_and_tabs_in_content(memoir_client, ctx):
...
@pytest.mark.asyncio
async def test_handles_json_special_characters(memoir_client, ctx):
...
@pytest.mark.asyncio
async def test_handles_empty_tags_array(memoir_client, ctx):
...
@pytest.mark.asyncio
async def test_handles_minimal_metadata(memoir_client, ctx):
...
@pytest.mark.asyncio
async def test_handles_empty_data_object(memoir_client, ctx):
    ...
    retrieved = await memoir_client.immutable.get(ctx.immutable_type("test-empty"), ctx.immutable_id("empty-data"))
...
@pytest.mark.asyncio
async def test_handles_importance_zero(memoir_client, ctx):
...
@pytest.mark.asyncio
async def test_handles_importance_100(memoir_client, ctx):
...
@pytest.mark.asyncio
async def test_handles_confidence_zero(memoir_client, ctx):
...
@pytest.mark.asyncio
async def test_handles_confidence_100(memoir_client, ctx):
...
@pytest.mark.asyncio
async def test_handles_concurrent_memory_creation(memoir_client, ctx):
...
@pytest.mark.asyncio
async def test_handles_concurrent_fact_updates(memoir_client, ctx):
...
@pytest.mark.asyncio
async def test_handles_deeply_nested_data_objects(memoir_client, ctx):
    ...
    retrieved = await memoir_client.immutable.get(ctx.immutable_type("nested-test"), ctx.immutable_id("deep-nest"))
    assert retrieved.data["level1"]["level2"]["level3"]["level4"]["level5"]["value"] == "deep nesting works"


@pytest.mark.asyncio
async def test_handles_arrays_in_data(memoir_client, ctx):
    ...
    retrieved = await memoir_client.immutable.get(ctx.immutable_type("array-test"), ctx.immutable_id("arrays"))
...
@pytest.mark.asyncio
async def test_handles_uuid_format_ids(memoir_client, ctx):
    ...
    retrieved = await memoir_client.immutable.get(immutable_type, uuid_id)
    assert retrieved.id == uuid_id


@pytest.mark.asyncio
async def test_handles_numeric_string_ids(memoir_client, ctx):
...
@pytest.mark.asyncio
async def test_handles_none_optional_fields(memoir_client, ctx):
...
@pytest.mark.asyncio
async def test_handles_very_old_timestamps(memoir_client, ctx):
...
@pytest.mark.asyncio
async def test_handles_future_timestamps(memoir_client, ctx):
...
@pytest.mark.asyncio
async def test_handles_all_content_types(memoir_client, ctx):
...
@pytest.mark.asyncio
async def test_handles_all_source_types(memoir_client, ctx):
...
@pytest.mark.asyncio
async def test_handles_bulk_memory_creation_50_plus(memoir_client, ctx):
    ...
    for mem in memories[:5]:  # Check first 5
        stored = await memoir_client.vector.get(memory_space_id, mem.memory_id)
        assert stored is not None


@pytest.mark.asyncio
async def test_handles_bulk_fact_creation_50_plus(memoir_client, ctx):
...
@pytest.mark.asyncio
async def test_handles_very_large_metadata_object(memoir_client, ctx):
    ...
    retrieved = await memoir_client.immutable.get(ctx.immutable_type("large-meta"), ctx.immutable_id("large-metadata"))
...
@pytest.mark.asyncio
async def test_handles_nonexistent_ids_gracefully(memoir_client, ctx):
    """Test handling nonexistent IDs gracefully. Port of: edgeCases.test.ts - line 708"""
    memory_space_id = ctx.memory_space_id()
    # Get nonexistent memory (use valid ID format)
    memory = await memoir_client.vector.get(memory_space_id, "mem-nonexistent-12345")
    assert memory is None

    # Get nonexistent fact (use valid ID format: fact-*)
    fact = await memoir_client.facts.get(memory_space_id, "fact-nonexistent-12345")
    assert fact is None

    # Get nonexistent conversation (use valid ID format: conv-*)
    conv = await memoir_client.conversations.get("conv-nonexistent-12345")
    assert conv is None
...
@pytest.mark.asyncio
async def test_handles_rapid_version_updates_10_plus(memoir_client, ctx):
    """Test handling rapid version updates (10+). Port of: edgeCases.test.ts - line 734"""
    ctx.memory_space_id()
    # Create initial
    v = await memoir_client.immutable.store(
...
@pytest.mark.asyncio
async def test_handles_sql_injection_attempt_in_content(memoir_client, ctx):
...
@pytest.mark.asyncio
async def test_handles_xss_attempt_in_content(memoir_client, ctx):
...
@pytest.mark.asyncio
async def test_handles_single_character_content(memoir_client, ctx):
...
@pytest.mark.asyncio
async def test_handles_content_with_only_spaces(memoir_client, ctx):
    """Test handling content with only spaces. Port of: edgeCases.test.ts - line 832"""
    memory_space_id = ctx.memory_space_id()
    from memoir.vector.validators import VectorValidationError
...
@pytest.mark.asyncio
async def test_handles_mixed_language_content(memoir_client, ctx):
...
@pytest.mark.asyncio
async def test_handles_conversation_with_100_plus_messages(memoir_client, ctx):
    """Test handling conversation with 100+ messages. Port of: edgeCases.test.ts - line 876"""
    memory_space_id = ctx.memory_space_id()
    user_id = ctx.user_id()
    from memoir.types import AddMessageInput
...
@pytest.mark.asyncio
async def test_handles_deep_context_chain_10_plus_levels(memoir_client, ctx):
    ...
    # Root
    root = await memoir_client.contexts.create(
        ContextInput(
            purpose="Root",
            memory_space_id=memory_space_id,
        )
    ...
    retrieved_root = await memoir_client.contexts.get_root(deepest.id)
...
@pytest.mark.asyncio
async def test_handles_boolean_values_in_data(memoir_client, ctx):
    ...
    retrieved = await memoir_client.immutable.get(ctx.immutable_type("boolean-test"), ctx.immutable_id("booleans"))
...
@pytest.mark.asyncio
async def test_handles_numeric_values_in_data(memoir_client, ctx):
    ...
    retrieved = await memoir_client.immutable.get(ctx.immutable_type("numeric-test"), ctx.immutable_id("numbers"))
...
@pytest.mark.asyncio
async def test_handles_tags_with_special_characters(memoir_client, ctx):
...
@pytest.mark.asyncio
async def test_handles_duplicate_tags(memoir_client, ctx):
