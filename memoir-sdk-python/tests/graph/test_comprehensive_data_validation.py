...
Validates nodes, edges, properties, and relationships for all Memoir APIs.
...
from memoir.types import GraphConnectionConfig, RememberParams
...
@pytest.fixture
async def memoir_client(neo4j_adapter):
    """Memoir client with Neo4j graph support"""
...
@pytest.fixture
async def memoir_client_memgraph(memgraph_adapter):
    """Memoir client with Memgraph graph support"""
    from memoir import MemoirConfig, GraphConfig
    client = Memoir(
        MemoirConfig(
            convex_url=CONVEX_URL,
...
class TestGraphDataValidation:
    ...
    @pytest.mark.asyncio
    async def test_neo4j_agent_registration_creates_node(self, neo4j_adapter, memoir_client):
        """
        ...
        from memoir import AgentRegistration
    ...
    @pytest.mark.asyncio
    async def test_memgraph_agent_registration_creates_node(self, memgraph_adapter, memoir_client_memgraph):
        """
        ...
        from memoir import AgentRegistration
        await memoir_client_memgraph.agents.register(
            AgentRegistration(
                id=agent_id,
    ...
    @pytest.mark.asyncio
    async def test_memory_creates_node_and_edges(self, neo4j_adapter, memoir_client):
        """
        ...
        from memoir import RememberOptions
    ...
    @pytest.mark.asyncio
    async def test_traverse_returns_actual_connected_nodes(self, neo4j_adapter, memoir_client):
        """
        Test: Traverse returns actual connected nodes
        Validates: Multi-hop traversal with actual relationship verification
        """
        from memoir.types import TraversalConfig
    ...
    @pytest.mark.asyncio
    async def test_fact_creates_node_with_properties(self, neo4j_adapter, memoir_client):
        """
        ...
        # Store fact
        from memoir.types import StoreFactOptions
