...
from memoir import ForgetOptions, RememberParams
...
@pytest.mark.asyncio
async def test_remember_creates_conversation_and_memories(memoir_client, test_ids, cleanup_helper):
    """
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_forget_deletes_both_layers(memoir_client, test_ids, cleanup_helper):
    """
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_cross_layer_reference_integrity(memoir_client, test_ids, cleanup_helper):
    """
    ...
    # Add messages
    from memoir import AddMessageInput
    ...
    from memoir import ConversationRef, MemoryMetadata, MemorySource, StoreMemoryInput
    ...
    conv_check = await memoir_client.conversations.get(conversation_id)
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_memory_facts_linkage(memoir_client, test_ids, cleanup_helper):
    """
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_user_cascade_affects_all_layers(memoir_client, test_ids, cleanup_helper):
    """
    ...
    mem_count_before = await memoir_client.vector.count(
        memory_space_id,
        user_id=user_id,
    )
    ...
    mem_count_after = await memoir_client.vector.count(
        memory_space_id,
        user_id=user_id,
    )
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_memory_space_isolation(memoir_client, test_ids, cleanup_helper):
    """
    ...
    list1 = await memoir_client.vector.list(space_1, limit=100)
    ...
    list2 = await memoir_client.vector.list(space_2, limit=100)
