...
from memoir.auth import create_auth_context
from memoir.immutable import ImmutableValidationError
...
@pytest.mark.asyncio
async def test_store_creates_version_1(memoir_client):
    """
    ...
    result = await memoir_client.immutable.store(
        ImmutableEntry(
    ...
    # Cleanup
    await memoir_client.immutable.purge("test-kb-article", unique_id)


@pytest.mark.asyncio
async def test_store_creates_version_2_on_update(memoir_client):
    """
    ...
    # Cleanup
    await memoir_client.immutable.purge("test-article", unique_id)
...
@pytest.mark.asyncio
async def test_get_latest_version(memoir_client, ctx):
    """
    ...
    # Create entry
    await memoir_client.immutable.store(
        ImmutableEntry(
            type=test_type,
            id=test_id,
            data={"theme": "dark"},
    ...
    # Get latest version
    result = await memoir_client.immutable.get(test_type, test_id)
    ...
    # Cleanup
    await memoir_client.immutable.purge(test_type, test_id)


@pytest.mark.asyncio
async def test_get_nonexistent_returns_none(memoir_client, ctx):
    """
    ...
    result = await memoir_client.immutable.get(test_type, "does-not-exist-12345")
...
@pytest.mark.asyncio
async def test_get_specific_version(memoir_client, ctx):
    """
    ...
    # Get version 1
    retrieved_v1 = await memoir_client.immutable.get_version(test_type, test_id, 1)
    ...
    # Get version 2
    retrieved_v2 = await memoir_client.immutable.get_version(test_type, test_id, 2)
...
@pytest.mark.asyncio
async def test_get_version_history(memoir_client, ctx):
    """
    ...
    # Get history
    history = await memoir_client.immutable.get_history(test_type, test_id)
...
@pytest.mark.asyncio
async def test_list_entries(memoir_client, ctx):
    """
    ...
    result = await memoir_client.immutable.list(
    ...
    for entry_id in created_ids:
        await memoir_client.immutable.purge(test_type, entry_id)
...
@pytest.mark.asyncio
async def test_count_entries(memoir_client, ctx):
    """
    ...
    # Count entries
    count = await memoir_client.immutable.count()
...
@pytest.mark.asyncio
async def test_purge_entry(memoir_client, ctx):
    """
    ...
    # Purge entry
    await memoir_client.immutable.purge(immutable_type, immutable_id)

    # Verify purged
    retrieved = await memoir_client.immutable.get(immutable_type, immutable_id)
...
@pytest.mark.asyncio
async def test_search_immutable(memoir_client, ctx):
    """
    ...
    results = await memoir_client.immutable.search(
...
@pytest.mark.asyncio
async def test_store_validation_missing_type(memoir_client):
...
@pytest.mark.asyncio
async def test_store_validation_empty_type(memoir_client):
...
@pytest.mark.asyncio
async def test_store_validation_type_invalid_chars(memoir_client):
...
@pytest.mark.asyncio
async def test_store_validation_missing_id(memoir_client):
...
@pytest.mark.asyncio
async def test_store_validation_empty_id(memoir_client):
...
@pytest.mark.asyncio
async def test_store_validation_missing_data(memoir_client):
...
@pytest.mark.asyncio
async def test_store_validation_invalid_data_type(memoir_client):
...
@pytest.mark.asyncio
async def test_store_validation_invalid_metadata(memoir_client):
...
@pytest.mark.asyncio
async def test_store_validation_empty_user_id(memoir_client):
...
@pytest.mark.asyncio
async def test_get_validation_empty_type(memoir_client):
    """Should throw on empty type."""
    with pytest.raises(ImmutableValidationError) as exc_info:
        await memoir_client.immutable.get("", "test-id")
    assert "Type must be a non-empty string" in str(exc_info.value)


@pytest.mark.asyncio
async def test_get_validation_empty_id(memoir_client):
    """Should throw on empty id."""
    with pytest.raises(ImmutableValidationError) as exc_info:
        await memoir_client.immutable.get("test-type", "")
    assert "ID must be a non-empty string" in str(exc_info.value)


# getVersion() validation tests
@pytest.mark.asyncio
async def test_get_version_validation_zero(memoir_client):
    """Should throw on version 0."""
    with pytest.raises(ImmutableValidationError) as exc_info:
        await memoir_client.immutable.get_version("test", "id", 0)
    assert "Version must be a positive integer >= 1" in str(exc_info.value)


@pytest.mark.asyncio
async def test_get_version_validation_negative(memoir_client):
    """Should throw on negative version."""
    with pytest.raises(ImmutableValidationError) as exc_info:
        await memoir_client.immutable.get_version("test", "id", -1)
    assert "Version must be a positive integer >= 1" in str(exc_info.value)


# getAtTimestamp() validation tests
@pytest.mark.asyncio
async def test_get_at_timestamp_validation_negative(memoir_client):
    """Should throw on negative timestamp."""
    with pytest.raises(ImmutableValidationError) as exc_info:
        await memoir_client.immutable.get_at_timestamp("test", "id", -1000)
    assert "Timestamp must be a positive integer" in str(exc_info.value)


# search() validation tests
@pytest.mark.asyncio
async def test_search_validation_empty_query(memoir_client):
    """Should throw on empty query."""
    with pytest.raises(ImmutableValidationError) as exc_info:
        await memoir_client.immutable.search(SearchImmutableInput(query=""))
    assert "Search query is required" in str(exc_info.value)


@pytest.mark.asyncio
async def test_search_validation_invalid_type_filter(memoir_client):
    """Should throw on invalid type filter."""
    with pytest.raises(ImmutableValidationError) as exc_info:
        await memoir_client.immutable.search(SearchImmutableInput(query="test", type=""))
    assert "Type must be a non-empty string" in str(exc_info.value)


# list() validation tests
@pytest.mark.asyncio
async def test_list_validation_invalid_limit_zero(memoir_client):
    """Should throw on limit = 0."""
    with pytest.raises(ImmutableValidationError) as exc_info:
        await memoir_client.immutable.list(ListImmutableFilter(limit=0))
    assert "Limit must be a positive integer" in str(exc_info.value)


@pytest.mark.asyncio
async def test_list_validation_invalid_limit_negative(memoir_client):
    """Should throw on negative limit."""
    with pytest.raises(ImmutableValidationError) as exc_info:
        await memoir_client.immutable.list(ListImmutableFilter(limit=-5))
    assert "Limit must be a positive integer" in str(exc_info.value)


# count() validation tests
@pytest.mark.asyncio
async def test_count_validation_empty_type(memoir_client):
    """Should throw on empty type."""
    with pytest.raises(ImmutableValidationError) as exc_info:
        await memoir_client.immutable.count(CountImmutableFilter(type=""))
    assert "Type must be a non-empty string" in str(exc_info.value)


# purge() validation tests
@pytest.mark.asyncio
async def test_purge_validation_empty_type(memoir_client):
    """Should throw on empty type."""
    with pytest.raises(ImmutableValidationError) as exc_info:
        await memoir_client.immutable.purge("", "test-id")
    assert "Type must be a non-empty string" in str(exc_info.value)


@pytest.mark.asyncio
async def test_purge_validation_empty_id(memoir_client):
    """Should throw on empty id."""
    with pytest.raises(ImmutableValidationError) as exc_info:
        await memoir_client.immutable.purge("test-type", "")
    assert "ID must be a non-empty string" in str(exc_info.value)


# purgeMany() validation tests
@pytest.mark.asyncio
async def test_purge_many_validation_no_filters(memoir_client):
    """Should throw when no filters provided."""
    with pytest.raises(ImmutableValidationError) as exc_info:
        await memoir_client.immutable.purge_many(PurgeManyFilter())
    assert "requires at least one filter" in str(exc_info.value)


@pytest.mark.asyncio
async def test_purge_many_validation_empty_type(memoir_client):
    """Should throw on empty type in filter."""
    with pytest.raises(ImmutableValidationError) as exc_info:
        await memoir_client.immutable.purge_many(PurgeManyFilter(type=""))
    assert "Type must be a non-empty string" in str(exc_info.value)


# purgeVersions() validation tests
@pytest.mark.asyncio
async def test_purge_versions_validation_keep_latest_zero(memoir_client):
    """Should throw on keep_latest = 0."""
    with pytest.raises(ImmutableValidationError) as exc_info:
        await memoir_client.immutable.purge_versions("test", "id", keep_latest=0)
    assert "keep_latest must be a positive integer >= 1" in str(exc_info.value)


@pytest.mark.asyncio
async def test_purge_versions_validation_keep_latest_negative(memoir_client):
    """Should throw on negative keep_latest."""
    with pytest.raises(ImmutableValidationError) as exc_info:
        await memoir_client.immutable.purge_versions("test", "id", keep_latest=-5)
    assert "keep_latest must be a positive integer >= 1" in str(exc_info.value)


# Validation error properties tests
@pytest.mark.asyncio
async def test_validation_error_has_code(memoir_client):
...
@pytest.mark.asyncio
async def test_validation_error_has_field(memoir_client):
...
@pytest.mark.asyncio
async def test_validation_error_is_exception(memoir_client):
...
@pytest.mark.asyncio
async def test_get_version_returns_expanded_type(memoir_client, ctx):
    """
    ...
    version = await memoir_client.immutable.get_version(test_type, test_id, 1)
...
@pytest.mark.asyncio
async def test_get_history_returns_expanded_types(memoir_client, ctx):
    """
...
@pytest.mark.asyncio
async def test_get_at_timestamp_with_datetime(memoir_client, ctx):
    """
    ...
    version = await memoir_client.immutable.get_at_timestamp(
...
@pytest.mark.asyncio
async def test_purge_returns_typed_result(memoir_client, ctx):
    """
    ...
    result = await memoir_client.immutable.purge(test_type, test_id)
...
@pytest.mark.asyncio
async def test_purge_versions_returns_typed_result(memoir_client, ctx):
    """
    ...
    result = await memoir_client.immutable.purge_versions(test_type, test_id, keep_latest=2)
...
@pytest.mark.asyncio
async def test_search_returns_typed_results(memoir_client, ctx):
    """
...
@pytest.mark.asyncio
async def test_list_with_filter_object(memoir_client, ctx):
    """
    ...
    results = await memoir_client.immutable.list(
...
@pytest.mark.asyncio
async def test_count_with_filter_object(memoir_client, ctx):
    """
...
@pytest.mark.asyncio
async def test_purge_many_with_filter_object(memoir_client, ctx):
    """
    ...
    result = await memoir_client.immutable.purge_many(
