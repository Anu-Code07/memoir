...
@pytest.mark.asyncio
async def test_register_missing_agent_id(memoir_client):
    """Should throw on missing agent ID."""
    with pytest.raises(Exception) as exc_info:
        await memoir_client.agents.register(
...
@pytest.mark.asyncio
async def test_register_empty_agent_id(memoir_client):
...
@pytest.mark.asyncio
async def test_register_whitespace_agent_id(memoir_client):
...
@pytest.mark.asyncio
async def test_register_agent_id_too_long(memoir_client):
...
@pytest.mark.asyncio
async def test_register_missing_agent_name(memoir_client):
...
@pytest.mark.asyncio
async def test_register_empty_agent_name(memoir_client):
...
@pytest.mark.asyncio
async def test_register_agent_name_too_long(memoir_client):
...
@pytest.mark.asyncio
async def test_register_invalid_metadata_format(memoir_client):
...
@pytest.mark.asyncio
async def test_register_invalid_config_format(memoir_client):
...
@pytest.mark.asyncio
async def test_get_empty_agent_id(memoir_client):
    """Should throw on empty agent ID."""
    with pytest.raises(Exception) as exc_info:
        await memoir_client.agents.get("")
...
@pytest.mark.asyncio
async def test_get_whitespace_agent_id(memoir_client):
...
@pytest.mark.asyncio
async def test_list_invalid_limit_zero(memoir_client):
    """Should throw on zero limit."""
    with pytest.raises(Exception) as exc_info:
        await memoir_client.agents.list(AgentFilters(limit=0))
...
@pytest.mark.asyncio
async def test_list_invalid_limit_too_large(memoir_client):
    """Should throw on limit too large."""
    with pytest.raises(Exception) as exc_info:
        await memoir_client.agents.list(AgentFilters(limit=2000))
...
@pytest.mark.asyncio
async def test_list_negative_offset(memoir_client):
    """Should throw on negative offset."""
    with pytest.raises(Exception) as exc_info:
        await memoir_client.agents.list(AgentFilters(offset=-5))
...
async def test_list_invalid_status(memoir_client):
    ...
    with pytest.raises(Exception) as exc_info:
        await memoir_client.agents.list(AgentFilters(status="deleted"))
...
@pytest.mark.asyncio
async def test_list_invalid_sort_by(memoir_client):
    """Should throw on invalid sortBy."""
    with pytest.raises(Exception) as exc_info:
        await memoir_client.agents.list(AgentFilters(sort_by="invalid"))
...
async def test_search_invalid_limit(memoir_client):
    ...
    with pytest.raises(Exception) as exc_info:
        await memoir_client.agents.search(AgentFilters(limit=-1))
...
async def test_search_invalid_status(memoir_client):
    ...
    with pytest.raises(Exception) as exc_info:
        await memoir_client.agents.search(AgentFilters(status="invalid"))
...
async def test_count_invalid_status(memoir_client):
    ...
    from memoir.agents.validators import AgentValidationError

    with pytest.raises(AgentValidationError) as exc_info:
        await memoir_client.agents.count(AgentFilters(status="invalid-status"))
...
@pytest.mark.asyncio
async def test_get_stats_empty_agent_id(memoir_client):
    """Should throw on empty agent ID."""
    with pytest.raises(Exception) as exc_info:
        await memoir_client.agents.get_stats("")
...
@pytest.mark.asyncio
async def test_update_empty_agent_id(memoir_client):
    """Should throw on empty agent ID."""
    with pytest.raises(Exception) as exc_info:
        await memoir_client.agents.update("", {"name": "New Name"})
...
@pytest.mark.asyncio
async def test_update_no_fields(memoir_client):
    """Should throw when no update fields provided."""
    with pytest.raises(Exception) as exc_info:
        await memoir_client.agents.update("test-agent", {})
...
async def test_update_invalid_status(memoir_client):
    """Should throw on invalid status."""
    with pytest.raises(Exception) as exc_info:
        await memoir_client.agents.update("test-agent", {"status": "deleted"})
...
@pytest.mark.asyncio
async def test_update_empty_name(memoir_client):
    """Should throw on empty name."""
    with pytest.raises(Exception) as exc_info:
        await memoir_client.agents.update("test-agent", {"name": ""})
...
@pytest.mark.asyncio
async def test_configure_empty_agent_id(memoir_client):
    """Should throw on empty agent ID."""
    with pytest.raises(Exception) as exc_info:
        await memoir_client.agents.configure("", {"setting": "value"})
...
@pytest.mark.asyncio
async def test_configure_empty_config(memoir_client):
    """Should throw on empty config object."""
    with pytest.raises(Exception) as exc_info:
        await memoir_client.agents.configure("test-agent", {})
...
@pytest.mark.asyncio
async def test_configure_invalid_config_format(memoir_client):
    """Should throw on invalid config format."""
    with pytest.raises(Exception) as exc_info:
        await memoir_client.agents.configure("test-agent", "invalid")
...
@pytest.mark.asyncio
async def test_unregister_empty_agent_id(memoir_client):
    """Should throw on empty agent ID."""
    with pytest.raises(Exception) as exc_info:
        await memoir_client.agents.unregister("")
...
@pytest.mark.asyncio
async def test_unregister_conflicting_options(memoir_client):
    """Should throw on conflicting options."""
    from memoir import UnregisterAgentOptions
...
@pytest.mark.asyncio
async def test_unregister_many_invalid_filters(memoir_client):
    """Should throw on invalid filters."""
    with pytest.raises(Exception) as exc_info:
        await memoir_client.agents.unregister_many(filters="invalid")
...
@pytest.mark.asyncio
async def test_unregister_many_conflicting_options(memoir_client):
    """Should throw on conflicting options."""
    from memoir import UnregisterAgentOptions

    with pytest.raises(Exception) as exc_info:
        await memoir_client.agents.unregister_many(
...
@pytest.mark.asyncio
async def test_register_agent(memoir_client, test_ids):
    """
    ...
    result = await memoir_client.agents.register(
        AgentRegistration(
            id=agent_id,
            name="Test Agent",
    ...
    await memoir_client.agents.unregister(agent_id)


@pytest.mark.asyncio
async def test_register_agent_updates_existing(memoir_client, test_ids):
    """
    ...
    result = await memoir_client.agents.update(
        agent_id,
        {
            "name": "Agent V2",
...
@pytest.mark.asyncio
async def test_register_agent_with_tenant_id(memoir_client, test_ids):
    """
...
@pytest.mark.asyncio
async def test_register_backward_compatible_without_tenant_id(memoir_client, test_ids):
    """
...
@pytest.mark.asyncio
async def test_get_registered_agent(memoir_client, test_ids):
    """
    ...
    # Get agent
    result = await memoir_client.agents.get(agent_id)
...
@pytest.mark.asyncio
async def test_get_nonexistent_returns_none(memoir_client):
    """
    Test that getting non-existent agent returns None.

    Port of: agents.test.ts - get tests
    """
    result = await memoir_client.agents.get("agent-does-not-exist")

    assert result is None


@pytest.mark.asyncio
async def test_get_agent_has_tenant_id_field(memoir_client, test_ids):
    """
...
@pytest.mark.asyncio
async def test_list_agents(memoir_client, test_ids):
    """
    ...
    result = await memoir_client.agents.list(AgentFilters(limit=100))
...
@pytest.mark.asyncio
async def test_unregister_agent(memoir_client, test_ids):
    """
    ...
    # Verify unregistered
    retrieved = await memoir_client.agents.get(agent_id)
...
@pytest.mark.asyncio
async def test_get_agent_stats(memoir_client, test_ids):
    """
    ...
    # Get stats
    stats = await memoir_client.agents.get_stats(agent_id)
...
@pytest.mark.asyncio
async def test_count_agents(memoir_client, test_ids):
    """
    ...
    # Count agents
    count = await memoir_client.agents.count()
...
@pytest.mark.asyncio
async def test_unregister_many_without_cascade(memoir_client, ctx):
    """
    ...
    result = await memoir_client.agents.unregister_many(
    ...
    # Verify unregistered
    agent1_check = await memoir_client.agents.get(agent1_id)
    agent2_check = await memoir_client.agents.get(agent2_id)
    agent3_check = await memoir_client.agents.get(agent3_id)
    ...
    # Cleanup
    await memoir_client.agents.unregister(agent3_id)


@pytest.mark.asyncio
async def test_unregister_many_dry_run(memoir_client, ctx):
    """
    ...
    agent = await memoir_client.agents.get(agent_id)
...
@pytest.mark.asyncio
async def test_cascade_delete_across_memory_spaces(memoir_client, ctx):
    """
    ...
    # Verify setup
    memories1 = await memoir_client.vector.list(memory_space_id=space1_id)
    memories2 = await memoir_client.vector.list(memory_space_id=space2_id)
    print(f"  ℹ️  Setup: {len(memories1)} memories in space1, {len(memories2)} in space2")

    # CASCADE DELETE
    result = await memoir_client.agents.unregister(
        agent_id,
    ...
    remaining1 = await memoir_client.vector.list(memory_space_id=space1_id)
    remaining2 = await memoir_client.vector.list(memory_space_id=space2_id)
    ...
    # Cleanup spaces
    await memoir_client.memory_spaces.delete(space1_id)
    await memoir_client.memory_spaces.delete(space2_id)


@pytest.mark.asyncio
async def test_cascade_delete_dry_run(memoir_client, ctx):
    """
...
@pytest.mark.asyncio
async def test_cascade_delete_with_verification(memoir_client, ctx):
    """
...
@pytest.mark.asyncio
async def test_cascade_delete_without_registration(memoir_client, ctx):
    """
    ...
    before_memories = await memoir_client.vector.list(memory_space_id=space_id)
    ...
    after_memories = await memoir_client.vector.list(memory_space_id=space_id)
    ...
    # Cleanup
    await memoir_client.memory_spaces.delete(space_id)


@pytest.mark.asyncio
async def test_unregister_many_with_cascade(memoir_client, ctx):
    """
...
@pytest.mark.asyncio
async def test_agent_statistics_from_actual_data(memoir_client, ctx):
    """
...
@pytest.mark.asyncio
async def test_exists_registered_agent(memoir_client, test_ids):
    """
    ...
    # Check exists
    result = await memoir_client.agents.exists(agent_id)
...
@pytest.mark.asyncio
async def test_exists_nonexistent_agent(memoir_client):
    """
    Test exists() returns False for non-existent agent.
    """
    result = await memoir_client.agents.exists("agent-does-not-exist-123")
    assert result is False


@pytest.mark.asyncio
async def test_exists_empty_agent_id(memoir_client):
    """Should throw on empty agent ID."""
    with pytest.raises(Exception) as exc_info:
        await memoir_client.agents.exists("")
...
@pytest.mark.asyncio
async def test_update_many_agents(memoir_client, ctx):
    """
    ...
    result = await memoir_client.agents.update_many(
    ...
    # Verify updates
    agent1 = await memoir_client.agents.get(agent1_id)
    agent2 = await memoir_client.agents.get(agent2_id)

    assert agent1.metadata.get("version") == "2.0"
    assert agent2.metadata.get("version") == "2.0"

    # Cleanup
    await memoir_client.agents.unregister(agent1_id)
    await memoir_client.agents.unregister(agent2_id)


@pytest.mark.asyncio
async def test_update_many_no_matches(memoir_client, ctx):
    """
...
@pytest.mark.asyncio
async def test_export_agents_json(memoir_client, ctx):
    """
    ...
    # Export as JSON
    result = await memoir_client.agents.export(
...
@pytest.mark.asyncio
async def test_export_agents_csv(memoir_client, ctx):
    """
...
@pytest.mark.asyncio
async def test_export_with_filters(memoir_client, ctx):
    """
...
async def test_export_invalid_format(memoir_client):
    ...
    with pytest.raises(Exception) as exc_info:
        await memoir_client.agents.export(
...
@pytest.mark.asyncio
async def test_filters_invalid_sort_order(memoir_client):
    """Should throw on invalid sort order."""
    with pytest.raises(Exception) as exc_info:
        await memoir_client.agents.list(AgentFilters(sort_order="random"))
...
@pytest.mark.asyncio
async def test_filters_invalid_capabilities_match(memoir_client):
    """Should throw on invalid capabilities_match."""
    with pytest.raises(Exception) as exc_info:
        await memoir_client.agents.list(AgentFilters(capabilities_match="some"))
...
@pytest.mark.asyncio
async def test_filters_timestamp_range_validation(memoir_client):
    """Should throw when registered_after >= registered_before."""
    with pytest.raises(Exception) as exc_info:
        await memoir_client.agents.list(
...
@pytest.mark.asyncio
async def test_filters_last_active_range_validation(memoir_client):
...
@pytest.mark.asyncio
async def test_unregister_nonexistent_agent_gracefully(memoir_client, ctx):
    """
...
@pytest.mark.asyncio
async def test_unregister_agent_with_no_data(memoir_client, ctx):
    """
