...
from memoir.governance import GovernanceValidationError
...
@pytest.mark.asyncio
async def test_set_policy_missing_scope(memoir_client: Memoir):
    ...
    with pytest.raises(GovernanceValidationError) as exc_info:
        await memoir_client.governance.set_policy(policy)

    assert "must specify either organization_id or memory_space_id" in str(exc_info.value)


@pytest.mark.asyncio
async def test_set_policy_invalid_period_format(memoir_client: Memoir):
...
@pytest.mark.asyncio
async def test_set_policy_overlapping_ranges(memoir_client: Memoir):
...
@pytest.mark.asyncio
async def test_set_policy_invalid_version_count(memoir_client: Memoir):
...
@pytest.mark.asyncio
async def test_set_policy_invalid_importance_bounds(memoir_client: Memoir):
...
@pytest.mark.asyncio
async def test_enforce_missing_scope(memoir_client: Memoir):
    ...
    with pytest.raises(Exception) as exc_info:
        await memoir_client.governance.enforce(
            EnforcementOptions(
                layers=["vector"],
                rules=["retention"],
...
@pytest.mark.asyncio
async def test_enforce_empty_scope(memoir_client: Memoir):
...
@pytest.mark.asyncio
async def test_enforce_invalid_layers(memoir_client: Memoir):
...
@pytest.mark.asyncio
async def test_enforce_invalid_rules(memoir_client: Memoir):
...
@pytest.mark.asyncio
async def test_enforce_empty_layers_array(memoir_client: Memoir):
...
@pytest.mark.asyncio
async def test_enforce_empty_rules_array(memoir_client: Memoir):
...
@pytest.mark.asyncio
async def test_compliance_report_invalid_date_range(memoir_client: Memoir):
    """Should throw when start date is after end date."""
    now = datetime.now()
    yesterday = now - timedelta(days=1)

    with pytest.raises(GovernanceValidationError) as exc_info:
        await memoir_client.governance.get_compliance_report(
            ComplianceReportOptions(
...
@pytest.mark.asyncio
async def test_enforcement_stats_invalid_period(memoir_client: Memoir):
    """Should throw on invalid period format."""
    with pytest.raises(GovernanceValidationError) as exc_info:
        await memoir_client.governance.get_enforcement_stats(
            EnforcementStatsOptions(
...
@pytest.mark.asyncio
async def test_agent_override_empty_memory_space_id(memoir_client: Memoir):
    ...
    with pytest.raises(GovernanceValidationError) as exc_info:
        await memoir_client.governance.set_agent_override("", override)

    assert "memory_space_id is required and cannot be empty" in str(exc_info.value)


@pytest.mark.asyncio
async def test_agent_override_whitespace_memory_space_id(memoir_client: Memoir):
...
@pytest.mark.asyncio
async def test_agent_override_invalid_period(memoir_client: Memoir):
    ...
    with pytest.raises(GovernanceValidationError) as exc_info:
        await memoir_client.governance.set_agent_override("test-space", override)

    assert "Invalid period format" in str(exc_info.value)


@pytest.mark.asyncio
async def test_simulate_invalid_period(memoir_client: Memoir):
    """Should throw on invalid period format in simulation."""
    with pytest.raises(GovernanceValidationError) as exc_info:
        await memoir_client.governance.simulate(
            SimulationOptions(
                organization_id="test-org",
                conversations=ConversationsPolicy(
                    retention=ConversationsRetention(
...
@pytest.mark.asyncio
async def test_simulate_invalid_version_count(memoir_client: Memoir):
...
@pytest.mark.asyncio
async def test_get_template_invalid_name(memoir_client: Memoir):
    """Should throw on invalid template name."""
    with pytest.raises(GovernanceValidationError) as exc_info:
        await memoir_client.governance.get_template("INVALID")  # type: ignore

    assert "Invalid compliance template" in str(exc_info.value)


@pytest.mark.asyncio
async def test_get_policy_empty_organization_id(memoir_client: Memoir):
    """Should throw on empty organization_id."""
    with pytest.raises(GovernanceValidationError) as exc_info:
        await memoir_client.governance.get_policy(PolicyScope(organization_id=""))

    assert "must include either organization_id or memory_space_id" in str(exc_info.value)


@pytest.mark.asyncio
async def test_get_policy_empty_memory_space_id(memoir_client: Memoir):
    """Should throw on empty memory_space_id."""
    with pytest.raises(GovernanceValidationError) as exc_info:
        await memoir_client.governance.get_policy(PolicyScope(memory_space_id="   "))
...
@pytest.mark.asyncio
async def test_set_organization_policy(memoir_client: Memoir):
    ...
    result = await memoir_client.governance.set_policy(policy)
...
@pytest.mark.asyncio
async def test_get_organization_policy(memoir_client: Memoir):
    """Should get organization-wide policy."""
    policy = await memoir_client.governance.get_policy(
...
@pytest.mark.asyncio
async def test_set_memory_space_override(memoir_client: Memoir):
    ...
    await memoir_client.governance.set_agent_override("audit-agent-space", override_policy)

    space_policy = await memoir_client.governance.get_policy(
...
@pytest.mark.asyncio
async def test_get_gdpr_template(memoir_client: Memoir):
    """Should get GDPR compliance template."""
    policy = await memoir_client.governance.get_template("GDPR")
...
@pytest.mark.asyncio
async def test_get_hipaa_template(memoir_client: Memoir):
    """Should get HIPAA compliance template."""
    policy = await memoir_client.governance.get_template("HIPAA")
...
@pytest.mark.asyncio
async def test_get_soc2_template(memoir_client: Memoir):
    """Should get SOC2 compliance template."""
    policy = await memoir_client.governance.get_template("SOC2")
...
@pytest.mark.asyncio
async def test_get_finra_template(memoir_client: Memoir):
    """Should get FINRA compliance template."""
    policy = await memoir_client.governance.get_template("FINRA")
...
@pytest.mark.asyncio
async def test_apply_gdpr_template(memoir_client: Memoir):
    """Should apply GDPR template to organization."""
    gdpr_policy = await memoir_client.governance.get_template("GDPR")
    gdpr_policy.organization_id = "test-org-gdpr"

    result = await memoir_client.governance.set_policy(gdpr_policy)

    assert result.success is True

    retrieved = await memoir_client.governance.get_policy(
...
@pytest.mark.asyncio
async def test_manual_enforcement(memoir_client: Memoir):
    ...
    # Enforce it
    result = await memoir_client.governance.enforce(
        EnforcementOptions(
...
@pytest.mark.asyncio
async def test_simulate_policy_impact(memoir_client: Memoir):
    """Should simulate policy impact."""
    impact = await memoir_client.governance.simulate(
        SimulationOptions(
...
@pytest.mark.asyncio
async def test_generate_compliance_report(memoir_client: Memoir):
    """Should generate compliance report."""
    now = datetime.now()
    thirty_days_ago = now - timedelta(days=30)

    report = await memoir_client.governance.get_compliance_report(
        ComplianceReportOptions(
...
@pytest.mark.asyncio
async def test_get_enforcement_stats(memoir_client: Memoir):
    """Should get 30-day enforcement stats."""
    stats = await memoir_client.governance.get_enforcement_stats(
        EnforcementStatsOptions(
            period="30d",
...
@pytest.mark.asyncio
async def test_enforcement_stats_periods(memoir_client: Memoir):
...
@pytest.mark.asyncio
async def test_gdpr_compliance_workflow(memoir_client: Memoir):
    ...
    await memoir_client.governance.set_policy(gdpr_policy)
    ...
    # 3. Simulate impact
    simulation = await memoir_client.governance.simulate(
        SimulationOptions(organization_id=org_id)
    )
    assert simulation.versions_affected >= 0

    # 4. Enforce policy
    enforcement = await memoir_client.governance.enforce(
...
@pytest.mark.asyncio
async def test_set_policy_with_session_lifecycle(memoir_client: Memoir):
...
@pytest.mark.asyncio
async def test_set_policy_with_session_lifecycle_only(memoir_client: Memoir):
...
@pytest.mark.asyncio
async def test_set_policy_with_session_cleanup_only(memoir_client: Memoir):
...
@pytest.mark.asyncio
async def test_set_policy_with_session_limits_only(memoir_client: Memoir):
...
@pytest.mark.asyncio
async def test_set_policy_with_partial_session_limits(memoir_client: Memoir):
...
@pytest.mark.asyncio
async def test_get_policy_without_sessions(memoir_client: Memoir):
...
@pytest.mark.asyncio
async def test_set_agent_override_with_sessions(memoir_client: Memoir):
    ...
    await memoir_client.governance.set_policy(org_policy)
    ...
    await memoir_client.governance.set_agent_override("test-space-override-sessions", override)
...
@pytest.mark.asyncio
async def test_session_lifecycle_policy_serialization(memoir_client: Memoir):
...
@pytest.mark.asyncio
async def test_session_cleanup_policy_serialization(memoir_client: Memoir):
...
@pytest.mark.asyncio
async def test_session_limits_policy_serialization(memoir_client: Memoir):
...
@pytest.mark.asyncio
async def test_session_policy_optional_limits(memoir_client: Memoir):
    ...
    await memoir_client.governance.set_policy(policy_no_limits)
