...
from memoir.types import StoreFactParams
...
class TestFactsFilterParametrized:
    ...
    @pytest.mark.asyncio
    async def test_list_filters_by_fact_type(self, memoir_client, fact_type):
        """Test facts.list() filters correctly by factType."""
        space_id = generate_test_memory_space_id()

        # Store target fact
        target_fact = await memoir_client.facts.store(
            StoreFactParams(
        ...
        results = await memoir_client.facts.list(
    ...
    @pytest.mark.asyncio
    async def test_count_filters_by_fact_type(self, memoir_client, fact_type):
    ...
    @pytest.mark.asyncio
    async def test_search_filters_by_fact_type(self, memoir_client, fact_type):
        """Test facts.search() filters correctly by factType."""
        space_id = generate_test_memory_space_id()

        # Store target fact with searchable content
        search_term = "searchable"
        _target_fact = await memoir_client.facts.store(
            StoreFactParams(
                memory_space_id=space_id,
                fact_type=fact_type,
        ...
        results = await memoir_client.facts.search(
    ...
    async def test_query_by_subject_filters_by_fact_type(
        ...
        results = await memoir_client.facts.query_by_subject(
    ...
    @pytest.mark.asyncio
    async def test_export_filters_by_fact_type(self, memoir_client, fact_type):
        ...
        export_data = await memoir_client.facts.export(
...
class TestFactsFilterEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_list_filter_no_matches(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_list_filter_multiple_results(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_observation_regression(self, memoir_client):
        ...
        _obs_fact = await memoir_client.facts.store(
            StoreFactParams(
                memory_space_id=space_id,
        ...
        # Test list
        from memoir.types import ListFactsFilter
        list_results = await memoir_client.facts.list(
            ListFactsFilter(memory_space_id=space_id, fact_type="observation")
        )
        ...
        # Test search
        from memoir.types import SearchFactsOptions
        search_results = await memoir_client.facts.search(
        ...
        from memoir.types import QueryBySubjectFilter
        subject_results = await memoir_client.facts.query_by_subject(
            QueryBySubjectFilter(
    ...
    @pytest.mark.asyncio
    async def test_combine_fact_type_with_other_filters(self, memoir_client):
        ...
        from memoir.types import ListFactsFilter
        results_with_tags = await memoir_client.facts.list(
            ListFactsFilter(
    ...
    @pytest.mark.asyncio
    async def test_count_with_combined_filters(self, memoir_client):
        ...
        from memoir.types import CountFactsFilter
        knowledge_count = await memoir_client.facts.count(
        ...
        from memoir.types import CountFactsFilter
        total_count = await memoir_client.facts.count(
    ...
    @pytest.mark.asyncio
    async def test_search_with_min_confidence_and_fact_type(self, memoir_client):
