...
from memoir import Memoir, MemoirConfig, RememberStreamParams
...
class TestMemoryStreaming:
    ...
    async def setup(self, test_run_context, request):
        ...
        self.memoir = Memoir(MemoirConfig(convex_url=self.convex_url))
        ...
        await self.memoir.memory_spaces.register(
    ...
    async def test_basic_streaming(self):
        ...
        result = await self.memoir.memory.remember_stream(
    ...
    async def test_empty_stream_error(self):
        ...
        with pytest.raises(Exception) as exc_info:
            await self.memoir.memory.remember_stream(
                RememberStreamParams(
                    memory_space_id=self.test_space_id,
    ...
    async def test_stream_verification_in_database(self):
        ...
        conversation = await self.memoir.conversations.get(conv_id)
        ...
        memories = await retry_async(
            lambda: self.memoir.memory.search(
                self.test_space_id,
                "weather sunny",
            ),
            max_retries=3,
