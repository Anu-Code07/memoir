...
from memoir.types import ContextInput
...
class TestContextsFilterParametrized:
    ...
    async def test_list_filters_by_status(self, memoir_client, status):
        ...
        target_ctx = await memoir_client.contexts.create(
            ContextInput(
        ...
        results = await memoir_client.contexts.list(
    ...
    @pytest.mark.asyncio
    async def test_count_filters_by_status(self, memoir_client, status):
...
class TestContextsFilterEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_status_transition(self, memoir_client):
        ...
        active_results = await memoir_client.contexts.list(
        ...
        # Update to completed
        updated_ctx = await memoir_client.contexts.update(
            ctx.id,
        ...
        completed_results = await memoir_client.contexts.list(
        ...
        active_results_after = await memoir_client.contexts.list(
            memory_space_id=space_id, status="active"
        )
    ...
    @pytest.mark.asyncio
    async def test_combine_status_with_user_id_filter(self, memoir_client):
        ...
        _target_ctx = await memoir_client.contexts.create(
            ContextInput(
                memory_space_id=space_id,
                user_id=target_user,
    ...
    @pytest.mark.asyncio
    async def test_multiple_statuses_same_space(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_count_all_vs_filtered(self, memoir_client):
        ...
        # Count active only
        active_count = await memoir_client.contexts.count(
            memory_space_id=space_id, status="active"
        )
        assert active_count >= 3, f"Should count at least 3 active, got {active_count}"

        # Count completed only
        completed_count = await memoir_client.contexts.count(
            memory_space_id=space_id, status="completed"
        )
        assert (
            completed_count >= 2
        ...
        # Count all (no filter)
        total_count = await memoir_client.contexts.count(memory_space_id=space_id)
        assert total_count >= 5, f"Should count at least 5 total, got {total_count}"

    @pytest.mark.asyncio
    async def test_combine_status_with_parent_id_filter(self, memoir_client):
        """Test combining status filter with parentId filter."""
        space_id = generate_test_memory_space_id()
        user_id = generate_test_user_id()

        # Create parent context
        parent_ctx = await memoir_client.contexts.create(
            ContextInput(
                memory_space_id=space_id,
                user_id=user_id,
        ...
        _active_child = await memoir_client.contexts.create(
            ContextInput(
                memory_space_id=space_id,
                user_id=user_id,
        ...
        _completed_child = await memoir_client.contexts.create(
            ContextInput(
                memory_space_id=space_id,
                user_id=user_id,
