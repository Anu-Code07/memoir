...
@pytest.mark.asyncio
async def test_store_memory_without_embedding(memoir_client, test_ids, cleanup_helper):
    """
    ...
    result = await memoir_client.vector.store(
        memory_space_id,
        StoreMemoryInput(
...
@pytest.mark.asyncio
async def test_store_memory_with_embedding(memoir_client, test_ids, cleanup_helper):
    """
...
@pytest.mark.asyncio
async def test_store_memory_with_conversation_ref(memoir_client, test_ids, cleanup_helper):
    """
...
@pytest.mark.asyncio
async def test_store_memory_with_user_id_for_gdpr(memoir_client, test_ids, cleanup_helper):
    """
...
@pytest.mark.asyncio
async def test_store_memory_with_tenant_id(memoir_client, test_ids, cleanup_helper):
    """
...
@pytest.mark.asyncio
async def test_store_memory_backward_compatible_without_tenant_id(memoir_client, test_ids, cleanup_helper):
    """
...
@pytest.mark.asyncio
async def test_get_existing_memory(memoir_client, test_ids, cleanup_helper):
    """
    ...
    stored = await memoir_client.vector.store(memory_space_id, memory_input)
    memory_id = stored.memory_id

    # Retrieve it
    result = await memoir_client.vector.get(memory_space_id, memory_id)
...
@pytest.mark.asyncio
async def test_get_nonexistent_memory_returns_none(memoir_client, test_ids):
    """
    ...
    result = await memoir_client.vector.get(memory_space_id, "mem-does-not-exist")

    assert result is None


@pytest.mark.asyncio
async def test_get_memory_space_isolation(memoir_client, test_ids, cleanup_helper):
    """
    ...
    stored = await memoir_client.vector.store(memory_space_id_1, memory_input)
    memory_id = stored.memory_id

    # Try to access from space 2
    result = await memoir_client.vector.get(memory_space_id_2, memory_id)
...
@pytest.mark.asyncio
async def test_search_keyword(memoir_client, test_ids, cleanup_helper):
    """
    ...
    await memoir_client.vector.store(
        memory_space_id,
        StoreMemoryInput(
    ...
    results = await retry_async(
        lambda: memoir_client.vector.search(memory_space_id, "password"),
        max_retries=3,
    )
...
@pytest.mark.asyncio
async def test_search_filter_by_user_id(memoir_client, test_ids, cleanup_helper):
    """
    ...
    results = await retry_async(
        lambda: memoir_client.vector.search(
            memory_space_id,
            "mode",
...
@pytest.mark.asyncio
async def test_search_filter_by_tags(memoir_client, test_ids, cleanup_helper):
    """
...
@pytest.mark.asyncio
async def test_search_filter_by_min_importance(memoir_client, test_ids, cleanup_helper):
    """
...
@pytest.mark.asyncio
async def test_search_respects_limit(memoir_client, test_ids, cleanup_helper):
    """
...
@pytest.mark.asyncio
async def test_update_memory_content(memoir_client, test_ids, cleanup_helper):
    """
    ...
    # Update content
    updated = await memoir_client.vector.update(
        memory_space_id,
        memory_id,
...
@pytest.mark.asyncio
async def test_update_memory_importance(memoir_client, test_ids, cleanup_helper):
    """
...
@pytest.mark.asyncio
async def test_delete_memory(memoir_client, test_ids, cleanup_helper):
    """
    ...
    # Delete it
    result = await memoir_client.vector.delete(memory_space_id, memory_id)

    # Verify deleted
    assert result.get("success") is True or result.get("deleted") is True

    # Try to retrieve - should be None
    retrieved = await memoir_client.vector.get(memory_space_id, memory_id)
...
@pytest.mark.asyncio
async def test_list_memories(memoir_client, test_ids, cleanup_helper):
    """
    ...
    # List memories
    result = await memoir_client.vector.list(memory_space_id, limit=10)
...
@pytest.mark.asyncio
async def test_list_memories_with_limit(memoir_client, test_ids, cleanup_helper):
    """
    ...
    # List with limit=2
    result = await memoir_client.vector.list(memory_space_id, limit=2)
...
@pytest.mark.asyncio
async def test_count_memories(memoir_client, test_ids, cleanup_helper):
    """
    ...
    # Count memories
    count = await memoir_client.vector.count(memory_space_id)
...
@pytest.mark.asyncio
async def test_memory_versioning(memoir_client, test_ids, cleanup_helper):
    """
    ...
    v1 = await memoir_client.vector.store(memory_space_id, memory_input)
    ...
    # Update to version 2
    v2 = await memoir_client.vector.update(
        memory_space_id,
        memory_id,
        {"content": "Version 2"},
    )
    ...
    # Update to version 3
    v3 = await memoir_client.vector.update(
        memory_space_id,
        memory_id,
        {"content": "Version 3"},
    )
...
@pytest.mark.asyncio
async def test_get_version(memoir_client, test_ids, cleanup_helper):
    """
    ...
    await memoir_client.vector.update(
        memory_space_id,
        memory_id,
        {"content": "Version 2"},
    )
    ...
    # Get version 1
    retrieved_v1 = await memoir_client.vector.get_version(
        memory_space_id,
        memory_id,
        1,
    )
    ...
    # Get version 2
    retrieved_v2 = await memoir_client.vector.get_version(
        memory_space_id,
        memory_id,
        2,
    )
...
@pytest.mark.asyncio
async def test_get_history(memoir_client, test_ids, cleanup_helper):
    """
    ...
    await memoir_client.vector.update(memory_space_id, memory_id, {"content": "Version 2"})
    await memoir_client.vector.update(memory_space_id, memory_id, {"content": "Version 3"})

    # Get history
    history = await memoir_client.vector.get_history(memory_space_id, memory_id)
...
@pytest.mark.asyncio
async def test_store_validates_in_convex_storage(memoir_client, test_ids, cleanup_helper):
    """
    ...
    result = await memoir_client.vector.store(memory_space_id, memory_input)
...
@pytest.mark.asyncio
async def test_store_memory_with_special_characters(memoir_client, test_ids, cleanup_helper):
    """
...
@pytest.mark.asyncio
async def test_store_memory_with_long_content(memoir_client, test_ids, cleanup_helper):
    """
...
@pytest.mark.asyncio
async def test_list_memories_filter_with_tenant_id(memoir_client, test_ids, cleanup_helper):
    """
...
@pytest.mark.asyncio
async def test_count_memories_filter_with_tenant_id(memoir_client, test_ids, cleanup_helper):
    """
...
@pytest.mark.asyncio
async def test_validate_list_filter_success(memoir_client, test_ids):
    """Test validate_list_filter() with valid filter."""
    from memoir.vector.validators import validate_list_filter
...
@pytest.mark.asyncio
async def test_validate_list_filter_missing_memory_space_id(memoir_client):
    """Test validate_list_filter() raises on missing memory_space_id."""
    from memoir.vector import VectorValidationError
    from memoir.vector.validators import validate_list_filter
...
@pytest.mark.asyncio
async def test_validate_list_filter_invalid_source_type(memoir_client, test_ids):
...
@pytest.mark.asyncio
async def test_validate_count_filter_success(memoir_client, test_ids):
    """Test validate_count_filter() with valid filter."""
    from memoir.vector.validators import validate_count_filter
...
@pytest.mark.asyncio
async def test_validate_count_filter_missing_memory_space_id(memoir_client):
...
@pytest.mark.asyncio
async def test_validate_export_options_success(memoir_client, test_ids):
    """Test validate_export_options() with valid options."""
    from memoir.vector.validators import validate_export_options
...
@pytest.mark.asyncio
async def test_validate_export_options_missing_memory_space_id(memoir_client):
...
@pytest.mark.asyncio
async def test_validate_export_options_invalid_format(memoir_client, test_ids):
...
@pytest.mark.asyncio
async def test_validate_delete_many_filter_success(memoir_client, test_ids):
    """Test validate_delete_many_filter() with valid filter."""
    from memoir.vector.validators import validate_delete_many_filter
...
@pytest.mark.asyncio
async def test_validate_delete_many_filter_missing_memory_space_id(memoir_client):
...
@pytest.mark.asyncio
async def test_validate_update_many_inputs_success(memoir_client, test_ids):
    """Test validate_update_many_inputs() with valid inputs."""
    from memoir.vector.validators import validate_update_many_inputs
...
@pytest.mark.asyncio
async def test_validate_update_many_inputs_missing_filter(memoir_client):
...
@pytest.mark.asyncio
async def test_validate_update_many_inputs_missing_updates(memoir_client, test_ids):
...
@pytest.mark.asyncio
async def test_validate_update_many_inputs_invalid_importance(memoir_client, test_ids):
...
@pytest.mark.asyncio
async def test_store_validation_empty_memory_space_id(memoir_client):
...
@pytest.mark.asyncio
async def test_store_validation_whitespace_memory_space_id(memoir_client):
...
@pytest.mark.asyncio
async def test_store_validation_empty_content(memoir_client, test_ids):
...
@pytest.mark.asyncio
async def test_store_validation_invalid_content_type(memoir_client, test_ids):
...
@pytest.mark.asyncio
async def test_store_validation_invalid_source_type(memoir_client, test_ids):
...
@pytest.mark.asyncio
async def test_store_validation_invalid_importance_negative(memoir_client, test_ids):
...
@pytest.mark.asyncio
async def test_store_validation_invalid_importance_too_high(memoir_client, test_ids):
...
@pytest.mark.asyncio
async def test_store_validation_tags_with_empty_strings(memoir_client, test_ids):
...
@pytest.mark.asyncio
async def test_store_validation_invalid_embedding_empty(memoir_client, test_ids):
...
@pytest.mark.asyncio
async def test_store_validation_invalid_embedding_nan(memoir_client, test_ids):
...
@pytest.mark.asyncio
async def test_get_validation_empty_memory_space_id(memoir_client):
    """Should throw on empty memory_space_id."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.get("", "mem-123")
    assert "memory_space_id cannot be empty" in str(exc_info.value)


@pytest.mark.asyncio
async def test_get_validation_empty_memory_id(memoir_client, test_ids):
    """Should throw on empty memory_id."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.get(test_ids["memory_space_id"], "")
...
@pytest.mark.asyncio
async def test_search_validation_empty_memory_space_id(memoir_client):
    """Should throw on empty memory_space_id."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.search("", "query")
    assert "memory_space_id cannot be empty" in str(exc_info.value)


@pytest.mark.asyncio
async def test_search_validation_empty_query(memoir_client, test_ids):
    """Should throw on empty query."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.search(test_ids["memory_space_id"], "   ")
    assert "query cannot be empty" in str(exc_info.value)


@pytest.mark.asyncio
async def test_search_validation_invalid_embedding_empty(memoir_client, test_ids):
    """Should throw on empty embedding array."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.search(
            test_ids["memory_space_id"],
            "query",
...
@pytest.mark.asyncio
async def test_search_validation_invalid_embedding_nan(memoir_client, test_ids):
    """Should throw on NaN in embedding."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.search(
            test_ids["memory_space_id"],
...
@pytest.mark.asyncio
async def test_search_validation_invalid_min_score_negative(memoir_client, test_ids):
...
@pytest.mark.asyncio
async def test_search_validation_invalid_min_score_too_high(memoir_client, test_ids):
...
@pytest.mark.asyncio
async def test_search_validation_invalid_limit_zero(memoir_client, test_ids):
...
@pytest.mark.asyncio
async def test_search_validation_invalid_limit_negative(memoir_client, test_ids):
...
@pytest.mark.asyncio
async def test_search_validation_tags_with_empty_strings(memoir_client, test_ids):
    """Should throw on tags with empty strings."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.search(
            test_ids["memory_space_id"],
...
@pytest.mark.asyncio
async def test_search_validation_invalid_min_importance(memoir_client, test_ids):
...
@pytest.mark.asyncio
async def test_update_validation_empty_memory_space_id(memoir_client):
    """Should throw on empty memory_space_id."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.update("", "mem-123", {"content": "Updated"})
    assert "memory_space_id cannot be empty" in str(exc_info.value)


@pytest.mark.asyncio
async def test_update_validation_empty_memory_id(memoir_client, test_ids):
    """Should throw on empty memory_id."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.update(test_ids["memory_space_id"], "", {"content": "Updated"})
    assert "memory_id cannot be empty" in str(exc_info.value)


@pytest.mark.asyncio
async def test_update_validation_no_update_fields(memoir_client, test_ids):
    """Should throw when no update fields provided."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.update(test_ids["memory_space_id"], "mem-123", {})
    assert "At least one update field must be provided" in str(exc_info.value)


@pytest.mark.asyncio
async def test_update_validation_invalid_importance(memoir_client, test_ids):
    """Should throw on invalid importance."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.update(test_ids["memory_space_id"], "mem-123", {"importance": -5})
    assert "importance must be between 0 and 100" in str(exc_info.value)


@pytest.mark.asyncio
async def test_update_validation_invalid_embedding(memoir_client, test_ids):
    """Should throw on invalid embedding."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.update(test_ids["memory_space_id"], "mem-123", {"embedding": []})
    assert "embedding cannot be empty" in str(exc_info.value)


@pytest.mark.asyncio
async def test_update_validation_tags_with_empty_strings(memoir_client, test_ids):
    """Should throw on tags with empty strings."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.update(test_ids["memory_space_id"], "mem-123", {"tags": ["", "valid"]})
    assert "must be a non-empty string" in str(exc_info.value)
...
@pytest.mark.asyncio
async def test_delete_validation_empty_memory_space_id(memoir_client):
    """Should throw on empty memory_space_id."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.delete("", "mem-123")
    assert "memory_space_id cannot be empty" in str(exc_info.value)


@pytest.mark.asyncio
async def test_delete_validation_empty_memory_id(memoir_client, test_ids):
    """Should throw on empty memory_id."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.delete(test_ids["memory_space_id"], "")
    assert "memory_id cannot be empty" in str(exc_info.value)
...
@pytest.mark.asyncio
async def test_update_many_validation_empty_memory_space_id(memoir_client):
    """Should throw on empty memory_space_id."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.update_many("", importance=80)
    assert "memory_space_id cannot be empty" in str(exc_info.value)


@pytest.mark.asyncio
async def test_update_many_validation_no_update_fields(memoir_client, test_ids):
    """Should throw when no update fields provided."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.update_many(test_ids["memory_space_id"])
    assert "At least one update field must be provided" in str(exc_info.value)


@pytest.mark.asyncio
async def test_update_many_validation_invalid_source_type(memoir_client, test_ids):
    """Should throw on invalid source_type."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.update_many(
            test_ids["memory_space_id"],
            source_type="invalid",
            importance=80
        )
...
@pytest.mark.asyncio
async def test_delete_many_validation_empty_memory_space_id(memoir_client):
    """Should throw on empty memory_space_id."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.delete_many("")
    assert "memory_space_id cannot be empty" in str(exc_info.value)


@pytest.mark.asyncio
async def test_delete_many_validation_invalid_source_type(memoir_client, test_ids):
    """Should throw on invalid source_type."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.delete_many(test_ids["memory_space_id"], source_type="invalid")
    assert "Invalid source_type" in str(exc_info.value)
...
@pytest.mark.asyncio
async def test_count_validation_empty_memory_space_id(memoir_client):
    """Should throw on empty memory_space_id."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.count("")
    assert "memory_space_id cannot be empty" in str(exc_info.value)


@pytest.mark.asyncio
async def test_count_validation_invalid_source_type(memoir_client, test_ids):
    """Should throw on invalid source_type."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.count(test_ids["memory_space_id"], source_type="invalid")
    assert "Invalid source_type" in str(exc_info.value)
...
@pytest.mark.asyncio
async def test_list_validation_empty_memory_space_id(memoir_client):
    """Should throw on empty memory_space_id."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.list("")
    assert "memory_space_id cannot be empty" in str(exc_info.value)


@pytest.mark.asyncio
async def test_list_validation_invalid_source_type(memoir_client, test_ids):
    """Should throw on invalid source_type."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.list(test_ids["memory_space_id"], source_type="invalid")
    assert "Invalid source_type" in str(exc_info.value)


@pytest.mark.asyncio
async def test_list_validation_invalid_limit_negative(memoir_client, test_ids):
    """Should throw on negative limit."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.list(test_ids["memory_space_id"], limit=-5)
    assert "limit must be a positive integer" in str(exc_info.value)


@pytest.mark.asyncio
async def test_list_validation_invalid_limit_zero(memoir_client, test_ids):
    """Should throw on limit=0."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.list(test_ids["memory_space_id"], limit=0)
    assert "limit must be a positive integer" in str(exc_info.value)
...
@pytest.mark.asyncio
async def test_export_validation_empty_memory_space_id(memoir_client):
    """Should throw on empty memory_space_id."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.export("", format="json")
    assert "memory_space_id cannot be empty" in str(exc_info.value)


@pytest.mark.asyncio
async def test_export_validation_invalid_format(memoir_client, test_ids):
    """Should throw on invalid format."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.export(test_ids["memory_space_id"], format="xml")
    assert "Invalid format" in str(exc_info.value)


@pytest.mark.asyncio
async def test_export_validation_empty_user_id(memoir_client, test_ids):
    """Should throw on empty user_id."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.export(test_ids["memory_space_id"], user_id="   ")
...
@pytest.mark.asyncio
async def test_archive_validation_empty_memory_space_id(memoir_client):
    """Should throw on empty memory_space_id."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.archive("", "mem-123")
    assert "memory_space_id cannot be empty" in str(exc_info.value)


@pytest.mark.asyncio
async def test_archive_validation_empty_memory_id(memoir_client, test_ids):
    """Should throw on empty memory_id."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.archive(test_ids["memory_space_id"], "")
    assert "memory_id cannot be empty" in str(exc_info.value)
...
@pytest.mark.asyncio
async def test_get_version_validation_empty_memory_space_id(memoir_client):
    """Should throw on empty memory_space_id."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.get_version("", "mem-123", 1)
    assert "memory_space_id cannot be empty" in str(exc_info.value)


@pytest.mark.asyncio
async def test_get_version_validation_empty_memory_id(memoir_client, test_ids):
    """Should throw on empty memory_id."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.get_version(test_ids["memory_space_id"], "", 1)
    assert "memory_id cannot be empty" in str(exc_info.value)


@pytest.mark.asyncio
async def test_get_version_validation_invalid_version_zero(memoir_client, test_ids):
    """Should throw on version=0."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.get_version(test_ids["memory_space_id"], "mem-123", 0)
    assert "version must be a positive integer" in str(exc_info.value)


@pytest.mark.asyncio
async def test_get_version_validation_negative_version(memoir_client, test_ids):
    """Should throw on negative version."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.get_version(test_ids["memory_space_id"], "mem-123", -1)
    assert "version must be a positive integer" in str(exc_info.value)
...
@pytest.mark.asyncio
async def test_get_history_validation_empty_memory_space_id(memoir_client):
    """Should throw on empty memory_space_id."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.get_history("", "mem-123")
    assert "memory_space_id cannot be empty" in str(exc_info.value)


@pytest.mark.asyncio
async def test_get_history_validation_empty_memory_id(memoir_client, test_ids):
    """Should throw on empty memory_id."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.get_history(test_ids["memory_space_id"], "")
    assert "memory_id cannot be empty" in str(exc_info.value)
...
@pytest.mark.asyncio
async def test_get_at_timestamp_validation_empty_memory_space_id(memoir_client):
    """Should throw on empty memory_space_id."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.get_at_timestamp("", "mem-123", 1000)
    assert "memory_space_id cannot be empty" in str(exc_info.value)


@pytest.mark.asyncio
async def test_get_at_timestamp_validation_empty_memory_id(memoir_client, test_ids):
    """Should throw on empty memory_id."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.get_at_timestamp(test_ids["memory_space_id"], "", 1000)
    assert "memory_id cannot be empty" in str(exc_info.value)


@pytest.mark.asyncio
async def test_get_at_timestamp_validation_invalid_timestamp_nan(memoir_client, test_ids):
    """Should throw on NaN timestamp."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.get_at_timestamp(test_ids["memory_space_id"], "mem-123", float('nan'))
    assert "timestamp must be a valid timestamp" in str(exc_info.value)


@pytest.mark.asyncio
async def test_get_at_timestamp_validation_negative_timestamp(memoir_client, test_ids):
    """Should throw on negative timestamp."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.get_at_timestamp(test_ids["memory_space_id"], "mem-123", -1000)
...
@pytest.mark.asyncio
async def test_restore_from_archive_validation_empty_memory_space_id(memoir_client):
    """Should throw on empty memory_space_id."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.restore_from_archive("", "mem-123")
    assert "memory_space_id cannot be empty" in str(exc_info.value)


@pytest.mark.asyncio
async def test_restore_from_archive_validation_empty_memory_id(memoir_client, test_ids):
    """Should throw on empty memory_id."""
    with pytest.raises(VectorValidationError) as exc_info:
        await memoir_client.vector.restore_from_archive(test_ids["memory_space_id"], "")
    assert "memory_id cannot be empty" in str(exc_info.value)
...
@pytest.mark.asyncio
async def test_search_with_query_category(memoir_client, test_ids, cleanup_helper):
    """
...
@pytest.mark.asyncio
async def test_search_validation_invalid_query_category_type(memoir_client, test_ids):
...
@pytest.mark.asyncio
async def test_restore_from_archive(memoir_client, test_ids, cleanup_helper):
    """
    ...
    # Archive it
    archive_result = await memoir_client.vector.archive(memory_space_id, memory_id)
    ...
    after_archive = await memoir_client.vector.get(memory_space_id, memory_id)
    ...
    restore_result = await memoir_client.vector.restore_from_archive(memory_space_id, memory_id)
    ...
    restored_memory = await memoir_client.vector.get(memory_space_id, memory_id)
...
@pytest.mark.asyncio
async def test_delete_many_with_flat_filter(memoir_client, test_ids, cleanup_helper):
    """
    ...
    result = await memoir_client.vector.delete_many(
        memory_space_id,
        source_type="system",
    )
...
@pytest.mark.asyncio
async def test_update_many_with_flat_filter(memoir_client, test_ids, cleanup_helper):
    """
    ...
    result = await memoir_client.vector.update_many(
        memory_space_id,
        source_type="system",
        importance=80,
    )
