...
from memoir.types import ApproveMessageInput, AddMessageInput, CollaborativeSettings
...
async def memoir_client_with_auth(test_config):
    """Memoir client fixture with auth context for collaborative tests."""
    convex_url = test_config["convex_url"]
    auth = AuthContext(user_id=AUTH_USER_ID)
    client = Memoir(MemoirConfig(convex_url=convex_url, auth=auth))
    yield client
    try:
        await client.close()
    except Exception:
...
@pytest.mark.asyncio
async def test_approve_message(memoir_client_with_auth, test_memory_space_id):
    ...
    conv = await memoir_client_with_auth.conversations.create(
        CreateConversationInput(
            memory_space_id=test_memory_space_id,
            type="user-agent",
            participants=ConversationParticipants(
    ...
    # Add pending message
    updated = await memoir_client_with_auth.conversations.add_message(
        AddMessageInput(
            conversation_id=conv.conversation_id,
            role="user",
    ...
    # Approve it
    approved = await memoir_client_with_auth.conversations.approve_message(
        ApproveMessageInput(
            conversation_id=conv.conversation_id,
            message_id=msg_id,
        )
...
@pytest.mark.asyncio
async def test_reject_message(memoir_client_with_auth, test_memory_space_id):
    ...
    # Reject it
    rejected = await memoir_client_with_auth.conversations.reject_message(
        ApproveMessageInput(
            conversation_id=conv.conversation_id,
            message_id=msg_id,
        )
...
@pytest.mark.asyncio
async def test_no_approval_required(memoir_client, test_memory_space_id, test_user_id):
...
@pytest.mark.asyncio
async def test_backward_compatibility(memoir_client, test_memory_space_id, test_user_id):
