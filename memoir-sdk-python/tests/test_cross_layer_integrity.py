...
@pytest.mark.asyncio
async def test_conversation_ref_points_to_actual_conversation(memoir_client, ctx):
    """
    ...
    msg_result = await memoir_client.conversations.add_message(
        AddMessageInput(
    ...
    if conv_ref:
        ...
        referenced_conv = await memoir_client.conversations.get(conv_id)
    else:
...
@pytest.mark.asyncio
async def test_handles_missing_conversation_ref_gracefully(memoir_client, ctx):
    """
...
@pytest.mark.asyncio
async def test_source_ref_in_facts_points_to_actual_conversation(memoir_client, ctx):
    """
...
@pytest.mark.asyncio
async def test_source_ref_memory_id_points_to_actual_memory(memoir_client, ctx):
    """
    ...
    if source_ref:
        ...
        if mem_id:
            referenced_mem = await memoir_client.vector.get(memory_space_id, mem_id)
...
@pytest.mark.asyncio
async def test_immutable_ref_points_to_actual_immutable_record(memoir_client, ctx):
    """
    ...
    if immut_ref:
        ...
        referenced = await memoir_client.immutable.get(ref_type, ref_id)
...
@pytest.mark.asyncio
async def test_mutable_ref_snapshot_matches_actual_value(memoir_client, ctx):
    """
    ...
    current = await memoir_client.mutable.get(namespace, "theme")
    ...
    if mut_ref:
        ...
        retrieved = await memoir_client.mutable.get(ref_namespace, ref_key)
...
@pytest.mark.asyncio
async def test_context_conversation_ref_points_to_actual_conversation(memoir_client, ctx):
    """
    ...
    ctx_obj = await memoir_client.contexts.create(
        ContextInput(
    ...
    if conv_ref:
        ...
        referenced = await memoir_client.conversations.get(conv_id)
        assert referenced is not None
...
@pytest.mark.asyncio
async def test_child_context_parent_id_points_to_actual_parent(memoir_client, ctx):
    """
    ...
    # Can retrieve parent
    retrieved_parent = await memoir_client.contexts.get(child.parent_id)
    assert retrieved_parent is not None
    assert retrieved_parent.id == parent.id


@pytest.mark.asyncio
async def test_parent_child_ids_array_contains_actual_children(memoir_client, ctx):
    """
    ...
    child1 = await memoir_client.contexts.create(
        ContextInput(
            purpose="Child 1",
            memory_space_id=memory_space_id,
            parent_id=parent.id,
    ...
    child2 = await memoir_client.contexts.create(
        ContextInput(
            purpose="Child 2",
            memory_space_id=memory_space_id,
            parent_id=parent.id,
    ...
    updated_parent = await memoir_client.contexts.get(parent.id)
    ...
    for child_id in child_ids:
        child = await memoir_client.contexts.get(child_id)
...
@pytest.mark.asyncio
async def test_all_contexts_in_chain_reference_same_root(memoir_client, ctx):
    """
    ...
    grandchild = await memoir_client.contexts.create(
        ContextInput(
    ...
    retrieved_root = await memoir_client.contexts.get_root(grandchild.id)
...
@pytest.mark.asyncio
async def test_parent_child_relationship_is_bidirectional(memoir_client, ctx):
    """
    ...
    retrieved_child = await memoir_client.contexts.get(child_ids[0])
...
@pytest.mark.asyncio
async def test_detects_orphaned_contexts_when_parent_deleted(memoir_client, ctx):
    """
...
@pytest.mark.asyncio
async def test_memory_previous_versions_reference_valid_versions(memoir_client, ctx):
    """
    ...
    # Create v1
    v1 = await memoir_client.vector.store(
        memory_space_id,
        StoreMemoryInput(
            content="Version 1",
            content_type="raw",
...
@pytest.mark.asyncio
async def test_conversation_message_ids_are_unique_and_retrievable(memoir_client, ctx):
    """
    ...
    for msg_id in message_ids:
        msg = await memoir_client.conversations.get_message(conv.conversation_id, msg_id)
        assert msg is not None
...
@pytest.mark.asyncio
async def test_user_deletion_cascade_maintains_referential_integrity(memoir_client, ctx):
    """
    ...
    # Create user data
    await memoir_client.users.update(test_user, {"displayName": "Cascade User"})
    ...
    await memoir_client.users.delete(test_user, DeleteUserOptions(cascade=True))

    # Memory should be deleted
    await memoir_client.vector.get(memory_space_id, mem.memory_id)
...
@pytest.mark.asyncio
async def test_fact_version_chain_maintains_integrity(memoir_client, ctx):
    """
    ...
    # Update to v3
    v3 = await memoir_client.facts.update(
        memory_space_id,
        v2.fact_id,
        {"confidence": 95},
    )
    ...
    latest = await memoir_client.facts.get(memory_space_id, v3.fact_id)
