...
@pytest.fixture
async def memoir_client():
    """Create Memoir client for tests."""
    convex_url = os.environ.get("CONVEX_URL")
    client = Memoir(MemoirConfig(convex_url=convex_url))
    yield client
    await client.close()
...
@pytest.fixture
async def setup_memory_space(memoir_client, memory_space_id, test_run_id):
...
class TestArtifactsCRUDParity:
    ...
    async def test_create_artifact_structure(
        ...
        """
        artifact = await memoir_client.artifacts.create(
            CreateArtifactOptions(
    ...
    async def test_get_artifact_returns_none_for_missing(
        ...
        """
        result = await memoir_client.artifacts.get("art-nonexistent-12345")
    ...
    async def test_update_creates_new_version(
        ...
        # Update artifact
        updated = await memoir_client.artifacts.update(
    ...
    async def test_delete_soft_and_hard(
        ...
        # Soft delete
        result = await memoir_client.artifacts.delete(artifact.artifact_id, hard=False)
        assert result.get("deleted") is True

        # Hard delete (cleanup)
        result = await memoir_client.artifacts.delete(artifact.artifact_id, hard=True)
        assert result.get("deleted") is True
...
class TestVersionHistoryParity:
    ...
    async def test_get_history_returns_all_versions(
        ...
        for i in range(2, 5):
            await memoir_client.artifacts.update(artifact.artifact_id, f"v{i}")

        # Get history
        history = await memoir_client.artifacts.get_history(artifact.artifact_id)
    ...
    async def test_get_specific_version(
        ...
        await memoir_client.artifacts.update(artifact.artifact_id, "Version 2 - updated")
        await memoir_client.artifacts.update(artifact.artifact_id, "Version 3 - final")

        # Get specific versions (parity with TS)
        v1 = await memoir_client.artifacts.get_version(artifact.artifact_id, 1)
        v2 = await memoir_client.artifacts.get_version(artifact.artifact_id, 2)
        v3 = await memoir_client.artifacts.get_version(artifact.artifact_id, 3)
        ...
        v99 = await memoir_client.artifacts.get_version(artifact.artifact_id, 99)
        assert v99 is None
...
class TestStreamingParity:
    ...
    async def test_streaming_lifecycle(
        ...
        result = await memoir_client.artifacts.start_streaming(
        ...
        for chunk in chunks:
            ...
            append_result = await memoir_client.artifacts.append_content(
                AppendContentParams(
        ...
        finalize_result = await memoir_client.artifacts.finalize_streaming(
            FinalizeStreamingParams(
                artifact_id=artifact.artifact_id,
                session_id=session_id,
            )
        ...
        finalized = await memoir_client.artifacts.get(artifact.artifact_id)
    ...
    async def test_pause_resume_streaming(
        ...
        # Append some content
        await memoir_client.artifacts.append_content(
            AppendContentParams(
                artifact_id=artifact.artifact_id,
                session_id=session_id,
                chunk="First part. ",
        ...
        paused = await memoir_client.artifacts.pause_streaming(
            artifact.artifact_id, session_id
        )
        assert paused.current_state == "paused"

        # Resume (parity with TS)
        resumed = await memoir_client.artifacts.resume_streaming(
            artifact.artifact_id, session_id
        )
...
class TestListCountParity:
    ...
    async def test_list_with_filters(
        ...
        code_artifacts = await memoir_client.artifacts.list(
            ListArtifactsFilter(
                memory_space_id=setup_memory_space,
                kind="code",
            )
        ...
        tagged = await memoir_client.artifacts.list(
            ListArtifactsFilter(
                memory_space_id=setup_memory_space,
                tags=["list-test"],
            )
    ...
    async def test_count_artifacts(
        ...
        count = await memoir_client.artifacts.count(
            CountArtifactsFilter(
                memory_space_id=setup_memory_space,
                kind="text",
            )
...
class TestErrorHandlingParity:
    ...
    async def test_undo_at_first_version_throws(
        ...
        with pytest.raises(Exception) as exc_info:
            await memoir_client.artifacts.undo(artifact.artifact_id)
    ...
    async def test_redo_at_latest_version_throws(
        ...
        with pytest.raises(Exception) as exc_info:
            await memoir_client.artifacts.redo(artifact.artifact_id)
