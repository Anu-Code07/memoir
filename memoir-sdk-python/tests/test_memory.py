...
from memoir import ForgetOptions, RememberParams, SearchOptions
...
@pytest.mark.asyncio
async def test_remember_basic(memoir_client, test_memory_space_id, test_conversation_id, test_user_id, test_agent_id):
    """Test basic remember operation."""
    result = await memoir_client.memory.remember(
        RememberParams(
...
async def test_remember_with_metadata(
    memoir_client, test_memory_space_id, test_conversation_id, test_user_id, test_agent_id
):
...
@pytest.mark.asyncio
async def test_search(memoir_client, test_memory_space_id, test_conversation_id, test_user_id, test_agent_id):
    """Test memory search."""
    # Store a memory first
    await memoir_client.memory.remember(
        RememberParams(
            memory_space_id=test_memory_space_id,
            conversation_id=test_conversation_id,
    ...
    results = await retry_async(
        lambda: memoir_client.memory.search(
...
@pytest.mark.asyncio
async def test_get_memory(memoir_client, test_memory_space_id, test_conversation_id, test_user_id, test_agent_id):
    ...
    # Get it back
    retrieved = await memoir_client.memory.get(test_memory_space_id, memory_id)
...
async def test_get_with_enrichment(
    ...
    # Get with enrichment
    enriched = await memoir_client.memory.get(
...
@pytest.mark.asyncio
async def test_update_memory(memoir_client, test_memory_space_id, test_conversation_id, test_user_id, test_agent_id):
    ...
    # Update it
    updated = await memoir_client.memory.update(
...
@pytest.mark.asyncio
async def test_delete_memory(memoir_client, test_memory_space_id, test_conversation_id, test_user_id, test_agent_id):
    ...
    # Delete it
    delete_result = await memoir_client.memory.delete(test_memory_space_id, memory_id)
...
async def test_forget_with_conversation(
    ...
    forget_result = await memoir_client.memory.forget(
        test_memory_space_id,
        memory_id,
...
@pytest.mark.asyncio
async def test_count_and_list(memoir_client, test_memory_space_id, test_conversation_id, test_user_id, test_agent_id):
    ...
    # Count them
    count = await memoir_client.memory.count(test_memory_space_id, user_id=test_user_id)
    assert count >= 6  # At least 6 (3 user + 3 agent messages)

    # List them
    memories = await memoir_client.memory.list(
...
@pytest.mark.asyncio
async def test_remember_with_embeddings(memoir_client, test_memory_space_id, test_conversation_id, test_user_id, test_agent_id, cleanup_helper):
    """
...
@pytest.mark.asyncio
async def test_remember_with_fact_extraction(memoir_client, test_memory_space_id, test_conversation_id, test_user_id, test_agent_id, cleanup_helper):
    """
...
@pytest.mark.asyncio
async def test_remember_fact_extraction_parameter_propagation(memoir_client, test_memory_space_id, test_user_id, cleanup_helper):
    """
    ...
    """
    from memoir import ListFactsFilter, RememberParams
    ...
    filtered_facts = await memoir_client.facts.list(
        ListFactsFilter(
            memory_space_id=test_memory_space_id,
            user_id=test_user_id,
        )
...
@pytest.mark.asyncio
async def test_search_with_strategy(memoir_client, test_memory_space_id, test_conversation_id, test_user_id, test_agent_id, cleanup_helper):
    """
...
@pytest.mark.asyncio
async def test_list_with_filters(memoir_client, test_memory_space_id, test_conversation_id, test_user_id, test_agent_id, cleanup_helper):
    """
...
@pytest.mark.asyncio
async def test_count_with_filters(memoir_client, test_memory_space_id, test_conversation_id, test_user_id, test_agent_id, cleanup_helper):
    """
    ...
    count = await memoir_client.memory.count(
        test_memory_space_id,
        user_id=test_user_id,
    )
...
@pytest.mark.asyncio
async def test_forget_with_options(memoir_client, test_memory_space_id, test_conversation_id, test_user_id, test_agent_id, cleanup_helper):
    """
...
@pytest.mark.asyncio
async def test_archive_and_restore(memoir_client, test_memory_space_id, test_conversation_id, test_user_id, test_agent_id, cleanup_helper):
    """
    ...
    # Archive it
    archived = await memoir_client.vector.archive(test_memory_space_id, memory_id)
...
@pytest.mark.asyncio
async def test_get_conversation_ref(memoir_client, test_memory_space_id, test_conversation_id, test_user_id, test_agent_id, cleanup_helper):
    """
    ...
    # Get memory
    memory = await memoir_client.memory.get(test_memory_space_id, memory_id)
...
@pytest.mark.asyncio
async def test_restore_from_archive(memoir_client, test_memory_space_id, test_conversation_id, test_user_id, test_agent_id, cleanup_helper):
    """
    ...
    # Archive it
    archived = await memoir_client.memory.archive(test_memory_space_id, memory_id)
    assert archived is not None
    assert archived.archived is True

    # Verify archived
    memory = await memoir_client.vector.get(test_memory_space_id, memory_id)
    assert "archived" in memory.tags
    assert memory.importance <= 10

    # Restore from archive
    restored = await memoir_client.memory.restore_from_archive(
    ...
    # Verify restoration
    restored_memory = await memoir_client.vector.get(test_memory_space_id, memory_id)
...
@pytest.mark.asyncio
async def test_restore_non_archived_throws_error(memoir_client, test_memory_space_id, test_conversation_id, test_user_id, test_agent_id, cleanup_helper):
    """
    ...
    with pytest.raises(Exception):
        await memoir_client.memory.restore_from_archive(test_memory_space_id, memory_id)
...
from memoir import MemoryMetadata, MemorySource, StoreMemoryInput
from memoir.memory import MemoryValidationError
...
class TestMemoryValidation:
    ...
    any API call is made. They only need the memoir_client to be
    ...
    @pytest.mark.asyncio
    async def test_remember_validation_missing_memory_space_id(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_remember_validation_empty_memory_space_id(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_remember_validation_missing_conversation_id(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_remember_validation_missing_user_message(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_remember_validation_empty_user_message(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_remember_validation_missing_agent_response(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_remember_validation_missing_owner(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_remember_validation_user_without_agent(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_remember_validation_invalid_importance_negative(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_remember_validation_invalid_importance_too_high(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_remember_validation_tags_with_empty_strings(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_remember_stream_validation_invalid_stream(self, memoir_client):
        """Should throw on invalid stream object."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.remember_stream({
    ...
    @pytest.mark.asyncio
    async def test_remember_stream_validation_null_stream(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_remember_stream_validation_inherits_remember_checks(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_forget_validation_empty_memory_space_id(self, memoir_client):
        """Should throw on empty memory_space_id."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.forget("", "mem-123")
        assert "memory_space_id cannot be empty" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_forget_validation_empty_memory_id(self, memoir_client):
        """Should throw on empty memory_id."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.forget(self.SPACE_ID, "   ")
        assert "memory_id cannot be empty" in str(exc_info.value)

    # ━━━ get() validation tests ━━━

    @pytest.mark.asyncio
    async def test_get_validation_empty_memory_space_id(self, memoir_client):
        """Should throw on empty memory_space_id."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.get("", "mem-123")
        assert "memory_space_id cannot be empty" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_get_validation_empty_memory_id(self, memoir_client):
        """Should throw on empty memory_id."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.get(self.SPACE_ID, "")
        assert "memory_id cannot be empty" in str(exc_info.value)

    # ━━━ search() validation tests ━━━

    @pytest.mark.asyncio
    async def test_search_validation_empty_memory_space_id(self, memoir_client):
        """Should throw on empty memory_space_id."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.search("", "query")
        assert "memory_space_id cannot be empty" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_search_validation_empty_query(self, memoir_client):
        """Should throw on empty query."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.search(self.SPACE_ID, "   ")
        assert "query cannot be empty" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_search_validation_invalid_embedding_empty(self, memoir_client):
        """Should throw on empty embedding array."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.search(
                self.SPACE_ID,
                "query",
    ...
    @pytest.mark.asyncio
    async def test_search_validation_invalid_embedding_nan(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_search_validation_invalid_min_score_negative(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_search_validation_invalid_min_score_too_high(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_search_validation_invalid_limit_zero(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_search_validation_invalid_limit_negative(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_search_validation_tags_with_empty_strings(self, memoir_client):
        """Should throw on tags with empty strings."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.search(
                self.SPACE_ID,
    ...
    @pytest.mark.asyncio
    async def test_search_validation_invalid_min_importance(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_store_validation_empty_memory_space_id(self, memoir_client):
        """Should throw on empty memory_space_id."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.store(
                "",
                StoreMemoryInput(
                    content="Test",
                    content_type="raw",
    ...
    @pytest.mark.asyncio
    async def test_store_validation_empty_content(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_store_validation_invalid_content_type(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_store_validation_invalid_source_type(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_store_validation_missing_conversation_ref(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_store_validation_invalid_embedding(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_store_validation_invalid_importance(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_store_validation_tags_with_empty_strings(self, memoir_client):
        """Should throw on tags with empty strings."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.store(
                self.SPACE_ID,
    ...
    @pytest.mark.asyncio
    async def test_update_validation_empty_memory_space_id(self, memoir_client):
        """Should throw on empty memory_space_id."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.update("", "mem-123", {"content": "Updated"})
        assert "memory_space_id cannot be empty" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_update_validation_empty_memory_id(self, memoir_client):
        """Should throw on empty memory_id."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.update(self.SPACE_ID, "", {"content": "Updated"})
        assert "memory_id cannot be empty" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_update_validation_no_update_fields(self, memoir_client):
        """Should throw when no update fields provided."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.update(self.SPACE_ID, "mem-123", {})
        assert "At least one update field must be provided" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_update_validation_invalid_importance(self, memoir_client):
        """Should throw on invalid importance."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.update(self.SPACE_ID, "mem-123", {"importance": -5})
        assert "importance must be between 0 and 100" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_update_validation_invalid_embedding(self, memoir_client):
        """Should throw on invalid embedding."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.update(self.SPACE_ID, "mem-123", {"embedding": []})
        assert "embedding cannot be empty" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_update_validation_tags_with_empty_strings(self, memoir_client):
        """Should throw on tags with empty strings."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.update(self.SPACE_ID, "mem-123", {"tags": ["", "valid"]})
        assert "must be a non-empty string" in str(exc_info.value)

    # ━━━ delete() validation tests ━━━

    @pytest.mark.asyncio
    async def test_delete_validation_empty_memory_space_id(self, memoir_client):
        """Should throw on empty memory_space_id."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.delete("", "mem-123")
        assert "memory_space_id cannot be empty" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_delete_validation_empty_memory_id(self, memoir_client):
        """Should throw on empty memory_id."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.delete(self.SPACE_ID, "")
        assert "memory_id cannot be empty" in str(exc_info.value)

    # ━━━ list() validation tests ━━━

    @pytest.mark.asyncio
    async def test_list_validation_empty_memory_space_id(self, memoir_client):
        """Should throw on empty memory_space_id."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.list("")
        assert "memory_space_id cannot be empty" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_list_validation_invalid_source_type(self, memoir_client):
        """Should throw on invalid source_type."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.list(self.SPACE_ID, source_type="invalid")
        assert "Invalid source_type" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_list_validation_invalid_limit_negative(self, memoir_client):
        """Should throw on negative limit."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.list(self.SPACE_ID, limit=-5)
        assert "limit must be a positive integer" in str(exc_info.value)

    # ━━━ count() validation tests ━━━

    @pytest.mark.asyncio
    async def test_count_validation_empty_memory_space_id(self, memoir_client):
        """Should throw on empty memory_space_id."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.count("")
        assert "memory_space_id cannot be empty" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_count_validation_invalid_source_type(self, memoir_client):
        """Should throw on invalid source_type."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.count(self.SPACE_ID, source_type="invalid")
        assert "Invalid source_type" in str(exc_info.value)
    ...
    @pytest.mark.asyncio
    async def test_update_many_validation_empty_memory_space_id(self, memoir_client):
        """Should throw on empty memory_space_id."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.update_many("", {}, {"importance": 80})
        assert "memory_space_id cannot be empty" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_update_many_validation_no_update_fields(self, memoir_client):
        """Should throw when no update fields provided."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.update_many(self.SPACE_ID, {}, {})
        assert "At least one update field must be provided" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_update_many_validation_invalid_importance(self, memoir_client):
        """Should throw on invalid importance."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.update_many(self.SPACE_ID, {}, {"importance": 200})
        assert "importance must be between 0 and 100" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_update_many_validation_tags_with_empty_strings(self, memoir_client):
        """Should throw on tags with empty strings."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.update_many(self.SPACE_ID, {}, {"tags": ["valid", ""]})
        assert "must be a non-empty string" in str(exc_info.value)

    # ━━━ delete_many() validation tests ━━━

    @pytest.mark.asyncio
    async def test_delete_many_validation_empty_memory_space_id(self, memoir_client):
        """Should throw on empty memory_space_id."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.delete_many("", {})
        assert "memory_space_id cannot be empty" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_delete_many_validation_empty_filter_prevents_mass_delete(self, memoir_client):
        """Should throw on empty filter to prevent mass delete."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.delete_many(self.SPACE_ID, {})
        assert "Filter must include at least one criterion" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_delete_many_validation_invalid_source_type(self, memoir_client):
        """Should throw on invalid source_type."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.delete_many(self.SPACE_ID, {"source_type": "invalid"})
        assert "Invalid source_type" in str(exc_info.value)

    # ━━━ export() validation tests ━━━

    @pytest.mark.asyncio
    async def test_export_validation_empty_memory_space_id(self, memoir_client):
        """Should throw on empty memory_space_id."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.export("", format="json")
        assert "memory_space_id cannot be empty" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_export_validation_invalid_format(self, memoir_client):
        """Should throw on invalid format."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.export(self.SPACE_ID, format="xml")
        assert "Invalid format" in str(exc_info.value)

    # ━━━ archive() validation tests ━━━

    @pytest.mark.asyncio
    async def test_archive_validation_empty_memory_space_id(self, memoir_client):
        """Should throw on empty memory_space_id."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.archive("", "mem-123")
        assert "memory_space_id cannot be empty" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_archive_validation_empty_memory_id(self, memoir_client):
        """Should throw on empty memory_id."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.archive(self.SPACE_ID, "")
        assert "memory_id cannot be empty" in str(exc_info.value)

    # ━━━ restore_from_archive() validation tests ━━━

    @pytest.mark.asyncio
    async def test_restore_validation_empty_memory_space_id(self, memoir_client):
        """Should throw on empty memory_space_id."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.restore_from_archive("", "mem-123")
        assert "memory_space_id cannot be empty" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_restore_validation_empty_memory_id(self, memoir_client):
        """Should throw on empty memory_id."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.restore_from_archive(self.SPACE_ID, "")
        assert "memory_id cannot be empty" in str(exc_info.value)

    # ━━━ get_version() validation tests ━━━

    @pytest.mark.asyncio
    async def test_get_version_validation_empty_memory_space_id(self, memoir_client):
        """Should throw on empty memory_space_id."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.get_version("", "mem-123", 1)
        assert "memory_space_id cannot be empty" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_get_version_validation_empty_memory_id(self, memoir_client):
        """Should throw on empty memory_id."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.get_version(self.SPACE_ID, "", 1)
        assert "memory_id cannot be empty" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_get_version_validation_invalid_version_zero(self, memoir_client):
        """Should throw on version=0."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.get_version(self.SPACE_ID, "mem-123", 0)
        assert "version must be a positive integer" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_get_version_validation_negative_version(self, memoir_client):
        """Should throw on negative version."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.get_version(self.SPACE_ID, "mem-123", -1)
        assert "version must be a positive integer" in str(exc_info.value)

    # ━━━ get_history() validation tests ━━━

    @pytest.mark.asyncio
    async def test_get_history_validation_empty_memory_space_id(self, memoir_client):
        """Should throw on empty memory_space_id."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.get_history("", "mem-123")
        assert "memory_space_id cannot be empty" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_get_history_validation_empty_memory_id(self, memoir_client):
        """Should throw on empty memory_id."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.get_history(self.SPACE_ID, "")
        assert "memory_id cannot be empty" in str(exc_info.value)

    # ━━━ get_at_timestamp() validation tests ━━━

    @pytest.mark.asyncio
    async def test_get_at_timestamp_validation_empty_memory_space_id(self, memoir_client):
        """Should throw on empty memory_space_id."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.get_at_timestamp("", "mem-123", 1000)
        assert "memory_space_id cannot be empty" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_get_at_timestamp_validation_empty_memory_id(self, memoir_client):
        """Should throw on empty memory_id."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.get_at_timestamp(self.SPACE_ID, "", 1000)
        assert "memory_id cannot be empty" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_get_at_timestamp_validation_invalid_timestamp_nan(self, memoir_client):
        """Should throw on NaN timestamp."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.get_at_timestamp(self.SPACE_ID, "mem-123", float('nan'))
        assert "timestamp must be a valid timestamp" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_get_at_timestamp_validation_negative_timestamp(self, memoir_client):
        """Should throw on negative timestamp."""
        with pytest.raises(MemoryValidationError) as exc_info:
            await memoir_client.memory.get_at_timestamp(self.SPACE_ID, "mem-123", -1000)
...
@pytest.mark.asyncio
async def test_remember_validation_agent_only_works(memoir_client, test_memory_space_id, test_conversation_id, test_agent_id, cleanup_helper):
...
@pytest.mark.asyncio
async def test_skip_layers_conversations(memoir_client, test_memory_space_id, test_conversation_id, test_agent_id, cleanup_helper):
    ...
    conversation = await memoir_client.conversations.get(test_conversation_id)
...
@pytest.mark.asyncio
async def test_skip_layers_vector(memoir_client, test_memory_space_id, test_conversation_id, test_user_id, test_agent_id, cleanup_helper):
...
@pytest.mark.asyncio
async def test_skip_layers_facts(memoir_client, test_memory_space_id, test_conversation_id, test_user_id, test_agent_id, cleanup_helper):
...
@pytest.mark.asyncio
async def test_skip_layers_multiple(memoir_client, test_memory_space_id, test_conversation_id, test_agent_id, cleanup_helper):
...
@pytest.mark.asyncio
async def test_skip_layers_users(memoir_client, test_memory_space_id, test_conversation_id, test_user_id, test_agent_id, cleanup_helper):
...
@pytest.mark.asyncio
async def test_skip_layers_agents(memoir_client, test_memory_space_id, test_conversation_id, test_user_id, cleanup_helper):
...
@pytest.mark.asyncio
async def test_no_skip_layers_default_behavior(memoir_client, test_memory_space_id, test_conversation_id, test_user_id, test_agent_id, cleanup_helper):
