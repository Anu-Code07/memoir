...
class TestA2ACoreSend:
    ...
    async def test_send_basic_message_between_agents(
        self, memoir_client, test_ids, cleanup_helper
    ):
        """Should send basic message between agents."""
        prefix = f"a2a-send-{int(time.time() * 1000)}"

        result = await memoir_client.a2a.send(
            A2ASendParams(
    ...
    async def test_send_creates_bidirectional_memories(
        ...
        sender_memory = await memoir_client.vector.get(agent1, result.sender_memory_id)
        assert sender_memory is not None
        assert "Sent to" in sender_memory.content

        # Verify receiver memory exists
        receiver_memory = await memoir_client.vector.get(
    ...
    async def test_send_links_to_user_id_when_provided(
        ...
        memory = await memoir_client.vector.get(agent1, result.sender_memory_id)
...
class TestA2ACoreBroadcast:
    ...
    async def test_broadcast_to_multiple_agents(
        ...
        result = await memoir_client.a2a.broadcast(
            A2ABroadcastParams(
                from_agent=sender,
...
class TestA2ACoreGetConversation:
    ...
    async def test_get_conversation_retrieves_conversation(
        ...
        convo = await memoir_client.a2a.get_conversation(agent1, agent2)
    ...
    async def test_get_conversation_with_filters_object(
        ...
        convo = await memoir_client.a2a.get_conversation(agent1, agent2, filters=filters)
    ...
    async def test_get_conversation_applies_importance_filters(
        ...
        convo = await memoir_client.a2a.get_conversation(
...
class TestA2ACoreRequest:
    ...
    async def test_request_throws_explaining_pubsub_requirement(
        ...
        with pytest.raises(A2ATimeoutError) as exc_info:
            await memoir_client.a2a.request(
                A2ARequestParams(
...
class TestA2AClientSideValidation:
    ...
    @pytest.mark.asyncio
    async def test_send_throws_on_missing_from_agent(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_send_throws_on_missing_to_agent(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_send_throws_on_empty_message(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_send_throws_on_whitespace_only_message(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_send_throws_on_message_too_large(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_send_throws_on_invalid_importance_negative(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_send_throws_on_invalid_importance_over_100(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_send_throws_on_invalid_agent_id_format(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_send_throws_when_from_equals_to(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_broadcast_throws_on_empty_recipients_array(self, memoir_client):
        """Should throw on empty recipients array."""
        with pytest.raises(A2AValidationError) as exc_info:
            await memoir_client.a2a.broadcast(
                A2ABroadcastParams(
                    from_agent="sender",
                    to_agents=[],
                    message="test",
    ...
    @pytest.mark.asyncio
    async def test_broadcast_throws_on_duplicate_recipients(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_broadcast_throws_on_too_many_recipients(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_broadcast_throws_on_invalid_recipient_id(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_broadcast_throws_when_sender_in_recipients(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_get_conversation_throws_when_since_after_until(self, memoir_client):
        """Should throw when since > until."""
        now = int(time.time() * 1000)
        yesterday = now - 24 * 60 * 60 * 1000

        with pytest.raises(A2AValidationError) as exc_info:
            await memoir_client.a2a.get_conversation(
    ...
    @pytest.mark.asyncio
    async def test_get_conversation_throws_on_limit_zero(self, memoir_client):
        """Should throw on limit = 0."""
        with pytest.raises(A2AValidationError) as exc_info:
            await memoir_client.a2a.get_conversation("agent-1", "agent-2", limit=0)

        assert exc_info.value.code == "INVALID_LIMIT"

    @pytest.mark.asyncio
    async def test_get_conversation_throws_on_limit_over_1000(self, memoir_client):
        """Should throw on limit > 1000."""
        with pytest.raises(A2AValidationError) as exc_info:
            await memoir_client.a2a.get_conversation("agent-1", "agent-2", limit=1001)

        assert exc_info.value.code == "INVALID_LIMIT"

    @pytest.mark.asyncio
    async def test_get_conversation_throws_on_negative_offset(self, memoir_client):
        """Should throw on negative offset."""
        with pytest.raises(A2AValidationError) as exc_info:
            await memoir_client.a2a.get_conversation("agent-1", "agent-2", offset=-1)

        assert exc_info.value.code == "INVALID_OFFSET"

    @pytest.mark.asyncio
    async def test_get_conversation_throws_on_empty_agent1(self, memoir_client):
        """Should throw on empty agent1."""
        with pytest.raises(A2AValidationError) as exc_info:
            await memoir_client.a2a.get_conversation("", "agent-2")

        assert exc_info.value.code == "INVALID_AGENT_ID"

    @pytest.mark.asyncio
    async def test_get_conversation_throws_on_empty_agent2(self, memoir_client):
        """Should throw on empty agent2."""
        with pytest.raises(A2AValidationError) as exc_info:
            await memoir_client.a2a.get_conversation("agent-1", "")
    ...
    @pytest.mark.asyncio
    async def test_request_throws_on_invalid_timeout_too_low(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_request_throws_on_invalid_timeout_too_high(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_request_throws_on_invalid_retries_negative(self, memoir_client):
    ...
    @pytest.mark.asyncio
    async def test_request_throws_on_invalid_retries_too_high(self, memoir_client):
...
class TestA2AIntegration:
    ...
    async def test_integrate_with_memory_api_search(
        ...
        from memoir import SearchOptions
...
class TestA2AEdgeCases:
    ...
    async def test_handles_special_characters_emoji_unicode(
        ...
        memory = await memoir_client.vector.get(f"{prefix}-1", result.sender_memory_id)
        assert memory is not None
    ...
    async def test_handles_boundary_importance_values(
        ...
        # Test importance = 0
        result0 = await memoir_client.a2a.send(
            A2ASendParams(
        ...
        # Test importance = 100
        result100 = await memoir_client.a2a.send(
            A2ASendParams(
...
@pytest.mark.asyncio
async def test_agent_to_agent_conversation(memoir_client, test_ids, cleanup_helper):
    """
    ...
    # Get conversation
    retrieved = await memoir_client.conversations.get(conversation_id)
...
@pytest.mark.asyncio
async def test_agent_memory_sharing(memoir_client, test_ids, cleanup_helper):
    """
    ...
    retrieved = await memoir_client.vector.get(memory_space_id, mem1.memory_id)
...
@pytest.mark.asyncio
async def test_agent_separate_spaces(memoir_client, test_ids, cleanup_helper):
    """
    ...
    _mem2 = await memoir_client.vector.store(
        space_2,
        StoreMemoryInput(
    ...
    result = await memoir_client.vector.get(space_2, mem1.memory_id)
