...
class TestVectorMemoryIsolation:
    ...
    async def test_cannot_get_from_wrong_space(self, memoir_client, space_a, space_b):
        """Test cannot get memory from wrong space."""
        mem_a = await memoir_client.vector.store(
            space_a,
            StoreMemoryInput(
        ...
        result = await memoir_client.vector.get(space_b, mem_a.memory_id)
        assert result is None

    async def test_list_never_returns_wrong_space_data(self, memoir_client, space_a, space_b):
        ...
        # List space A
        list_a = await memoir_client.vector.list(memory_space_id=space_a)

        for mem in list_a:
            assert mem.memory_space_id == space_a
            assert "MARKER_SPACE_B" not in mem.content

    async def test_search_respects_space_boundaries(self, memoir_client, space_a, space_b):
        ...
        results = await retry_async(
            lambda: memoir_client.vector.search(space_b, "UNIQUE_SEARCH_MARKER"),
            max_retries=3,
        )
...
class TestConversationIsolation:
    ...
    async def test_list_only_returns_correct_space(self, memoir_client, space_a, space_b):
        ...
        list_a = await memoir_client.conversations.list(ListConversationsFilter(memory_space_id=space_a))
        list_b = await memoir_client.conversations.list(ListConversationsFilter(memory_space_id=space_b))
...
class TestFactsIsolation:
    ...
    async def test_cannot_get_fact_from_wrong_space(self, memoir_client, space_a, space_b):
        """Test cannot get fact from wrong space."""
        fact_a = await memoir_client.facts.store(
            StoreFactParams(
                memory_space_id=space_a,
                fact="Space A fact",
                fact_type="knowledge",
        ...
        result = await memoir_client.facts.get(space_b, fact_a.fact_id)
        assert result is None
...
class TestStatisticsIsolation:
    ...
    async def test_stats_independent_per_space(self, memoir_client, space_a, space_b):
        ...
        stats_a = await memoir_client.memory_spaces.get_stats(space_a)
...
class TestBulkOperationIsolation:
    ...
    async def test_delete_many_only_affects_specified_space(self, memoir_client, space_a, space_b):
        ...
        mem_b = await memoir_client.vector.store(
            space_b,
            StoreMemoryInput(
                content="Bulk B",
                content_type="raw",
        ...
        for mem in list_a:
            if "bulk-iso" in mem.tags:
                await memoir_client.vector.delete(space_a, mem.memory_id)

        # Space B memory should still exist
        check_b = await memoir_client.vector.get(space_b, mem_b.memory_id)
