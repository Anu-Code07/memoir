...
from memoir.facts import StoreFactWithDedupOptions
...
@pytest.mark.asyncio
async def test_store_with_dedup_stores_first_fact(ctx, memoir_client, scoped_cleanup):
    """
    ...
    result = await memoir_client.facts.store_with_dedup(
        StoreFactParams(
...
@pytest.mark.asyncio
async def test_store_with_dedup_detects_exact_duplicate(ctx, memoir_client, scoped_cleanup):
    """
    ...
    # Store first fact
    first_result = await memoir_client.facts.store_with_dedup(
        StoreFactParams(
            memory_space_id=memory_space_id,
            fact=unique_fact,
            fact_type="preference",
    ...
    # Store duplicate
    second_result = await memoir_client.facts.store_with_dedup(
        StoreFactParams(
            memory_space_id=memory_space_id,
...
@pytest.mark.asyncio
async def test_store_with_dedup_structural_match(ctx, memoir_client, scoped_cleanup):
    """
    ...
    # Store first fact
    await memoir_client.facts.store_with_dedup(
        StoreFactParams(
            memory_space_id=memory_space_id,
...
@pytest.mark.asyncio
async def test_store_with_dedup_disabled(ctx, memoir_client, scoped_cleanup):
    """
    ...
    facts = await memoir_client.facts.list(
        ListFactsFilter(
            memory_space_id=memory_space_id,
            subject="user-789",
        )
...
@pytest.mark.asyncio
async def test_cross_session_deduplication(ctx, memoir_client, scoped_cleanup):
    """
...
@pytest.mark.asyncio
async def test_memory_space_isolation(ctx, memoir_client, scoped_cleanup):
    """
...
async def test_remember_defaults_to_semantic_fallback_structural(
    ctx, memoir_client, scoped_cleanup
):
    """
...
@pytest.mark.asyncio
async def test_remember_with_deduplication_disabled(ctx, memoir_client, scoped_cleanup):
    """
...
@pytest.mark.asyncio
async def test_real_world_name_stated_multiple_times(ctx, memoir_client, scoped_cleanup):
    """
