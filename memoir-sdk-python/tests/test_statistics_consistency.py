...
class TestMemorySpaceStatsConsistency:
    ...
    async def test_stats_match_conversation_count(self, memoir_client):
        ...
        stats = await memoir_client.memory_spaces.get_stats(space_id)
        direct_count = await memoir_client.conversations.count(CountConversationsFilter(memory_space_id=space_id))

        assert stats.total_conversations == direct_count

    async def test_stats_match_memory_count(self, memoir_client):
        ...
        stats = await memoir_client.memory_spaces.get_stats(space_id)
        direct_count = await memoir_client.vector.count(memory_space_id=space_id)
...
class TestCountMatchesListLength:
    ...
    async def test_vector_count_matches_list(self, memoir_client):
        ...
        count = await memoir_client.vector.count(memory_space_id=space_id)
        mem_list = await memoir_client.vector.list(memory_space_id=space_id)

        assert count == len(mem_list)

    async def test_facts_count_matches_list(self, memoir_client):
        ...
        from memoir.types import CountFactsFilter, ListFactsFilter
        count = await memoir_client.facts.count(CountFactsFilter(memory_space_id=space_id))
        fact_list = await memoir_client.facts.list(ListFactsFilter(memory_space_id=space_id))
...
class TestStatsUpdateImmediately:
    ...
    async def test_creating_conversation_increments_stats(self, memoir_client):
        ...
        before = await memoir_client.memory_spaces.get_stats(space_id)
        ...
        after = await memoir_client.memory_spaces.get_stats(space_id)

        assert after.total_conversations == before.total_conversations + 1

    async def test_creating_memory_increments_stats(self, memoir_client):
...
class TestBulkOperationsStats:
    ...
    async def test_delete_many_result_matches_count_change(self, memoir_client):
        ...
        before = await memoir_client.vector.count(memory_space_id=space_id)

        # Delete manually (delete_many doesn't support tags)
        to_delete = [m for m in await memoir_client.vector.list(memory_space_id=space_id) if "bulk-del-stats" in m.tags]
        for mem in to_delete:
            await memoir_client.vector.delete(space_id, mem.memory_id)
        result = {"deleted": len(to_delete)}

        after = await memoir_client.vector.count(memory_space_id=space_id)
