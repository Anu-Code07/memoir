...
async def hive_space_fixture(module_memoir_client, test_run_context):
    ...
    await module_memoir_client.memory_spaces.register(
        RegisterMemorySpaceParams(
...
@pytest.mark.asyncio
async def test_all_participants_see_same_conversations(memoir_client, hive_space_fixture):
    """
    ...
    all_convs = await memoir_client.conversations.list(ListConversationsFilter(memory_space_id=hive_space))
...
@pytest.mark.asyncio
async def test_all_participants_contribute_to_shared_memory_pool(memoir_client, hive_space_fixture):
    """
    ...
    result = await memoir_client.vector.list(memory_space_id=hive_space)
...
@pytest.mark.asyncio
async def test_all_participants_contribute_to_shared_fact_base(memoir_client, hive_space_fixture):
    """
    ...
    all_facts_result = await memoir_client.facts.list(ListFactsFilter(memory_space_id=hive_space))
...
@pytest.mark.asyncio
async def test_facts_about_same_subject_from_different_tools(memoir_client, hive_space_fixture):
    """
    ...
    user_facts_result = await memoir_client.facts.query_by_subject(
...
@pytest.mark.asyncio
async def test_single_memory_space_eliminates_duplication(memoir_client, hive_space_fixture):
    """
    ...
    from memoir.types import QueryBySubjectFilter
    facts_result = await memoir_client.facts.query_by_subject(
        QueryBySubjectFilter(
            memory_space_id=hive_space,
            subject="user-alice"
        )
...
@pytest.mark.asyncio
async def test_tracks_which_participant_created_what(memoir_client, hive_space_fixture):
    """
    ...
    conv2 = await memoir_client.conversations.create(
        CreateConversationInput(
            memory_space_id=hive_space,
            type="user-agent",
            participants=ConversationParticipants(
...
@pytest.mark.asyncio
async def test_multi_tool_coordination_for_user_workflow(memoir_client, hive_space_fixture):
    """
    ...
    agent_conv = await memoir_client.conversations.create(
        CreateConversationInput(
            memory_space_id=hive_space,
            type="user-agent",
            participants=ConversationParticipants(
    ...
    user_msg = await memoir_client.conversations.add_message(
        AddMessageInput(
    ...
    meetings_result = await memoir_client.facts.search(hive_space, "meetings")
    ...
    stats = await memoir_client.memory_spaces.get_stats(hive_space)

    assert stats.total_conversations >= 1
    assert stats.total_facts >= 1

    # 6. Verify all participants tracked
    space = await memoir_client.memory_spaces.get(hive_space)
...
@pytest.mark.asyncio
async def test_single_query_retrieves_all_participant_memories(memoir_client, hive_space_fixture):
    """
...
@pytest.mark.asyncio
async def test_participant_id_persists_through_vector_update(memoir_client, hive_space_fixture):
    """
    ...
    # Verify in database
    stored = await memoir_client.vector.get(hive_space, mem.memory_id)
...
@pytest.mark.asyncio
async def test_tracks_5_plus_participants_in_same_hive(memoir_client, hive_space_fixture):
    """
...
@pytest.mark.asyncio
async def test_multiple_participants_use_remember_in_same_hive(memoir_client, hive_space_fixture):
    """
...
@pytest.mark.asyncio
async def test_message_participant_id_for_agent_messages(memoir_client, hive_space_fixture):
    """
    ...
    updated_conv = await memoir_client.conversations.get(conv.conversation_id)
    agent_msgs = [
...
@pytest.mark.asyncio
async def test_facts_participant_id_persists_through_update(memoir_client, hive_space_fixture):
    """
    ...
    # Verify in database
    stored = await memoir_client.facts.get(hive_space, v2.fact_id)
    stored_participant = stored.participant_id if hasattr(stored, 'participant_id') else stored.get("participantId")
    assert stored_participant == "agent-assistant"


@pytest.mark.asyncio
async def test_can_distinguish_memories_by_participant_in_search(memoir_client, hive_space_fixture):
    """
    ...
    from memoir.types import SearchOptions
...
@pytest.mark.asyncio
async def test_contexts_track_participants_correctly(memoir_client, hive_space_fixture):
    """
...
@pytest.mark.asyncio
async def test_participant_statistics_accurate(memoir_client, hive_space_fixture):
    """
...
@pytest.mark.asyncio
async def test_participant_id_in_all_layers_for_end_to_end_workflow(memoir_client, hive_space_fixture):
    """
    ...
    conv_check = await memoir_client.conversations.get(result.conversation["conversationId"])
    # Just verify conversation exists
    assert conv_check is not None

    mem = await memoir_client.vector.get(hive_space, result.memories[0].memory_id)
...
@pytest.mark.asyncio
async def test_handles_undefined_participant_id_across_all_operations(memoir_client, hive_space_fixture):
    """
    ...
    stored_mem = await memoir_client.vector.get(hive_space, mem.memory_id)
    assert stored_mem is not None

    stored_fact = await memoir_client.facts.get(hive_space, fact.fact_id)
