...
All notable changes to the Memoir Python SDK will be documented in this file.
...
```python
from memoir import RecallLimits
...
- `MEMOIR_RECALL_LIMIT_MEMORIES` (default: 20)
- `MEMOIR_RECALL_LIMIT_FACTS` (default: 15)
- `MEMOIR_RECALL_GRAPH_HOPS` (default: 2)
- `MEMOIR_RECALL_GRAPH_ENTITIES_PER_HOP` (default: 5)
- `MEMOIR_RECALL_GRAPH_RESULTS_PER_ENTITY` (default: 3)
- `MEMOIR_RECALL_LIMIT_TOTAL` (default: 30)
...
```python
facts = await memoir.facts.semantic_search(
    memory_space_id="user-123-space",
...
New `memoir/config.py` module with:
...
### New Auth Module (`memoir.auth`)

```python
from memoir.auth import create_auth_context
from memoir import AuthContext, AuthMethod
...
```python
session = await memoir.sessions.create(CreateSessionParams(
    user_id='user-123',
    tenant_id='tenant-456',
    metadata={'device': 'Chrome on macOS'},
))

await memoir.sessions.touch(session.session_id)
active = await memoir.sessions.get_active('user-123')
await memoir.sessions.end(session.session_id)
```
