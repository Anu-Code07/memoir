...
export interface AppConfig {
  ...
  projectPath: string;
  /** Whether this app should be started with `memoir start` */
  enabled: boolean;
...
/**
 * CLI configuration stored in ~/.memoirrc or ./memoir.config.json
 */
...
export interface DeploymentConfig {
   ...
   * Allows running `memoir start -d <name>` from anywhere.
   * Not needed for remote-only deployments (e.g., Vercel).
   */
  projectPath?: string;
  /**
   * Whether this deployment should be started with `memoir start`.
