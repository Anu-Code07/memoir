...
export function registerStatusCommands(
  ...
  program
    .command("status")
    .description("Show Memoir setup status dashboard for all deployments")
    .option(
      "-d, --deployment <name>",
  ...
  if (deployment.projectPath) {
    ...
    if (status.projectExists) {
      ...
      if (dockerComposeExists) {
        ...
        try {
          const containerName = `memoir-${status.graphType || "neo4j"}`;
...
function displayMultiDeploymentDashboard(
  ...
  console.log(
    pc.cyan("║") +
      pc.bold("   Memoir Status Dashboard").padEnd(width) +
      pc.cyan("║"),
  );
  console.log(pc.cyan("╚" + line + "╝"));
  console.log();
  ...
  if (status.deployments.length === 0) {
    ...
    console.log();
    console.log(pc.yellow("   Run 'memoir init' to set up a new project"));
    console.log(
      pc.yellow("   Or 'memoir config add-deployment' to add existing"),
    );
    console.log();
  } else {
  ...
  console.log(
    pc.dim("   memoir start -d <name>") +
      pc.dim("  # Start specific deployment"),
  );
  console.log(
    pc.dim("   memoir config list") + pc.dim("      # List all deployments"),
  );
  console.log(
    pc.dim("   memoir config enable <name>") +
