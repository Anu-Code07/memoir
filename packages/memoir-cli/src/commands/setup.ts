...
export function registerConfigCommands(
    ...
    .action(async () => {
      try {
        ...
        if (deployments.length === 0) {
          ...
          console.log(pc.dim("   • Run 'memoir init' to create a new project"));
          console.log(
            pc.dim(
              "   • Run 'memoir config add-deployment' to add an existing deployment",
            ),
          );
          return;
        }
        ...
        if (config.default) {
          console.log(
            pc.dim(
              `  Default: ${config.default} (→) • Enabled deployments started with 'memoir start'`,
            ),
          );
        }
        ...
        if (apps.length > 0) {
          ...
          console.log();
          console.log(pc.dim("  Enabled apps started with 'memoir start'"));
        }
  ...
  configCmd
    ...
    .action(async (deploymentName, pathArg) => {
      try {
        ...
        console.log();
        printInfo(`You can now run: memoir start -d ${deploymentName}`);
  ...
  // config enable - Enable a deployment or app for `memoir start`
  configCmd
    .command("enable <name>")
    .description(
    ...
    .action(async (name) => {
      try {
        ...
        if (config.deployments[name]) {
          ...
          console.log(pc.dim("   Will be started with 'memoir start'"));
          return;
        }
    ...
    .action(async (name) => {
      try {
        ...
        if (config.deployments[name]) {
          ...
          console.log(pc.dim("   Will NOT be started with 'memoir start'"));
          return;
        }
  ...
  configCmd
    ...
    .action(async (options) => {
      try {
        ...
        if (deployments.length === 0) {
          printWarning(
            "No deployments configured. Run 'memoir setup' to configure.",
          );
          return;
        }
    ...
    .action(async (nameArg, options) => {
      try {
        ...
        if (!options.jsonOnly) {
          ...
          printSuccess(`Added deployment "${name}" to ~/.memoirrc`);
        }
  ...
  configCmd
    ...
    .option(
      "--json-only",
      "Only remove from ~/.memoirrc (skip .env.local)",
      false,
    )
    .action(async (nameArg, options) => {
      try {
        ...
        // Remove from user config (~/.memoirrc)
        ...
        if (!options.jsonOnly) {
          ...
          printSuccess(`Removed deployment "${name}" from ~/.memoirrc`);
        }
      } catch (error) {
        printError(
  ...
  configCmd
    ...
    .option("--json-only", "Only update ~/.memoirrc (skip .env.local)", false)
      ...
      try {
        ...
        if (!name) {
          ...
          if (deploymentNames.length === 0) {
            ...
            console.log(
              pc.dim("   Run 'memoir config add-deployment' to add one\n"),
            );
            return;
          }
        ...
        if (!options.jsonOnly) {
          ...
          printSuccess(`Set deploy key for "${name}" in ~/.memoirrc`);
        }
      } catch (error) {
        printError(
    ...
    .action(async (deploymentArg, options) => {
      try {
        ...
        if (!options.jsonOnly) {
          ...
          printSuccess(`Updated URL for "${name}" in ~/.memoirrc`);
          printInfo(`${oldUrl} → ${url}`);
        }
      } catch (error) {
        printError(
    ...
    .action(async (deploymentName, options) => {
      try {
        ...
        if (!deploymentName) {
          ...
          console.log();
          console.log(pc.dim("  Usage: memoir use <deployment>"));
          console.log(pc.dim("         memoir use --clear"));
          console.log();
          return;
        }
