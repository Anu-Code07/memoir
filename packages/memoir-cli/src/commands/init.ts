 ...
 * Init Command
 *
 * Interactive wizard for setting up a new Memoir project.
 * Replaces `npx create-memoir-memories` with a more integrated CLI experience.
 */
    ...
    .action(async (options) => {
      ...
      if (options.deployment) {
        ...
        if (!deployment) {
          ...
          console.log(
            pc.dim("   Run 'memoir config list' to see all deployments"),
          );
          process.exit(1);
        }
        ...
        if (deployment.projectPath && !fs.existsSync(projectPath)) {
          ...
          console.log(
            pc.dim(
              "   Run 'memoir config set-path <deployment> <path>' to update",
            ),
          );
          process.exit(1);
        }
        ...
        for (const [name, deployment] of Object.entries(config.deployments)) {
          ...
          if (!deployment.projectPath) {
            ...
            console.log(
              pc.dim(
                `   Run 'memoir config set-path ${name} <path>' to configure\n`,
              ),
            );
            continue;
          }
      ...
      if (deploymentsToStart.length === 0) {
        ...
        console.log(
          pc.dim("   Run 'memoir config enable <name>' to enable a deployment"),
        );
        console.log(
          pc.dim("   Or use 'memoir start -d <name>' to start a specific one"),
        );
        process.exit(0);
      }
      ...
      if (options.foreground && deploymentsToStart.length > 1) {
        ...
        console.log(
          pc.dim("   Use 'memoir start -d <name> -f' for foreground mode"),
        );
        process.exit(1);
      }
      ...
      console.log(pc.bold("  Memoir Start"));
      console.log(
        pc.bold(
          "  ═══════════════════════════════════════════════════════════",
        ),
      ...
      if (!options.foreground) {
        ...
        console.log();
        console.log(pc.dim("  Use 'memoir stop' to stop all services"));
        console.log(
          pc.dim("  Use 'memoir config list' to see deployment status"),
        );
        console.log();
      }
    });
    ...
    .action(async (options) => {
      ...
      if (options.app) {
        ...
        if (!appConfig) {
          ...
          if (!matchingApp) {
            ...
            console.log(
              pc.dim("   Run 'memoir config list' to see available apps\n"),
            );
            process.exit(1);
          }
      ...
      if (options.deployment) {
        ...
        if (!deployment) {
          ...
          console.log(
            pc.dim(
              "   Run 'memoir config list' to see available deployments\n",
            ),
          );
          process.exit(1);
        }
...
export function registerInitCommand(
  ...
  program
    ...
    .description("Initialize a new Memoir project")
...
async function getInstallationType(
  ...
  if (exists && !isEmpty) {
    const response = await prompts({
      type: "confirm",
      name: "addToExisting",
      message: `Directory ${path.basename(projectPath)} already exists. Add Memoir to existing project?`,
      initial: true,
    });
...
async function getTemplateChoice(
  ...
  const response = await prompts({
    ...
    choices: [
      {
        title: "Basic",
        description: "Interactive CLI + HTTP API server for learning Memoir",
        value: "basic",
      },
      {
...
async function getConvexSetup(
  ...
  console.log(pc.dim("   Memoir uses Convex as its backend database\n"));
...
async function executeSetup(
  ...
  try {
    ...
    if (config.templateChoice === "chat-sdk") {
      ...
      // Copy Memoir backend functions (chat-sdk template only has schema)
      const backendSpinner = ora(
        "Setting up Memoir backend functions...",
      ).start();
      await deployMemoirBackend(config.projectPath);
      backendSpinner.succeed("Memoir backend files ready");
    } else {
      ...
      // Copy Memoir backend functions FIRST (before convex dev)
    ...
    // Save deployment to user config (~/.memoirrc)
...
/**
 * Save deployment configuration to ~/.memoirrc
 */
async function saveDeploymentToConfig(config: WizardConfig): Promise<void> {
  ...
  try {
    ...
    console.log(
      pc.dim("   Run 'memoir config add-deployment' to add manually"),
    );
  }
}
...
async function createChatSDKEnvFile(
  ...
  let envContent = `# Memoir Chat SDK Environment
# Generated by memoir init
...
async function addCLIScripts(projectPath: string): Promise<void> {
  ...
  try {
    ...
    packageJson.scripts["memoir"] = "memoir";
    packageJson.scripts["memoir:setup"] = "memoir setup";
    packageJson.scripts["memoir:stats"] = "memoir db stats";
    packageJson.scripts["memoir:spaces"] = "memoir spaces list";
    packageJson.scripts["memoir:status"] = "memoir status";
...
function showSuccessMessage(config: WizardConfig): void {
  console.log(
    pc.bold(pc.green("\n   Memoir successfully initialized!\n")),
  );
  ...
  console.log(pc.bold("\n   CLI Commands:\n"));
  console.log(
    pc.cyan("   memoir status") +
      pc.dim("        # View setup status dashboard"),
  );
  console.log(
    pc.cyan("   memoir db stats") + pc.dim("      # View database statistics"),
  );
  console.log(
    pc.cyan("   memoir spaces list") + pc.dim("   # List memory spaces"),
  );
  console.log(
    pc.cyan("   memoir --help") + pc.dim("        # See all CLI commands"),
  );

  console.log(pc.bold("\n   Learn more:\n"));
  console.log(
    pc.dim(
      "   Documentation: https://github.com/Anu-Code07/memoir/tree/main/Documentation",
    ),
  );
  console.log(
    pc.dim(
      "   Examples:      https://github.com/Anu-Code07/memoir/tree/main/Examples",
    ),
  );

  console.log(pc.bold(pc.cyan("\n   Happy building with Memoir!\n")));
}
...
async function showRunningStatus(
  ...
  console.log(
    pc.cyan("║") +
      pc.bold("   Memoir Development Environment").padEnd(width) +
      pc.cyan("║"),
  );
  console.log(pc.cyan("╚" + line + "╝"));
  console.log();
  ...
  if (isLocal) {
    ...
    if (convexRunning) {
      ...
      console.log(pc.dim("     Run: memoir start"));
    }
  } else {
  ...
  if (hasGraphConfig) {
    ...
    try {
      const result = await execCommand(
        "docker",
        ["ps", "--filter", "name=memoir-neo4j", "--format", "{{.Status}}"],
        { quiet: true },
      );
  ...
  if (templateChoice !== "basic") {
    ...
    const appDescription =
      templateChoice === "chat-sdk"
        ? "Full-featured chat app with Memoir memory"
  ...
  console.log(pc.dim("  " + thinLine));
  console.log(
    pc.dim("   memoir status") + pc.dim("           # Full status dashboard"),
  );
  if (isLocal) {
    console.log(
      pc.dim("   memoir start -f") +
