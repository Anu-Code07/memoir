...
export function registerDeployCommands(
  ...
  program
    ...
    .option(
      "--dev",
      "Use dev mode (link to local SDK via MEMOIR_SDK_DEV_PATH)",
      false,
    )
    .option(
      "--sync-template",
    ...
    .option("--sdk-version <version>", "Specific Memoir SDK version to install")
    ...
    .action(async (options) => {
      ...
      // Check for dev mode
      const devPath = process.env.MEMOIR_SDK_DEV_PATH;
      const isDevMode = options.dev || !!devPath;

      if (isDevMode && !devPath) {
        console.log(
          pc.red(
            "\n   Dev mode requires MEMOIR_SDK_DEV_PATH environment variable",
          ),
        );
        console.log(
          pc.dim("   Set it to the path of your local Project-Memoir repo:"),
        );
        console.log(
          pc.dim("   export MEMOIR_SDK_DEV_PATH=/path/to/Project-Memoir\n"),
        );
        return;
      }
      ...
      if (options.app) {
        ...
        if (!app) {
          ...
          if (appNames.length > 0) {
            ...
            console.log(
              pc.dim("   No apps configured. Run 'memoir init' to add one."),
            );
          }
          return;
        }
      ...
      if (
        ...
        console.log(pc.red("\n   Nothing to update"));
        console.log(
          pc.dim("   Run 'memoir init' to configure deployments and apps\n"),
        );
        return;
      }
...
async function updateDeployment(
  ...
  try {
    // Get current Memoir SDK version
    let currentSdkVersion = "not installed";
    try {
      const result = await execCommand(
        "npm",
    ...
    // Get latest Memoir SDK version from npm
    let latestSdkVersion = "unknown";
    try {
      const result = await execCommand(
        "npm",
    ...
    // Get Memoir SDK's peer dependency on Convex
    let sdkConvexPeerDep = "unknown";
    try {
      const result = await execCommand(
        "npm",
