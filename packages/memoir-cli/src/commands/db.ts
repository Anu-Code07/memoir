...
export function registerDbCommands(program: Command, _config: CLIConfig): void {
  ...
  db.command("clear")
    ...
    .action(async (options) => {
      ...
      try {
        ...
        // Check if graph sync is enabled (same logic as Memoir.create())
        ...
        if (process.env.DEBUG || program.opts().debug) {
          console.log(
            pc.dim(
              `  [DEBUG] MEMOIR_GRAPH_SYNC=${process.env.MEMOIR_GRAPH_SYNC}`,
            ),
          );
          console.log(
        ...
        await withClient(
          ...
          async (client) => {
            ...
            // Check both explicit flag and auto-detection (same logic as Memoir.create())
            const neo4jUri = process.env.NEO4J_URI;
            const memgraphUri = process.env.MEMGRAPH_URI;
            const graphSyncEnabled =
              process.env.MEMOIR_GRAPH_SYNC === "true" ||
  ...
  db.command("backup")
    ...
    .option("-d, --deployment <name>", "Target deployment")
    .option("-o, --output <file>", "Output file path", "memoir-backup.json")
  ...
  db.command("export")
    ...
    .option("-d, --deployment <name>", "Target deployment")
    .option("-o, --output <file>", "Output file path", "memoir-export.json")
    .action(async (options) => {
      const currentConfig = await loadConfig();
      const selection = await selectDeployment(
        currentConfig,
