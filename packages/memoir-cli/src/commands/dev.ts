...
async function showNotConfiguredPrompt(): Promise<boolean> {
  ...
  console.log(
    `   Press ${pc.bold(pc.green("i"))} to run ${pc.cyan("memoir init")} and set up a project`,
  );
...
async function runInteractiveDevMode(
  ...
  if (deploymentsList.length === 0) {
    ...
    if (deploymentsList.length === 0) {
      console.log(
        pc.red(
          "\n   Still no deployments configured. Please run 'memoir init' manually.\n",
        ),
      );
      process.exit(1);
    }
...
async function refreshStatus(state: DevState): Promise<void> {
  ...
  console.log(
    pc.cyan("║") + pc.bold(`   Memoir Dev Mode`).padEnd(width) + pc.cyan("║"),
  );
  console.log(pc.cyan("╚" + line + "╝"));
  console.log();
