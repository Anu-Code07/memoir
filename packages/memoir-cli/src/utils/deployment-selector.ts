 ...
 * Priority order: -d flag > memoir use > interactive/single
 */
 ...
 */
const CURRENT_DEPLOYMENT_PATH = join(homedir(), ".memoir-current");

/**
 * Get the current deployment set via `memoir use`
 */
...
/**
 * Set the current deployment via `memoir use`
 */
...
export async function selectDeployment(
  ...
  if (deployments.length === 0) {
    ...
    console.log(
      pc.dim(
        "   Run 'memoir init' or 'memoir config add-deployment' to add one\n",
      ),
    );
    return null;
  }
