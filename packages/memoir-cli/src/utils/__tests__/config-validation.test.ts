...
const mockMemoirClose: any = jest.fn();
...
const mockMemoir = Memoir as any;
...
describe("validateAndCleanConfig", () => {
  beforeEach(() => {
    ...
    // Reset Memoir mock to use the standard mock functions
    mockMemoir.mockImplementation(() => ({
      memorySpaces: { list: mockMemorySpacesList },
      close: mockMemoirClose,
    }));

    // Default: Memoir succeeds
  ...
  it("should handle partial failures correctly", async () => {
    ...
    mockMemoir.mockImplementation(({ convexUrl }: { convexUrl: string }) => {
      return {
        memorySpaces: {
