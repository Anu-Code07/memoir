...
async function createQuickstartEnv(
  ...
  let envContent = `# Memoir Quickstart Environment
...
export function getAppPidFilePath(
  ...
  return path.join(projectPath, `.memoir-app-${safeName}.pid`);
}
...
export async function startApp(appName: string, app: AppConfig): Promise<void> {
  ...
  const logFile = path.join(app.projectPath, `.memoir-app-${appName}.log`);
...
export async function findAppPidFiles(
  ...
  const pidFiles = files.filter(
    (f) => f.startsWith(".memoir-app-") && f.endsWith(".pid"),
  );

  return pidFiles.map((pidFile) => {
    // Extract app name from ".memoir-app-{name}.pid"
    const name = pidFile.replace(/^\.memoir-app-/, "").replace(/\.pid$/, "");
