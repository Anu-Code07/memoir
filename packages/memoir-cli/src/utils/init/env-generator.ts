...
export function generateEnvFile(config: WizardConfig): string {
  let env = `# Memoir SDK Configuration
# Generated by memoir init on ${new Date().toISOString()}
...
NEO4J_PASSWORD=${config.graphPassword || "memoir-password"}
`;
  }
...
export async function createEnvFile(
  ...
  if (hasExistingConvexConfig) {
    ...
    if (!existingContent.includes("# Memoir SDK Configuration")) {
      finalContent = `# Memoir SDK Configuration
# Generated by memoir init on ${new Date().toISOString()}
...
export async function createGraphDockerCompose(
  ...
  const graphPassword = password || "memoir-password";
...
`;
  ...
  memgraph:
    ...
    container_name: memoir-memgraph
    ports:
      - "7687:7687"  # Bolt
