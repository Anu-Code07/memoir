...
export async function startGraphContainers(
  ...
  const containerName = `memoir-${graphType}`;
...
async function waitForGraphReady(
  ...
  while ((Date.now() - startTime) / 1000 < maxWait) {
    const healthCheck = await execCommand(
      "docker",
      ["ps", "--filter", `name=memoir-${graphType}`, "--format", "{{.Status}}"],
      { quiet: true },
    );
...
async function initializeGraph() {
  ...
  await graphAdapter.connect({
    ...
    password: process.env.NEO4J_PASSWORD || 'memoir-password',
  });

  // 3. Initialize schema (constraints and indexes)
  await initializeGraphSchema(graphAdapter);

  // 4. Initialize Memoir with graph integration
  ...
  return { memoir, graphAdapter };
}

// Example usage
async function main() {
  const { memoir, graphAdapter } = await initializeGraph();

  try {
    // Use Memoir normally - graph sync happens automatically!
    await memoir.memory.remember({
      memorySpaceId: 'my-agent',
      conversationId: 'conv-1',
      userMessage: 'Hello, Memoir with graph!',
