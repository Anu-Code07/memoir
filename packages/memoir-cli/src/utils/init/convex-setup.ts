 ...
 * - The team slug (from output like "- Memoir (nicholasgeil)")
 *
...
export async function getConvexAuthStatus(): Promise<ConvexAuthStatus> {
  try {
    ...
    // Parse team from output: "- Memoir (nicholasgeil)"
...
export async function ensureConvexAuth(): Promise<ConvexAuthStatus> {
  ...
  if (!status.isLoggedIn) {
    ...
    const exitCode = await execCommandLive(
      "npx",
      ["convex", "login", "--device-name", "memoir-init"],
      {},
    );
...
export function sanitizeProjectName(name: string): string {
  ...
  return sanitized || "memoir-project";
}
...
export async function setupLocalConvex(): Promise<ConvexConfig> {
  ...
  console.log(
    pc.dim("   After setup, run 'memoir start --local' for local development"),
  );
  console.log(
...
/**
 * Deploy Memoir backend functions to Convex
 *
...
export async function deployToConvex(
  ...
  console.log(pc.cyan("\n   Deploying Memoir backend to Convex..."));
  ...
  try {
    ...
    if (exitCode !== 0) {
      throw new Error("Failed to deploy Memoir backend");
    }
