 ...
 */
const explorer = cosmiconfig("memoir", {
  searchPlaces: [
    "memoir.config.json",
    "memoir.config.js",
    ".memoirrc",
...
export function getProjectConfigPath(): string {
  return join(process.cwd(), "memoir.config.json");
}
...
export async function loadConfig(): Promise<CLIConfig> {
  ...
  // Try to load user config from ~/.memoirrc
 ...
 *
 * The CLI config (~/.memoirrc) is the source of truth.
 ...
 *   memoir --url <url> --key <key> <command>
 */
  ...
  if (!deploymentConfig) {
    throw new Error(
      `Deployment "${options.deployment ?? config.default}" not found in configuration. ` +
        `Run 'memoir setup' to configure deployments.`,
    );
  }
...
async function checkConvexUrl(
  ...
  let client: Memoir | undefined;

  try {
    client = new Memoir({ convexUrl: url });
