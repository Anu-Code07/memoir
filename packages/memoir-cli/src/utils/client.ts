/**
 * Memoir SDK Client Initialization
 *
 ...
 */
let cachedClient: Memoir | null = null;
let cachedUrl: string | null = null;

/**
 * Create a Memoir client from configuration and options
 */
...
export async function withClient<T>(
  config: CLIConfig,
  options: GlobalOptions,
  operation: (client: Memoir) => Promise<T>,
): Promise<T> {
  const client = createClient(config, options);
