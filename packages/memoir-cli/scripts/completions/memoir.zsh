...
_memoir() {
  ...
  local -a global_opts=(
    '-d[Deployment name to use]:deployment:_memoir_deployments'
    '--deployment[Deployment name to use]:deployment:_memoir_deployments'
  ...
  _arguments -C \
    $global_opts \
    '1:command:_memoir_commands' \
    '*::arg:->args'

  case $state in
    args)
      case $words[1] in
        config) _memoir_config ;;
        db) _memoir_db ;;
        convex) _memoir_convex ;;
        users) _memoir_users ;;
        spaces) _memoir_spaces ;;
        conversations|convs) _memoir_conversations ;;
        memory) _memoir_memory ;;
        facts) _memoir_facts ;;
        use) _memoir_use ;;
        start) _memoir_start ;;
        stop) _memoir_stop ;;
        deploy) _memoir_deploy ;;
        update) _memoir_update ;;
        status) _memoir_status ;;
        init) _memoir_init ;;
        dev) _memoir_dev ;;
        completion) _memoir_completion ;;
      esac
      ;;
  esac
}

# Dynamic deployment completion from ~/.memoirrc
_memoir_deployments() {
  local config="$HOME/.memoirrc"
  [[ -f "$config" ]] || return
  local -a deployments
...
# Dynamic app completion from ~/.memoirrc
_memoir_apps() {
  local config="$HOME/.memoirrc"
  [[ -f "$config" ]] || return
  local -a apps
...
_memoir_commands() {
  local -a commands=(
    ...
    'status:Show Memoir setup status dashboard'
    'config:Manage CLI configuration'
    'init:Initialize a new Memoir project'
    ...
    args)
      case $words[1] in
        ...
        set)
          ...
          ;;
        test)
          _arguments '-d[Deployment to test]:deployment:_memoir_deployments'
          ;;
        set-path)
          _arguments '1:deployment:_memoir_deployments' '2:path:_files -/'
          ;;
        enable|disable)
          _arguments '1:name:_memoir_deployments_and_apps'
          ;;
        deployments)
          _arguments '-f[Output format]:format:(table json)'
          ;;
        add-deployment)
          _arguments \
            ...
            '--json-only[Only save to ~/.memoirrc]'
          ;;
        remove-deployment)
          _arguments \
            '1:name:_memoir_deployments' \
            '--json-only[Only remove from ~/.memoirrc]'
          ;;
        set-key)
          _arguments \
            '1:deployment:_memoir_deployments' \
            '-k[Deploy key]:key:' \
            '--key[Deploy key]:key:' \
            '--json-only[Only update ~/.memoirrc]'
          ;;
        set-url)
          _arguments \
            '1:deployment:_memoir_deployments' \
  ...
  case $state in
    ...
    args)
      case $words[1] in
        stats)
          _arguments \
            '-d[Deployment]:deployment:_memoir_deployments' \
    ...
    args)
      case $words[1] in
        status|deploy|dev|logs|dashboard|update|schema|init)
          _arguments '-d[Deployment]:deployment:_memoir_deployments'
          ;;
        env)
          _arguments \
            '-d[Deployment]:deployment:_memoir_deployments' \
...
_memoir_start() {
  _arguments \
    '-d[Specific deployment to start]:deployment:_memoir_deployments' \
    '--deployment[Specific deployment to start]:deployment:_memoir_deployments' \
...
_memoir_stop() {
  _arguments \
    '-d[Specific deployment to stop]:deployment:_memoir_deployments' \
    '--deployment[Specific deployment to stop]:deployment:_memoir_deployments' \
...
_memoir_deploy() {
  _arguments \
    '-d[Deployment to deploy]:deployment:_memoir_deployments' \
    '--deployment[Deployment to deploy]:deployment:_memoir_deployments' \
...
_memoir_update() {
  _arguments \
    '-d[Specific deployment to update]:deployment:_memoir_deployments' \
    '--deployment[Specific deployment to update]:deployment:_memoir_deployments' \
...
_memoir_status() {
  _arguments \
    '-d[Specific deployment to check]:deployment:_memoir_deployments' \
    '--deployment[Specific deployment to check]:deployment:_memoir_deployments' \
    '-f[Output format]:format:(table json)'
}
...
_memoir_dev() {
  _arguments \
    '-d[Deployment to use]:deployment:_memoir_deployments' \
    '--deployment[Deployment to use]:deployment:_memoir_deployments'
}
