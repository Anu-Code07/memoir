...
function installCompletion(shell) {
  ...
  const sourceFile = join(completionsDir, `memoir.${shell}`);
  ...
  const targetFile = join(targetDir, `memoir.${shell}`);
...
function main() {
  ...
  try {
    ...
    // User can manually run `memoir completion <shell>` if needed
  }
}
