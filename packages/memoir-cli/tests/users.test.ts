...
describe("CLI User Commands", () => {
  ...
  const TEST_PREFIX = `cli-test-users-${TIMESTAMP}`;
  ...
  describe("users list", () => {
    beforeAll(async () => {
      // Create test users
      for (let i = 0; i < 3; i++) {
        await memoir.users.update(`${TEST_PREFIX}-user-${i}`, {
          name: `Test User ${i}`,
    ...
    it("should list users", async () => {
      const users = await memoir.users.list({ limit: 100 });

      expect(users.length).toBeGreaterThanOrEqual(3);
    });

    it("should respect limit", async () => {
      const users = await memoir.users.list({ limit: 2 });
  ...
  describe("users get", () => {
    ...
    it("should return null for non-existent user", async () => {
      const user = await memoir.users.get("non-existent-user-id");
  ...
  describe("users update", () => {
    ...
    beforeAll(async () => {
      await memoir.users.update(userId, { name: "Original Name" });
    });
    ...
    it("should increment version on update", async () => {
      const before = await memoir.users.get(userId);
      const beforeVersion = before!.version;

      await memoir.users.update(userId, { updated: true });

      const after = await memoir.users.get(userId);
  ...
  describe("users exists", () => {
    ...
    it("should return false for non-existent user", async () => {
      const exists = await memoir.users.exists("definitely-not-a-user");
  ...
  describe("users getHistory", () => {
    ...
    beforeAll(async () => {
      ...
      await memoir.users.update(userId, { version1: true });
      await memoir.users.update(userId, { version2: true });
      await memoir.users.update(userId, { version3: true });
    });
  ...
  describe("users delete", () => {
    it("should delete a user", async () => {
      ...
      // Verify deleted
      const afterDelete = await memoir.users.exists(userId);
    ...
    it("should cascade delete user data when requested", async () => {
      ...
      // Create user
      await memoir.users.update(userId, { name: "Cascade Test" });
  ...
  describe("users deleteMany", () => {
    beforeAll(async () => {
      ...
      for (let i = 0; i < 3; i++) {
        await memoir.users.update(`${TEST_PREFIX}-bulk-${i}`, {
          name: `Bulk Delete ${i}`,
        });
      }
    });
