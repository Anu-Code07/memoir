...
describeE2E("Memory Workflow E2E", () => {
  ...
  describe("Full Memory Lifecycle", () => {
    ...
    it("should store memory via vector store", async () => {
      const result = await memoir.vector.store(TEST_SPACE_ID, {
    ...
    it("should get a specific memory", async () => {
      const memory = await memoir.memory.get(TEST_SPACE_ID, testMemoryId);
    ...
    it("should return null for non-existent memory", async () => {
      const memory = await memoir.memory.get(
        TEST_SPACE_ID,
    ...
    it("should archive a memory", async () => {
      const result = await memoir.memory.archive(TEST_SPACE_ID, testMemoryId);

      expect(result.archived).toBe(true);

      // Verify archived
      const memory = await memoir.vector.get(TEST_SPACE_ID, testMemoryId);
      expect(memory?.tags).toContain("archived");
    });

    it("should restore an archived memory", async () => {
      const result = await memoir.memory.restoreFromArchive(
        TEST_SPACE_ID,
        testMemoryId,
      );
    ...
    it("should delete a specific memory", async () => {
      ...
      const stored = await memoir.vector.store(TEST_SPACE_ID, {
      ...
      const result = await memoir.memory.delete(TEST_SPACE_ID, stored.memoryId);

      expect(result.deleted).toBe(true);

      // Verify deleted
      const deleted = await memoir.vector.get(TEST_SPACE_ID, stored.memoryId);
    ...
    it("should delete multiple memories by filter", async () => {
      ...
      for (let i = 0; i < 3; i++) {
        await memoir.vector.store(TEST_SPACE_ID, {
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle large content storage", async () => {
      ...
      // Clean up
      await memoir.memory.delete(TEST_SPACE_ID, result.memoryId);
    });
  });
});
