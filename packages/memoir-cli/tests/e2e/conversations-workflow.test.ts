...
describeE2E("Conversations Workflow E2E", () => {
  ...
  describe("Conversation CRUD Operations", () => {
    ...
    it("should return null for non-existent conversation", async () => {
      const conv = await memoir.conversations.get("nonexistent-conv-id");
    ...
    it("should delete a conversation", async () => {
      ...
      const deleteConv = await memoir.conversations.create({
        type: "user-agent",
        memorySpaceId: TEST_SPACE_ID,
        participants: {
          userId: TEST_USER_ID,
      ...
      // Verify deleted
      const deleted = await memoir.conversations.get(deleteConv.conversationId);
  ...
  describe("Conversation Listing", () => {
    ...
    it("should list conversations", async () => {
      const result = await memoir.conversations.list({ limit: 100 });
    ...
    it("should respect limit", async () => {
      const result = await memoir.conversations.list({ limit: 2 });
      const convs = result.conversations || result;
