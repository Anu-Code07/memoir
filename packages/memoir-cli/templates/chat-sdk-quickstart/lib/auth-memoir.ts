/**
 * Auth.js to Memoir Auth Context Bridge
 *
 * Helpers to extract authentication context from Auth.js sessions
 * and convert to Memoir SDK AuthContext format.
 */
...
/**
 * Get Memoir AuthContext from the current Auth.js session.
 *
 * Call this in API routes to get a properly formatted auth context
 * that can be passed to Memoir SDK operations.
 *
 ...
 * ```
 */
export async function getMemoirAuthContext(): Promise<AuthContext | null> {
...
/**
 * Create Memoir AuthContext from an existing session object.
 *
 ...
 * const session = await auth();
 * if (session?.user) {
 *   const authContext = createMemoirAuthContextFromSession(session);
 *   // Use authContext...
 * }
 * ```
 */
export function createMemoirAuthContextFromSession(
