 ...
 *
 * These functions implement chat persistence using the Memoir SDK's Conversations API.
 * The Memoir Conversations API provides ACID-compliant, immutable storage for chat history.
 *
 * Schema Mapping:
 * | Chat SDK Field | Memoir Conversation Field |
 * |---------------|---------------------------|
 * | id            | conversationId            |
...
import { getMemoir, getMemorySpaceId, getAgentId } from "@/lib/memoir";
import type {
  Conversation,
...
/**
 * Convert a Memoir Conversation to a Chat SDK Chat
 */
...
/**
 * Convert a Memoir Message to a Chat SDK DBMessage
 */
function memoirMessageToDBMessage(
  message: MemoirMessage,
  chatId: string
): DBMessage {
  // Map Memoir roles to Chat SDK roles
  ...
  // Parse content - Memoir stores as string, Chat SDK uses parts
  let parts: unknown;
  try {
...
/**
 * Convert Chat SDK message parts to Memoir content string
 */
...
export async function getUser(
  ...
  // User management is handled by NextAuth, not Memoir
...
// ============================================================================
// Chat Queries (Memoir Conversations API)
// ============================================================================
...
export async function getChatById({
  ...
  const conversation = await memoir.conversations.get(id);
  if (!conversation) {
    return null;
  }
...
export async function getChatsByUserId({
  ...
  // Memoir uses offset-based pagination, convert cursor to offset if needed
...
export async function saveChat({
  ...
  const existing = await memoir.conversations.get(id);
  if (existing) {
...
export async function deleteChatById({ id }: { id: string }): Promise<Chat> {
  ...
  await memoir.conversations.delete(id);
...
// ============================================================================
// Message Queries (Memoir Conversations API)
// ============================================================================
...
export async function getMessagesByChatId({
  ...
  // Convert Memoir messages to DBMessages
  return chatMessages.map((msg) => memoirMessageToDBMessage(msg, id));
}
...
export async function getMessageById({
  ...
  // Since Memoir doesn't have a global message search by ID,
...
export async function saveMessages({
  ...
  // Map Chat SDK roles to Memoir roles
  ...
  for (const message of messages) {
    ...
    const memoirRole = roleMap[message.role] || "agent";
...
// ============================================================================
// Document Queries (Memoir Artifacts API)
// Documents are stored as Memoir Artifacts with full versioning support.
// ============================================================================

/**
 * Map Chat SDK artifact kinds to Memoir artifact kinds
 */
type MemoirArtifactKind = "text" | "code" | "sheet" | "image" | "diagram" | "html" | "custom";

function mapToMemoirKind(chatSdkKind: string): MemoirArtifactKind {
  switch (chatSdkKind) {
    case "text":
      return "text";
    case "code":
...
function mapFromMemoirKind(memoirKind: MemoirArtifactKind): string {
  // Direct mapping since Chat SDK kinds are a subset of Memoir kinds
  return memoirKind;
}

/**
 * Convert a Memoir Artifact to a Chat SDK Document
 */
function artifactToDocument(artifact: Artifact): Document {
  return {
    ...
    kind: mapFromMemoirKind(artifact.kind),
...
export async function getDocumentById({
  ...
  const artifact = await memoir.artifacts.get(id);
  if (!artifact) {
    return null;
  }
...
export async function getDocumentsById({
  ...
  const history = await memoir.artifacts.getHistory(id, {
    sortOrder: "desc",
  });
...
export async function saveDocument({
  ...
  const existing = await memoir.artifacts.get(id);

  if (existing) {
    // Update existing artifact (creates new version)
    const updated = await memoir.artifacts.update(id, content, {
      title,
...
export async function deleteDocumentsByIdAfterTimestamp({
  ...
  if (artifact.createdAt >= timestamp.getTime()) {
    ...
    await memoir.artifacts.delete(id, true);
  ...
  for (let i = 0; i < undoCount; i++) {
    await memoir.artifacts.undo(id);
  }
...
export async function getDocumentVersion({
  ...
  const versionData = await memoir.artifacts.getVersion(id, version);
  if (!versionData) {
    return null;
  }
...
export async function undoDocumentChange({
  ...
  try {
    const result = await memoir.artifacts.undo(id);
    return {
...
export async function redoDocumentChange({
  ...
  try {
    const result = await memoir.artifacts.redo(id);
    return {
      success: result.success,
      version: result.currentVersion,
    };
