# Memoir Integration Guide

This document provides detailed information about how Memoir is integrated into the Chat SDK template.

## Overview

Memoir transforms a stateless AI chatbot into one with persistent, intelligent memory. Every conversation is automatically stored, indexed, and made retrievable for future interactions.
...
Auth context flows from Auth.js to Memoir in `lib/auth-memoir.ts`:

```typescript
import { auth } from "@/app/(auth)/auth";

export async function getMemoirAuthContext() {
  const session = await auth();
...
Memoir intelligently handles conflicting information:
...
Memoir tables are automatically created when you deploy. The app-specific tables are in `convex/schema.ts`:

```typescript
// App-specific tables only
// Memoir tables (conversations, memories, facts, etc.) are auto-managed
...
- [Vercel AI Provider API Reference](https://github.com/Anu-Code07/memoir/docs/integrations/vercel-ai-sdk)
- [Convex Documentation](https://docs.convex.dev)
- [Example Implementations](https://github.com/Anu-Code07/memoir/examples)
