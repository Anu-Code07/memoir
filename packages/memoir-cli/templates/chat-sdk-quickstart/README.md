...
<p align="center">
  <img alt="Memoir Chat SDK - AI chatbot with persistent memory" src="app/(chat)/opengraph-image.png">
</p>

<p align="center">
  A production-ready AI chatbot with persistent memory, powered by <strong>Memoir</strong>.
</p>

<p align="center">
  <a href="#features"><strong>Features</strong></a> ·
  <a href="#quick-start"><strong>Quick Start</strong></a> ·
  <a href="#memoir-features"><strong>Memoir</strong></a> ·
...
- **AI**: [Vercel AI SDK](https://ai-sdk.dev) with Memoir Provider
...
### Option 1: Using Memoir CLI (Recommended)

```bash
# Create a new project
npx memoir create my-chatbot --template chat-sdk
...
```bash
# Clone the template
git clone https://github.com/Anu-Code07/memoir/chat-sdk-quickstart my-chatbot
cd my-chatbot
...
| `MEMOIR_FACT_EXTRACTION` | No | Enable LLM-powered fact extraction (default: `true`) |
| `MEMOIR_FACT_EXTRACTION_MODEL` | No | Model for fact extraction (default: `gpt-4o-mini`) |
...
|----------|-------------|
| `MEMOIR_GRAPH_SYNC` | Enable automatic graph sync (`true`/`false`) |
...
This template showcases the full power of Memoir orchestration. Every message flows through multiple memory layers.
...
When users change their preferences, Memoir intelligently updates existing facts:
...
│   │   ├── api/chat/        # Main chat endpoint with Memoir
...
├── components/
│   ├── memory-retrieval-panel.tsx  # Memoir memory visualization
...
│   ├── memoir-config.ts  # Memoir configuration factory
│   ├── memoir.ts            # Memoir SDK helpers
│   └── db/
...
const memoirConfig = getMemoirMemoryConfig(
  getMemorySpaceId(),
  session.user.id,
  chatId,
...
// Wrap your model with Memoir
const memoirMemory = await createMemoirMemoryAsync(memoirConfig);

const result = streamText({
  model: memoirMemory(getLanguageModel(selectedModel)),
  messages,
  // ... tools and options
});
```
...
`lib/memoir-config.ts` — Centralized memory configuration:

```typescript
export function getMemoirMemoryConfig(
...
[![Deploy with Vercel](https://vercel.com/button)](https://vercel.com/new/clone?repository-url=https://github.com/Anu-Code07/memoir/chat-sdk-quickstart)
...
- **Replaced PostgreSQL** with Memoir (Convex-backed)
- **Added Memoir orchestration** for persistent AI memory
