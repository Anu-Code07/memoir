...
function getReasoningType(part: unknown): ReasoningType {
  // Check providerMetadata.memoir.memoryPhase which is passed from the server
  const providerMetadata = (part as { providerMetadata?: Record<string, unknown> })?.providerMetadata;
  const memoirMeta = providerMetadata?.memoir as { memoryPhase?: string } | undefined;
  if (memoirMeta?.memoryPhase === "recall") return "memory-recall";
  if (memoirMeta?.memoryPhase === "storage") return "memory-storage";
  return "llm";
}
