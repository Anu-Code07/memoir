 ...
 *
 * Tests the conversations API routes with mocked Memoir SDK.
 */
...
import {
  ...
  seedTestData,
  type MockMemoir,
} from "../helpers/mock-memoir";

// Mock the Memoir SDK module
let mockMemoir: MockMemoir;

jest.mock("../../lib/memoir", () => ({
  getMemoir: () => mockMemoir,
}));
...
describe("Conversations API Routes", () => {
  ...
  describe("GET /api/conversations", () => {
    ...
    it("should use default memorySpaceId if not provided", async () => {
      ...
      expect(mockMemoir.conversations.list).toHaveBeenCalledWith(
  ...
  describe("DELETE /api/conversations", () => {
    ...
    it("should call memoir.conversations.delete with correct ID", async () => {
      const request = createRequest("DELETE", {
        searchParams: { conversationId: "conv-123" },
      });
      await conversationsDelete(request);

      expect(mockMemoir.conversations.delete).toHaveBeenCalledWith("conv-123");
    });
  });
  ...
  describe("edge cases", () => {
    it("should handle SDK errors gracefully for GET", async () => {
      mockMemoir.conversations.list = jest
        .fn()
    ...
    it("should handle SDK errors gracefully for POST", async () => {
      mockMemoir.conversations.create = jest
        .fn()
        .mockRejectedValue(new Error("SDK error"));
    ...
    it("should handle SDK errors gracefully for DELETE", async () => {
      mockMemoir.conversations.delete = jest
        .fn()
        .mockRejectedValue(new Error("SDK error"));
