 ...
 *
 * Tests the complete chat pipeline with mocked Memoir SDK.
...
// Create mock memoir that uses the module-level state
const createMockMemoir = () => ({
  memory: {
    recall: vi.fn(async () => {
      recallCalled = true;
      if (recallShouldFail) {
...
let mockMemoir = createMockMemoir();

// Mock the memoir module
vi.mock("../../memoir.js", () => ({
  getMemoir: vi.fn(() => mockMemoir),
  closeMemoir: vi.fn(),
  CONFIG: {
