/**
 * Memoir - HTTP Server Mode
 *
 * REST API server for demonstrating Memoir SDK.
...
import { closeMemoir, initMemoir, CONFIG } from "./memoir.js";
...
app.get("/facts", async (c) => {
  try {
    ...
    const result = await memoir.facts.list({
      memorySpaceId: CONFIG.memorySpaceId,
      limit: 50,
    });
...
async function main(): Promise<void> {
  ...
  if (!process.env.CONVEX_URL) {
    printError(
      "CONVEX_URL is required. Set it in .env.local or run: memoir init",
    );
    process.exit(1);
  }
  ...
  // Initialize Memoir client (async for graph support)
  // v0.29.0+: Uses Memoir.create() for automatic graph configuration
  try {
    await initMemoir();
    ...
    printError("Failed to initialize Memoir", error instanceof Error ? error : undefined);
    process.exit(1);
  }
