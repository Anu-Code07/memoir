/**
 * Memoir SDK Client Configuration
 *
...
export const CONFIG = {
  ...
  agentName: process.env.AGENT_NAME || "Memoir CLI Assistant",

  // Feature flags
  enableFactExtraction: process.env.MEMOIR_FACT_EXTRACTION !== "false",
...
let initPromise: Promise<Memoir> | null = null;

/**
 * Initialize the Memoir SDK client (async)
 *
 * Uses Memoir.create() for automatic graph configuration from env vars.
 * When MEMOIR_GRAPH_SYNC=true and NEO4J_URI (or MEMGRAPH_URI) is set,
 ...
 */
export async function initMemoir(): Promise<Memoir> {
  if (memoirClient) {
    return memoirClient;
  }
  ...
  initPromise = (async () => {
    ...
    if (!convexUrl) {
      throw new Error(
        "CONVEX_URL environment variable is required.\n" +
          "Set it in .env.local or run: memoir init",
      );
    }
    ...
    if (process.env.OPENAI_API_KEY && CONFIG.enableFactExtraction) {
      config.llm = {
        ...
        model: process.env.MEMOIR_FACT_EXTRACTION_MODEL || "gpt-4o-mini",
      };
    }

    // Use Memoir.create() for async initialization with auto graph configuration
    // This automatically:
    // - Detects MEMOIR_GRAPH_SYNC=true
    // - Reads NEO4J_URI/MEMGRAPH_URI and credentials from env
    // - Creates and connects the CypherGraphAdapter
    memoirClient = await Memoir.create(config);
    return memoirClient;
  })();
...
/**
 * Get the Memoir SDK client (must call initMemoir first)
 *
...
export function getMemoir(): Memoir {
  if (!memoirClient) {
    throw new Error(
      "Memoir client not initialized. Call initMemoir() first.",
    );
  }
  return memoirClient;
}
...
/**
 * Close the Memoir client connection
 */
export function closeMemoir(): void {
  if (memoirClient) {
    memoirClient.close();
    memoirClient = null;
  }
...
export async function buildRememberParams(
  ...
  const params: RememberParams = {
    ...
    // by the llmConfig on the Memoir client when enableFactExtraction is true
  };
